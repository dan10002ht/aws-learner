...
| 4 | Lịch sử netcode 1993 → nay | Doom lockstep → Quake client-server → QuakeWorld prediction (Carmack) → Valve lag compensation (Bernier, GDC 2001) → GGPO rollback |
...
| 7 | Đồng hồ & scheduling | `sleep(dt)` vs deadline tuyệt đối; drift cộng dồn và tăng theo tải; monotonic vs wall clock; busy-wait đổi CPU lấy p99; vì sao metric luôn xanh là metric không so với gì cả |
...
| 11 | Fixed-point & simulation tất định | Vì sao rollback thật phải bỏ float; số nguyên Q16.16; sqrt/sin bằng bảng tra; test đối chiếu hai nền tảng |
...
| 15 | Giới hạn vật lý của gói tin | MTU và mốc ~1200 byte an toàn; fragmentation nhân xác suất mất gói; packet loss vs jitter vs bandwidth; vì sao snapshot phải tự giới hạn kích thước |
| 16 | NAT, kết nối & phục hồi | Vì sao dedicated server không cần hole punching còn P2P thì cần; STUN/TURN; connection lifecycle; reconnect & session resume không mất trận |
...
| # | Bài | Nội dung lõi |
|---|---|---|
| 17 | Input pipeline & đồng bộ tick | Input có `seq` + tick; client chạy trước server một khoảng; input buffer ở server; sửa lệch bằng co giãn tick chứ không nhảy |
...
| 19 | Server reconciliation | Nhận state kèm `lastProcessedSeq` → rewind → replay; ngưỡng bỏ qua sai lệch nhỏ; vì sao lệch một chút là rung liên tục |
...
| 21 | Lag compensation | Server lưu lịch sử vị trí ~1 giây, tua ngược world về thời điểm người bắn đã thấy; vì sao bạn chết sau khi đã nấp; giới hạn công bằng |
| 22 | Rollback netcode & lockstep | GGPO: tua ngược 7 frame, thay input, chạy lại trong 1 frame; vì sao fighting game chọn nhánh này; lockstep cho RTS; điều kiện determinism tuyệt đối |

## Chương 6 — Đồng bộ state & băng thông (5 bài)

| # | Bài | Nội dung lõi |
|---|---|---|
| 23 | Ba mô hình replication | Full snapshot vs delta vs event-based; ai chịu được mất gói, ai không; vì sao event-based cần reliable |
...
| 25 | Delta compression | Baseline theo snapshot client đã ack; vòng đời baseline; chuyện gì xảy ra khi ack mất; bộ nhớ phải giữ bao nhiêu snapshot |
| 26 | Area of Interest | Grid/cell, radius, visibility; vì sao AOI giảm băng thông nhiều hơn mọi thủ thuật serialization; entity vào/ra tầm nhìn |
...
| 30 | Matchmaking | Queue, MMR/ELO, region & ping, thời gian chờ vs chất lượng trận; backfill; vì sao matchmaking là bài toán tối ưu chứ không phải hàng đợi |
| 31 | Persistence & meta services | Ba tầng state (simulation/session/persistent); DB ngoài critical path; snapshot theo chu kỳ; graceful shutdown & drain |
| 32 | Sharding world & handoff | Chia zone/grid khi một trận quá đông; cross-node entity handoff; vì sao cắt thế giới là quyết định thiết kế game chứ không phải hạ tầng |

## Chương 8 — Chống gian lận (3 bài)

| # | Bài | Nội dung lõi |
|---|---|---|
| 33 | Authority: intent vs result | Client gửi ý định, không gửi kết quả; mọi tham số trỏ tới thế giới là một khẳng định cần kiểm chứng; cái giá của authoritative |
| 34 | Phân loại cheat & phòng thủ | Speed hack, teleport, aimbot, wallhack, item dup, replay attack; cái nào server chặn được tuyệt đối, cái nào chỉ giảm được |
| 35 | Phát hiện & vận hành chống gian lận | Thống kê server-side (aim pattern, input bất khả thi); replay verification; anti-cheat phía client và giới hạn của nó; xử lý false positive |
...
| 37 | Zero-allocation hot loop | GC là kẻ thù; pre-allocate, `sync.Pool`, tránh interface boxing/closure; `GOGC`/`GOMEMLIMIT`; bố trí dữ liệu quan trọng hơn số phép tính (SoA/ECS-lite) |
| 38 | Concurrency đúng cách | 1 goroutine/connection cho I/O, single-threaded cho simulation; channel vs mutex; backpressure khi client chậm; tránh goroutine leak |
| 39 | Load test bằng bot client | Bot dùng lại client code thật; hành vi phải động; chạy bot ở máy khác; đo cả phía client; tìm cái gãy trước — CPU, băng thông hay fd |
...
| # | Bài | Nội dung lõi |
|---|---|---|
| 41 | agar-lite phần 1 — chạy được | Tick loop + WebSocket + room; input → simulation → broadcast; cố ý thêm độ trễ giả lập để *cảm* thấy 182 ms |
| 42 | agar-lite phần 2 — chơi được | Prediction + reconciliation + interpolation; AOI; hit detection có lag compensation; đo lại toàn bộ số và so với chương 2 |
