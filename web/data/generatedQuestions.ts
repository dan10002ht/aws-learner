...
,
  {"id":"saa-m1-001","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty có nhiều ứng dụng EC2 cần truy cập một S3 bucket để lưu log. Hiện tại developer đang nhúng access key vào code chạy trên các instance này. Là Solutions Architect, bạn cần loại bỏ long-term credentials mà vẫn cấp quyền truy cập S3 với least operational overhead. Giải pháp nào phù hợp nhất?","options":["Tạo IAM role với policy truy cập S3 và gán role đó vào instance profile của các EC2 instance","Tạo một IAM user dùng chung, gắn policy S3, rồi phân phối access key cho tất cả instance qua Systems Manager Parameter Store","Lưu access key trong Secrets Manager và để ứng dụng đọc key khi khởi động","Bật S3 bucket policy cho phép truy cập public và giới hạn bằng VPC endpoint"],"correctIndices":[0],"explanation":"IAM role gắn qua instance profile cung cấp temporary credentials tự động xoay vòng, loại bỏ hoàn toàn long-term key.\n✓ IAM role + instance profile: EC2 nhận temporary credentials qua STS, không cần quản lý key, least operational overhead.\n✗ IAM user dùng chung + access key: vẫn là long-term credentials, vi phạm best practice và khó xoay vòng.\n✗ Lưu access key trong Secrets Manager: vẫn tồn tại long-term key, thêm phức tạp vận hành.\n✗ Bucket policy public: tạo lỗ hổng bảo mật, không giải quyết vấn đề credential.","domain":1,"mock":1},
  {"id":"saa-m2-001","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một tổ chức quản lý nhiều AWS account bằng AWS Organizations. Security team muốn đảm bảo rằng KHÔNG account thành viên nào, kể cả admin của account đó, có thể tắt CloudTrail hoặc tạo tài nguyên ngoài region eu-west-1. Giải pháp nào thực thi điều này một cách tập trung và hiệu quả nhất?","options":["Áp dụng Service Control Policy (SCP) ở cấp OU để deny các action cloudtrail:StopLogging và deny mọi action ngoài eu-west-1","Gắn IAM permission boundary cho mọi IAM user trong các account thành viên","Tạo IAM policy deny gắn trực tiếp vào từng role admin của mỗi account","Bật AWS Config rule để phát hiện và tự động bật lại CloudTrail khi bị tắt"],"correctIndices":[0],"explanation":"SCP đặt giới hạn quyền tối đa cho toàn bộ account thành viên, kể cả admin của account đó, và được quản lý tập trung.\n✓ SCP ở cấp OU: chặn được mọi IAM principal trong account (kể cả admin), thực thi tập trung, đúng mục tiêu phòng ngừa.\n✗ Permission boundary cho từng user: chỉ giới hạn user/role cụ thể, admin có thể bỏ qua bằng cách tạo entity khác, không tập trung.\n✗ IAM deny policy trên từng role: admin có thể chỉnh sửa hoặc gỡ policy, không ngăn chặn triệt để.\n✗ AWS Config auto-remediation: mang tính phản ứng (detective) chứ không phòng ngừa, CloudTrail vẫn bị tắt trong khoảng thời gian giữa.","domain":1,"mock":2},
  {"id":"saa-m3-001","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Account A chứa một S3 bucket cần được account B truy cập đọc dữ liệu. Yêu cầu là account B chỉ được cấp quyền tạm thời và account A muốn kiểm soát chính xác principal nào ở account B được phép. Cách triển khai cross-account access nào là tối ưu và an toàn nhất?","options":["Tạo IAM role trong account A với trust policy cho phép account B AssumeRole, và account B dùng STS AssumeRole để lấy temporary credentials","Tạo IAM user trong account A và chia sẻ access key với account B","Gắn bucket policy trong account A cho phép Principal là toàn bộ account B (root)","Bật S3 Access Points và cấp quyền public-read cho bucket"],"correctIndices":[0],"explanation":"IAM role với trust policy cung cấp cross-account access bằng temporary credentials và cho phép kiểm soát chi tiết principal được tin tưởng.\n✓ IAM role + STS AssumeRole: temporary credentials, trust policy kiểm soát chính xác ai được assume, best practice cho cross-account.\n✗ Chia sẻ access key của IAM user: long-term credentials, rủi ro cao, vi phạm least privilege.\n✗ Bucket policy cho toàn bộ root account B: cấp quyền quá rộng, mọi principal account B đều truy cập được, không kiểm soát chi tiết.\n✗ Public-read access point: lộ dữ liệu ra ngoài, không phù hợp yêu cầu cấp quyền có kiểm soát.","domain":1,"mock":3},
  {"id":"saa-m1-002","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một IAM user có một identity-based policy ALLOW s3:GetObject trên bucket X. Account chứa user này nằm trong OU có SCP chỉ cho phép các action EC2 (không bao gồm S3). Đồng thời bucket X (cùng account) có bucket policy ALLOW user đó GetObject. Khi user gọi s3:GetObject, kết quả là gì?","options":["Bị từ chối, vì SCP không cho phép S3 nên permission tối đa của account đã loại trừ s3:GetObject","Được phép, vì cả identity-based policy lẫn resource-based policy đều ALLOW","Được phép, vì resource-based policy luôn ưu tiên hơn SCP","Bị từ chối, vì cần thêm một explicit allow trong permission boundary"],"correctIndices":[0],"explanation":"SCP đặt trần quyền tối đa; nếu action không nằm trong SCP allow thì coi như implicit deny, request bị chặn dù các policy khác allow.\n✓ Bị từ chối do SCP: SCP chỉ allow EC2, s3:GetObject nằm ngoài giới hạn nên bị implicit deny ở cấp account.\n✗ Được phép vì identity + resource policy allow: SCP được đánh giá và giới hạn permission tối đa, nên các allow này không có hiệu lực.\n✗ Resource-based policy ưu tiên hơn SCP: sai, SCP luôn giới hạn boundary của account trước.\n✗ Cần explicit allow trong permission boundary: tình huống không đề cập permission boundary; vấn đề nằm ở SCP.","domain":1,"mock":1},
  {"id":"saa-m2-002","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty dùng Active Directory on-premises và muốn cho phép nhân viên đăng nhập AWS Management Console bằng tài khoản AD hiện có, không tạo IAM user riêng cho từng người, và quản lý quyền tập trung trên nhiều account trong Organizations. Giải pháp nào phù hợp nhất với least operational overhead?","options":["Triển khai AWS IAM Identity Center, kết nối với AD qua AWS Managed Microsoft AD hoặc AD Connector, và gán permission set cho các account","Tạo IAM user cho từng nhân viên trong mỗi account và đồng bộ mật khẩu thủ công với AD","Thiết lập SAML federation trực tiếp giữa AD FS và từng IAM identity provider trong mỗi account","Dùng Amazon Cognito user pool để federate nhân viên vào console"],"correctIndices":[0],"explanation":"IAM Identity Center tích hợp AD và quản lý quyền truy cập đa account tập trung qua permission set, giảm tối đa vận hành.\n✓ IAM Identity Center + AD: SSO tập trung, gán permission set cho nhiều account, không cần IAM user, least operational overhead.\n✗ IAM user từng account đồng bộ mật khẩu: vận hành cực lớn, không scale, không tập trung.\n✗ SAML federation riêng cho từng account: cấu hình IdP lặp lại ở mỗi account, vận hành nặng so với Identity Center.\n✗ Cognito user pool: dành cho ứng dụng web/mobile customer-facing, không tối ưu cho SSO console của nhân viên với AD.","domain":1,"mock":2},
  {"id":"saa-m3-002","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một mobile app cần cho phép hàng triệu người dùng cuối upload ảnh lên S3. App dùng Google và Facebook để đăng nhập. Bạn cần cấp temporary AWS credentials cho từng người dùng với quyền giới hạn, không tạo IAM user cho mỗi người. Giải pháp nào tối ưu?","options":["Dùng Amazon Cognito identity pool để federate người dùng qua các OIDC provider và cấp temporary credentials qua STS","Tạo một IAM role chung và nhúng access key của role vào app","Tạo IAM user cho mỗi người dùng cuối thông qua API tự động","Dùng SCP để giới hạn quyền upload của từng người dùng"],"correctIndices":[0],"explanation":"Cognito identity pool được thiết kế để federate người dùng web/mobile qua social IdP và cấp temporary AWS credentials theo từng người với quyền giới hạn.\n✓ Cognito identity pool + OIDC: web identity federation, temporary credentials qua STS, scale tới hàng triệu user, không tạo IAM user.\n✗ Nhúng access key của role: role không có long-term key để nhúng; nhúng key tĩnh trong app là rủi ro nghiêm trọng.\n✗ IAM user cho mỗi người dùng: vượt quota IAM user và không scale tới hàng triệu người.\n✗ SCP: dùng cho account trong Organizations, không liên quan tới người dùng cuối của app.","domain":1,"mock":3},
  {"id":"saa-m1-003","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một nền tảng cho phép nhiều team tự tạo IAM role cho ứng dụng của họ. Security team lo ngại các team sẽ tự cấp quyền quá rộng (ví dụ AdministratorAccess). Yêu cầu: cho phép team tạo role nhưng đảm bảo các role họ tạo KHÔNG BAO GIỜ có quyền vượt quá một tập giới hạn cho trước. Cơ chế nào phù hợp nhất?","options":["Buộc các role do team tạo phải gắn một permission boundary do security team định nghĩa, và dùng IAM policy điều kiện để chỉ cho phép tạo role khi có boundary đó","Áp dụng SCP deny mọi action iam:CreateRole cho các team","Tạo sẵn tất cả role và không cho team quyền tạo role nào","Gắn AWS Config rule cảnh báo khi role có quyền quá rộng"],"correctIndices":[0],"explanation":"Permission boundary đặt trần quyền tối đa cho một entity; bắt buộc gắn boundary khi tạo role cho phép delegation an toàn.\n✓ Permission boundary bắt buộc khi tạo role: role do team tạo không bao giờ vượt boundary, vẫn cho phép team tự tạo role, đúng yêu cầu delegation.\n✗ SCP deny CreateRole: chặn hoàn toàn việc tạo role, mâu thuẫn với yêu cầu cho team tự tạo.\n✗ Tạo sẵn mọi role: không scale, mất tính tự phục vụ, vận hành nặng.\n✗ Config rule cảnh báo: detective, role rộng quyền vẫn tồn tại đến khi được phát hiện và sửa.","domain":1,"mock":1},
  {"id":"saa-m2-003","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"multi","question":"Để bảo vệ root user của AWS account theo best practice, Solutions Architect nên thực hiện những hành động nào? (Chọn 2)","options":["Bật MFA cho root user","Khóa lại access key của root user và không tạo access key cho root để dùng hàng ngày","Dùng root user cho mọi hoạt động vận hành hàng ngày để dễ kiểm soát","Chia sẻ mật khẩu root cho toàn bộ admin team để đảm bảo tính sẵn sàng","Gắn SCP trực tiếp lên root user để giới hạn quyền"],"correctIndices":[0,1],"explanation":"Bảo vệ root gồm bật MFA và loại bỏ/không dùng access key của root, chỉ dùng root cho các tác vụ bắt buộc.\n✓ Bật MFA cho root: lớp bảo vệ thiết yếu chống chiếm tài khoản.\n✓ Không tạo/khóa access key root: tránh long-term credentials quyền tối cao bị lộ.\n✗ Dùng root hàng ngày: vi phạm least privilege, nên dùng IAM role/user thường ngày.\n✗ Chia sẻ mật khẩu root: cực kỳ rủi ro, mất khả năng truy vết và kiểm soát.\n✗ SCP trực tiếp lên root user: SCP áp dụng cho account/OU, không gắn lên một principal cụ thể.","domain":1,"mock":2},
  {"id":"saa-m3-003","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty có hàng nghìn project, mỗi project cần một bộ tài nguyên EC2/S3 riêng. Họ muốn một mô hình phân quyền nơi mỗi engineer chỉ truy cập tài nguyên có tag Project khớp với tag Project gắn trên principal của họ, tránh phải viết policy riêng cho từng project. Cách tiếp cận nào tối ưu?","options":["Dùng ABAC: gắn tag cho principal và resource, viết một IAM policy dùng điều kiện aws:PrincipalTag khớp với aws:ResourceTag","Dùng RBAC: tạo một IAM role riêng cho mỗi project và viết policy liệt kê ARN cụ thể","Tạo một group cho mỗi project và gán user vào group tương ứng theo cách thủ công","Dùng SCP với điều kiện tag để giới hạn từng project"],"correctIndices":[0],"explanation":"ABAC dùng tag trên principal và resource để cấp quyền động, cho phép một policy duy nhất phục vụ hàng nghìn project và scale tốt.\n✓ ABAC với PrincipalTag/ResourceTag: một policy duy nhất, tự động khớp quyền theo tag, scale tới hàng nghìn project, ít bảo trì.\n✗ RBAC với role/policy riêng từng project: bùng nổ số lượng policy/role, vận hành nặng khi có hàng nghìn project.\n✗ Group thủ công cho mỗi project: không scale, vẫn cần policy riêng cho từng group.\n✗ SCP theo tag: SCP là guardrail cấp account/OU, không phải cơ chế phân quyền chi tiết động cho engineer.","domain":1,"mock":3},
  {"id":"saa-m1-004","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một Lambda function cần ghi vào DynamoDB và đọc một secret từ Secrets Manager. Solutions Architect muốn cấp quyền theo least privilege và least operational overhead. Cách nào đúng nhất?","options":["Tạo một IAM execution role cho Lambda với policy chỉ cho phép các action cụ thể trên đúng bảng DynamoDB và đúng secret ARN","Tạo IAM user, sinh access key và lưu vào biến môi trường của Lambda","Gắn AdministratorAccess vào execution role để đảm bảo function chạy được","Dùng resource-based policy trên Lambda để cấp quyền truy cập DynamoDB"],"correctIndices":[0],"explanation":"Lambda execution role với policy giới hạn đúng action và đúng ARN tài nguyên là cách chuẩn theo least privilege.\n✓ Execution role với quyền cụ thể: temporary credentials tự động, chỉ đúng quyền cần thiết, ít vận hành.\n✗ IAM user + access key trong env: long-term credentials, rủi ro lộ key, sai best practice.\n✗ AdministratorAccess: vi phạm least privilege nghiêm trọng.\n✗ Resource-based policy trên Lambda: kiểm soát ai được gọi Lambda, không cấp quyền cho Lambda truy cập DynamoDB.","domain":1,"mock":1},
  {"id":"saa-m2-004","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Trong policy evaluation logic của IAM, một request có: identity-based policy ALLOW action, không có SCP nào liên quan, permission boundary của principal KHÔNG bao gồm action đó, và resource-based policy (cùng account) có explicit ALLOW cho principal. Kết quả cuối cùng đối với một IAM role principal là gì?","options":["Bị từ chối, vì permission boundary giới hạn quyền hiệu lực và action không nằm trong boundary","Được phép, vì resource-based policy explicit ALLOW luôn thắng permission boundary","Được phép, vì identity-based ALLOW kết hợp resource-based ALLOW là đủ","Bị từ chối, vì thiếu explicit ALLOW trong SCP"],"correctIndices":[0],"explanation":"Permission boundary đặt trần quyền hiệu lực của principal; trong cùng account, action phải được cả identity policy lẫn boundary cho phép.\n✓ Bị từ chối do permission boundary: quyền hiệu lực là giao của identity policy và boundary; action ngoài boundary nên không được phép trong cùng account.\n✗ Resource-based luôn thắng boundary: chỉ đúng trong cross-account; ở cùng account vẫn chịu giới hạn của boundary.\n✗ Identity + resource ALLOW là đủ: bỏ qua giới hạn của permission boundary trong cùng account.\n✗ Thiếu ALLOW trong SCP: đề nêu không có SCP liên quan, không phải nguyên nhân.","domain":1,"mock":2},
  {"id":"saa-m3-004","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một team có 15 developer cần cùng một bộ quyền truy cập vào môi trường dev. Quản trị viên muốn quản lý quyền cho cả nhóm một cách hiệu quả và dễ bảo trì khi có người vào/ra. Cách tổ chức IAM nào phù hợp nhất?","options":["Tạo một IAM group cho team dev, gắn policy vào group, và thêm/bớt user vào group","Gắn policy trực tiếp vào từng IAM user của 15 developer","Tạo một IAM user dùng chung cho cả 15 developer","Tạo 15 IAM role và yêu cầu mỗi developer assume role của mình"],"correctIndices":[0],"explanation":"IAM group cho phép quản lý quyền tập trung cho nhiều user và dễ thêm/bớt thành viên.\n✓ IAM group + policy: quản lý quyền một nơi, thêm/bớt user đơn giản, dễ bảo trì.\n✗ Policy gắn trực tiếp từng user: trùng lặp, khó đồng bộ khi thay đổi quyền.\n✗ User dùng chung: mất khả năng truy vết hành động từng cá nhân, rủi ro bảo mật.\n✗ 15 role riêng: phức tạp không cần thiết cho nhóm có quyền giống nhau.","domain":1,"mock":3},
  {"id":"saa-m1-005","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng on-premises cần gọi AWS API để upload dữ liệu vào S3. Yêu cầu là không lưu long-term IAM credentials trên máy chủ on-premises. Hệ thống on-premises đã có một OIDC identity provider. Giải pháp nào tối ưu để cấp quyền tạm thời?","options":["Cấu hình IAM OIDC identity provider và để ứng dụng dùng web identity token gọi STS AssumeRoleWithWebIdentity lấy temporary credentials","Tạo IAM user, sinh access key và lưu trong file cấu hình trên máy chủ on-premises","Dùng IAM Roles Anywhere với access key tĩnh","Tạo một IAM role và chia sẻ secret key của role cho ứng dụng on-premises"],"correctIndices":[0],"explanation":"OIDC federation cho phép workload bên ngoài đổi token sang temporary AWS credentials mà không cần lưu long-term key.\n✓ IAM OIDC provider + AssumeRoleWithWebIdentity: dùng token OIDC sẵn có, nhận temporary credentials, không lưu long-term key.\n✗ IAM user access key trong file: chính là long-term credentials cần tránh.\n✗ IAM Roles Anywhere với access key tĩnh: Roles Anywhere dùng X.509 certificate, không dùng access key tĩnh; mô tả sai.\n✗ Chia sẻ secret key của role: role không có long-term secret key để chia sẻ.","domain":1,"mock":1},
  {"id":"saa-m2-005","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Account Production (A) chứa role có quyền deploy. Một CI/CD pipeline chạy trong account Tooling (B) cần assume role này. Security team yêu cầu chỉ pipeline cụ thể được assume, ngăn 'confused deputy', kể cả khi role ARN bị lộ. Biện pháp bổ sung nào trong trust policy là phù hợp nhất?","options":["Thêm điều kiện sts:ExternalId (hoặc aws:PrincipalArn cụ thể) trong trust policy của role ở account A","Cấp quyền AdministratorAccess cho role ở account A để pipeline chạy trơn tru","Dùng access key của một IAM user trong account A và lưu trong pipeline","Mở trust policy cho phép Principal là * và lọc bằng bucket policy"],"correctIndices":[0],"explanation":"ExternalId (hoặc giới hạn principal ARN cụ thể) trong trust policy chống confused deputy và giới hạn chính xác ai được assume.\n✓ sts:ExternalId / PrincipalArn cụ thể: chỉ caller biết đúng ExternalId hoặc đúng ARN mới assume được, chống confused deputy.\n✗ AdministratorAccess: không liên quan tới kiểm soát ai được assume, lại vi phạm least privilege.\n✗ Access key IAM user trong pipeline: long-term credentials, đi ngược best practice AssumeRole.\n✗ Principal * + bucket policy: trust policy quá mở, bucket policy không kiểm soát việc assume role.","domain":1,"mock":2},
  {"id":"saa-m3-005","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty muốn áp dụng nguyên tắc least privilege khi tạo IAM policy mới cho một service team, nhưng không chắc team thực sự cần những action nào. Công cụ AWS nào giúp xác định quyền tối thiểu dựa trên hoạt động thực tế đã xảy ra?","options":["IAM Access Analyzer (policy generation dựa trên CloudTrail activity)","Amazon Inspector","AWS Trusted Advisor cost check","Amazon GuardDuty"],"correctIndices":[0],"explanation":"IAM Access Analyzer có thể sinh policy least-privilege dựa trên hoạt động thực tế ghi trong CloudTrail.\n✓ IAM Access Analyzer: phân tích CloudTrail để gợi ý policy chỉ chứa action thực sự được dùng, hỗ trợ least privilege.\n✗ Amazon Inspector: quét lỗ hổng workload, không sinh IAM policy.\n✗ Trusted Advisor cost check: kiểm tra chi phí, không liên quan quyền.\n✗ GuardDuty: phát hiện mối đe dọa, không sinh policy least-privilege.","domain":1,"mock":3},
  {"id":"saa-m1-006","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một policy có một explicit Deny cho s3:DeleteObject với điều kiện aws:MultiFactorAuthPresent là false, và một policy khác Allow s3:DeleteObject. Một user không dùng MFA gọi s3:DeleteObject. Theo policy evaluation logic, kết quả là gì?","options":["Bị từ chối, vì explicit Deny luôn ưu tiên hơn mọi Allow khi điều kiện Deny khớp","Được phép, vì có một Allow rõ ràng cho action","Được phép, vì điều kiện MFA chỉ là cảnh báo chứ không chặn","Bị từ chối, vì thiếu resource-based policy"],"correctIndices":[0],"explanation":"Trong IAM, explicit Deny luôn thắng Allow; vì user không có MFA nên điều kiện Deny khớp và request bị chặn.\n✓ Bị từ chối do explicit Deny: điều kiện MFA-false khớp, explicit Deny override Allow.\n✗ Được phép vì có Allow: explicit Deny luôn ưu tiên hơn Allow.\n✗ Điều kiện MFA chỉ cảnh báo: sai, điều kiện trong Deny thực sự chặn request.\n✗ Thiếu resource-based policy: không liên quan; nguyên nhân là explicit Deny.","domain":1,"mock":1},
  {"id":"saa-m2-006","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"multi","question":"Một công ty triển khai AWS IAM Identity Center cho 50 account trong Organizations. Họ muốn quản lý quyền truy cập hiệu quả và an toàn. Những phát biểu nào về permission set là ĐÚNG? (Chọn 2)","options":["Một permission set có thể được gán cho nhiều account và nhóm user, giúp quản lý quyền tập trung","Khi user đăng nhập, Identity Center cấp temporary credentials qua role tương ứng trong account đích","Permission set chỉ áp dụng cho một account duy nhất và phải tạo lại cho mỗi account","Permission set cấp long-term access key cho mỗi user để dùng lâu dài","Permission set thay thế hoàn toàn nhu cầu dùng SCP làm guardrail"],"correctIndices":[0,1],"explanation":"Permission set là tập quyền tái sử dụng được gán cho nhiều account/nhóm và cấp temporary credentials khi đăng nhập.\n✓ Gán cho nhiều account/nhóm: permission set tái sử dụng, quản lý tập trung trên nhiều account.\n✓ Cấp temporary credentials qua role: Identity Center tạo IAM role trong account đích và cấp credentials tạm thời.\n✗ Chỉ cho một account: sai, permission set được thiết kế để dùng lại trên nhiều account.\n✗ Cấp long-term access key: sai, credentials là tạm thời.\n✗ Thay thế SCP: SCP và permission set bổ trợ nhau; SCP vẫn là guardrail cấp account/OU.","domain":1,"mock":2},
  {"id":"saa-m3-006","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng web chạy trên EC2 dùng IAM role qua instance profile. Developer phàn nàn rằng đôi khi ứng dụng nhận lỗi do credentials hết hạn vì code tự đọc và cache credentials. Solutions Architect nên hướng dẫn điều gì là đúng nhất về temporary credentials từ instance profile?","options":["AWS SDK tự động lấy và làm mới credentials từ Instance Metadata Service; ứng dụng nên dùng SDK thay vì cache credentials thủ công","Cần tạo IAM user và gắn access key cố định để tránh hết hạn","Phải tăng thời hạn session lên 1 năm trong instance profile","Phải gọi STS GetSessionToken thủ công mỗi giờ và lưu vào file"],"correctIndices":[0],"explanation":"Credentials từ instance profile là tạm thời và được xoay vòng tự động; AWS SDK lấy/làm mới chúng qua IMDS, ứng dụng không nên tự cache.\n✓ Dùng SDK để lấy/làm mới từ IMDS: SDK xử lý refresh tự động, tránh lỗi credentials hết hạn.\n✗ IAM user access key cố định: long-term credentials, sai best practice và không cần thiết.\n✗ Tăng thời hạn lên 1 năm: instance profile credentials không cấu hình kiểu đó; vấn đề là cache thủ công.\n✗ Gọi GetSessionToken thủ công lưu file: phức tạp không cần thiết, SDK đã tự xử lý.","domain":1,"mock":3},
  {"id":"saa-m1-007","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một tổ chức muốn các developer chỉ có thể thao tác trong region us-east-1 và ap-southeast-1, áp dụng cho TẤT CẢ principal trong một OU gồm 30 account, đồng thời vẫn cho phép một số global service hoạt động. Cách thiết kế guardrail nào hợp lý và ít vận hành nhất?","options":["Dùng SCP gắn ở OU với điều kiện aws:RequestedRegion để deny action ngoài hai region, kèm NotAction loại trừ các global service cần thiết","Viết IAM policy region-restriction và gắn thủ công vào từng role trong 30 account","Tạo permission boundary region-restriction cho từng user trong mỗi account","Dùng AWS Config rule để xóa tài nguyên tạo sai region sau khi phát hiện"],"correctIndices":[0],"explanation":"SCP với điều kiện aws:RequestedRegion và NotAction loại trừ global service thực thi guardrail region tập trung cho cả OU với ít vận hành.\n✓ SCP với RequestedRegion + NotAction global service: áp dụng tự động cho mọi principal trong OU, loại trừ global service, tập trung và ít bảo trì.\n✗ IAM policy gắn thủ công từng role: vận hành cực lớn trên 30 account, dễ bỏ sót.\n✗ Permission boundary từng user: chỉ giới hạn user gắn boundary, không bao trùm mọi principal, vận hành nặng.\n✗ Config auto-delete: detective và phá hủy tài nguyên sau khi đã tạo, không phải guardrail phòng ngừa.","domain":1,"mock":1},
  {"id":"saa-m2-007","courseId":"SAA-C03","lesson":"ch3-01-iam-deep-dive","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một nhà thầu bên ngoài cần truy cập tạm thời vào một số tài nguyên trong account của bạn trong 2 tuần. Bạn muốn cấp quyền an toàn, có thể thu hồi dễ dàng, không tạo credentials lâu dài. Cách tiếp cận nào tối ưu?","options":["Tạo một IAM role với trust policy cho phép account của nhà thầu AssumeRole, gắn policy least-privilege, và xóa/điều chỉnh trust sau 2 tuần","Tạo IAM user cho nhà thầu với access key và xóa sau 2 tuần","Gắn AdministratorAccess cho một IAM user mới của nhà thầu","Chia sẻ mật khẩu root account cho nhà thầu trong thời gian làm việc"],"correctIndices":[0],"explanation":"IAM role với cross-account trust cấp temporary credentials, dễ thu hồi bằng cách sửa/xóa trust policy, không tạo long-term credentials.\n✓ IAM role + cross-account AssumeRole: temporary credentials least-privilege, thu hồi dễ bằng cách chỉnh trust, an toàn cho truy cập tạm.\n✗ IAM user + access key: long-term credentials, rủi ro lộ và khó kiểm soát hơn.\n✗ AdministratorAccess cho user mới: vi phạm least privilege nghiêm trọng.\n✗ Chia sẻ root: cực kỳ nguy hiểm, không bao giờ làm.","domain":1,"mock":2},
  {"id":"saa-m3-007","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một ứng dụng web chạy trên EC2 trong public subnet. Security group hiện cho phép inbound HTTPS (443) từ 0.0.0.0/0. Team muốn cho phép trả response về client mà không cần khai báo thêm rule outbound nào. Đặc tính nào của security group giải thích vì sao điều này hoạt động?","options":["Security group là stateful: traffic phản hồi của một kết nối inbound được tự động cho phép ra","Security group là stateless nên cần thêm outbound rule cho ephemeral ports","NACL stateful tự động cho phép response","Security group đánh giá rule theo thứ tự số như NACL"],"correctIndices":[0],"explanation":"Security group là stateful nên response của kết nối inbound được tự động cho qua, không cần outbound rule tương ứng.\n✓ Stateful, response tự động cho ra — đúng bản chất security group.\n✗ Stateless cần ephemeral ports — đó là đặc tính của NACL, không phải security group.\n✗ NACL stateful — sai, NACL là stateless.\n✗ Đánh giá theo thứ tự số — đó là NACL; security group đánh giá tất cả rule (allow-only).","domain":1,"mock":3},
  {"id":"saa-m1-008","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty cần chặn một dải IP độc hại cụ thể (CIDR) ở cấp subnet, áp dụng cho mọi instance trong subnet đó, kể cả khi rule chặn cần đứng trước rule allow. Cơ chế nào phù hợp nhất?","options":["Network ACL với explicit deny rule có số thứ tự thấp","Security group với deny rule","AWS WAF web ACL gắn vào subnet","Route table với blackhole route cho CIDR đó"],"correctIndices":[0],"explanation":"NACL hỗ trợ explicit deny và đánh giá theo thứ tự số, áp ở cấp subnet — phù hợp để chặn một CIDR.\n✓ NACL explicit deny rule số thấp — đúng, NACL có deny và xét theo thứ tự.\n✗ Security group deny rule — security group chỉ có allow, không có deny.\n✗ WAF gắn vào subnet — WAF gắn vào ALB/CloudFront/API Gateway, không gắn subnet.\n✗ Blackhole route — chỉ ảnh hưởng routing đích, không lọc theo source IP độc hại.","domain":1,"mock":1},
  {"id":"saa-m2-008","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Các EC2 trong private subnet cần kéo OS patches từ Internet nhưng KHÔNG được nhận kết nối khởi tạo từ Internet. Giải pháp nào highly available và least operational overhead?","options":["Triển khai NAT gateway tại mỗi AZ và trỏ route private subnet ra NAT gateway cùng AZ","Gán Elastic IP cho từng instance để chúng tự ra Internet","Triển khai một NAT instance EC2 tự quản tại một AZ duy nhất","Dùng Internet gateway và đặt instance vào public subnet"],"correctIndices":[0],"explanation":"NAT gateway là managed, HA trong AZ; đặt một NAT gateway mỗi AZ tránh single point of failure và phí cross-AZ.\n✓ NAT gateway mỗi AZ — đúng, HA và ít vận hành nhất (managed).\n✗ Elastic IP cho từng instance — biến instance thành public, nhận được kết nối từ Internet, vi phạm yêu cầu.\n✗ NAT instance một AZ — phải tự vá/scale, single point of failure, nhiều vận hành.\n✗ Public subnet + IGW — phơi bày instance ra Internet.","domain":1,"mock":2},
  {"id":"saa-m3-008","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Admin cần truy cập shell quản trị các EC2 trong private subnet. Công ty muốn loại bỏ bastion host và không mở cổng 22 inbound nào, đồng thời ghi log mọi phiên. Giải pháp least operational overhead nào nên dùng?","options":["AWS Systems Manager Session Manager qua SSM Agent và VPC endpoints","Bastion host trong public subnet với security group giới hạn IP văn phòng","Client VPN endpoint cho phép SSH trực tiếp vào instance","Mở port 22 và dùng key pair luân phiên định kỳ"],"correctIndices":[0],"explanation":"Session Manager cho phép truy cập không cần mở inbound port, không cần bastion, và log phiên ra CloudWatch/S3.\n✓ SSM Session Manager — đúng, không mở cổng 22, không bastion, có log phiên.\n✗ Bastion host — vẫn cần host, vẫn mở port 22, nhiều vận hành hơn.\n✗ Client VPN + SSH — vẫn phải mở 22 và quản lý VPN, không loại bỏ inbound port.\n✗ Mở port 22 — trái yêu cầu không mở inbound port.","domain":1,"mock":3},
  {"id":"saa-m1-009","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng EC2 trong private subnet cần đọc/ghi object trong Amazon S3 mà KHÔNG đi qua Internet và muốn tránh chi phí xử lý dữ liệu của NAT gateway. Giải pháp cost-effective nào phù hợp?","options":["Gateway VPC endpoint cho S3 và thêm route vào route table của private subnet","Interface VPC endpoint (PrivateLink) cho S3 với phí giờ và phí GB","NAT gateway cho instance ra Internet truy cập S3 public endpoint","VPC peering giữa VPC và một VPC do AWS quản lý chứa S3"],"correctIndices":[0],"explanation":"Gateway endpoint cho S3 không tính phí và định tuyến qua mạng AWS, tránh hoàn toàn NAT.\n✓ Gateway VPC endpoint cho S3 — đúng, miễn phí và không qua Internet/NAT.\n✗ Interface endpoint cho S3 — tốn phí giờ + GB, kém cost-effective hơn gateway endpoint cho cùng mục đích.\n✗ NAT gateway ra public endpoint — phát sinh phí xử lý GB qua NAT, kém cost-effective.\n✗ VPC peering tới VPC chứa S3 — S3 không nằm trong VPC khách, không peer được như vậy.","domain":1,"mock":1},
  {"id":"saa-m2-009","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Các EC2 private cần gọi AWS Systems Manager, Secrets Manager và KMS API nhưng VPC không có NAT gateway hay Internet gateway và phải giữ traffic trong mạng AWS. Giải pháp nào đáp ứng?","options":["Tạo Interface VPC endpoints (PrivateLink) cho từng service đó","Tạo một Gateway VPC endpoint chung cho cả ba service","Thêm NAT gateway để instance gọi public API endpoints","Bật VPC peering tới VPC dịch vụ của AWS"],"correctIndices":[0],"explanation":"Các API như SSM/Secrets Manager/KMS chỉ hỗ trợ Interface endpoint (PrivateLink), giữ traffic riêng tư qua ENI trong VPC.\n✓ Interface VPC endpoints — đúng, PrivateLink cho các API service này, không cần Internet.\n✗ Gateway endpoint chung — gateway endpoint chỉ hỗ trợ S3 và DynamoDB.\n✗ NAT gateway — đưa traffic ra Internet, trái yêu cầu giữ trong mạng AWS.\n✗ VPC peering tới VPC AWS — không phải cách truy cập các API service này.","domain":1,"mock":2},
  {"id":"saa-m3-009","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng web sau ALB hứng đợt tấn công SQL injection và HTTP flood ở Layer 7. Công ty muốn lọc request độc hại theo pattern và rate-limit theo IP với least operational overhead. Dịch vụ nào phù hợp nhất?","options":["AWS WAF với managed rule groups và rate-based rules gắn vào ALB","AWS Network Firewall đặt trước ALB","Security group thắt chặt trên các instance backend","AWS Shield Standard tự bảo vệ Layer 7"],"correctIndices":[0],"explanation":"WAF hoạt động ở Layer 7, có managed rules cho SQLi và rate-based rules, gắn trực tiếp vào ALB.\n✓ AWS WAF + managed rules + rate-based — đúng cho SQLi và HTTP flood ở L7.\n✗ Network Firewall — lọc L3/L4 (và một số L7) ở cấp VPC, không tối ưu cho web exploits theo HTTP pattern như WAF.\n✗ Security group — chỉ lọc IP/port, không hiểu nội dung HTTP.\n✗ Shield Standard — chống DDoS L3/L4 cơ bản, không lọc SQLi L7.","domain":1,"mock":3},
  {"id":"saa-m1-010","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một sàn giao dịch tài chính chạy ứng dụng quan trọng sau ALB và CloudFront. Họ cần bảo vệ DDoS nâng cao với cam kết phản hồi từ SRT (Shield Response Team) và bảo vệ chi phí scaling khi bị tấn công. Giải pháp nào?","options":["Đăng ký AWS Shield Advanced và bật bảo vệ cho ALB, CloudFront cùng WAF","Chỉ dựa vào AWS Shield Standard miễn phí cho mọi tài nguyên","Triển khai AWS Network Firewall để hấp thụ tấn công DDoS thể tích lớn","Dùng nhiều NACL rule để chặn IP tấn công thủ công"],"correctIndices":[0],"explanation":"Shield Advanced cung cấp bảo vệ DDoS nâng cao, hỗ trợ SRT và cost protection cho scaling do tấn công.\n✓ Shield Advanced — đúng, có SRT support và DDoS cost protection.\n✗ Shield Standard — chỉ bảo vệ cơ bản L3/L4, không có SRT hay cost protection.\n✗ Network Firewall — không phải dịch vụ chống DDoS thể tích và không có SRT.\n✗ NACL thủ công — không khả thi để chống DDoS quy mô lớn theo thời gian thực.","domain":1,"mock":1},
  {"id":"saa-m2-010","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một tổ chức cần kiểm soát egress: cho phép EC2 chỉ gọi ra các domain được allowlist (vd *.example.com) và chặn mọi domain khác, áp dụng tập trung ở biên VPC với khả năng inspect TLS SNI. Giải pháp nào phù hợp nhất?","options":["AWS Network Firewall với stateful rule group lọc theo domain/SNI","Security group outbound rule liệt kê domain được phép","NACL outbound chặn theo domain name","NAT gateway với danh sách domain cho phép cấu hình sẵn"],"correctIndices":[0],"explanation":"Network Firewall hỗ trợ stateful domain filtering (HTTP host/TLS SNI) ở cấp VPC để kiểm soát egress tập trung.\n✓ Network Firewall domain/SNI filtering — đúng, lọc egress theo domain tập trung.\n✗ Security group theo domain — security group chỉ chấp nhận IP/CIDR/port, không phải domain.\n✗ NACL theo domain — NACL cũng chỉ làm việc với IP/port.\n✗ NAT gateway — không có chức năng lọc domain allowlist.","domain":1,"mock":2},
  {"id":"saa-m3-010","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng RDS PostgreSQL dùng username/password được hardcode trong code. Team muốn lưu credential an toàn và tự động luân phiên password định kỳ với least operational overhead. Dịch vụ nào phù hợp nhất?","options":["AWS Secrets Manager với automatic rotation cho RDS","AWS Systems Manager Parameter Store SecureString tự viết Lambda rotation","Lưu credential trong S3 bucket được mã hóa bằng KMS","Lưu credential trong biến môi trường EC2 mã hóa bằng KMS"],"correctIndices":[0],"explanation":"Secrets Manager hỗ trợ tích hợp sẵn automatic rotation cho RDS, ít vận hành nhất.\n✓ Secrets Manager + auto rotation — đúng, rotation tích hợp sẵn cho RDS.\n✗ Parameter Store + tự viết Lambda rotation — Parameter Store không có rotation tích hợp, phải tự xây dựng, nhiều vận hành.\n✗ S3 mã hóa KMS — lưu được nhưng không có rotation tự động.\n✗ Biến môi trường — không có rotation và khó quản lý vòng đời bí mật.","domain":1,"mock":3},
  {"id":"saa-m1-011","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một mobile/web app cần đăng ký, đăng nhập người dùng và phát hành token để gọi API Gateway, hỗ trợ cả social login (Google, Facebook) và MFA, với least operational overhead. Dịch vụ nào phù hợp nhất?","options":["Amazon Cognito user pools làm identity provider phát hành JWT cho API Gateway","IAM users tạo cho từng người dùng cuối của ứng dụng","AWS Directory Service với một AD tự quản","Lưu user/password trong DynamoDB và tự viết logic xác thực"],"correctIndices":[0],"explanation":"Cognito user pools cung cấp đăng ký/đăng nhập, federation social, MFA và phát hành JWT tích hợp với API Gateway.\n✓ Cognito user pools — đúng, quản lý user, social IdP, MFA, token cho API Gateway.\n✗ IAM users cho end users — IAM dành cho người/dịch vụ vận hành AWS, không scale cho end users app.\n✗ Directory Service tự quản — phức tạp, nặng vận hành cho app web/mobile công khai.\n✗ Tự viết auth trên DynamoDB — nhiều vận hành và rủi ro bảo mật cao.","domain":1,"mock":1},
  {"id":"saa-m2-011","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một web app cần truy cập tạm thời vào S3 và DynamoDB từ người dùng đã đăng nhập qua Cognito. Kiến trúc nào cấp quyền AWS theo nguyên tắc least privilege mà không nhúng AWS credential dài hạn vào client?","options":["Cognito identity pools đổi token lấy temporary IAM credentials qua STS gắn IAM role","Tạo một IAM access key chung nhúng vào ứng dụng client","Dùng API key của API Gateway thay cho IAM credentials","Cấp cho mỗi user một IAM user với access key riêng"],"correctIndices":[0],"explanation":"Cognito identity pools đổi token (từ user pool/IdP) lấy temporary credentials qua STS với IAM role có quyền hạn chế.\n✓ Cognito identity pools + STS temporary credentials — đúng, không nhúng credential dài hạn, least privilege qua role.\n✗ Access key chung nhúng client — credential dài hạn lộ ra client, rất rủi ro.\n✗ API key của API Gateway — chỉ để định danh/throttle, không cấp quyền truy cập S3/DynamoDB.\n✗ IAM user mỗi end user — không scale và vẫn dùng credential dài hạn.","domain":1,"mock":2},
  {"id":"saa-m3-011","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ALB phục vụ ứng dụng cần chỉ chấp nhận traffic từ CloudFront, không cho client truy cập trực tiếp ALB DNS. Giải pháp nào phù hợp và ít vận hành?","options":["Dùng AWS managed prefix list của CloudFront trong security group của ALB và header secret kiểm tra ở WAF","Đặt ALB vào private subnet và bỏ Internet gateway","Chặn mọi IP công khai bằng NACL trên subnet của ALB","Bật Shield Advanced để ẩn ALB khỏi Internet"],"correctIndices":[0],"explanation":"Security group dùng managed prefix list của CloudFront giới hạn nguồn, kết hợp custom header verify ở WAF chặn truy cập trực tiếp.\n✓ CloudFront prefix list + header check ở WAF — đúng, chỉ cho CloudFront tới ALB.\n✗ ALB private subnet bỏ IGW — CloudFront vẫn cần tới được origin; private-only phá vỡ kiến trúc CloudFront->ALB công khai.\n✗ NACL chặn mọi IP công khai — sẽ chặn luôn CloudFront, không phân biệt được nguồn hợp lệ.\n✗ Shield Advanced — bảo vệ DDoS, không ẩn ALB hay giới hạn nguồn truy cập.","domain":1,"mock":3},
  {"id":"saa-m1-012","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một kiến trúc 3-tier: web tier (public subnet) gọi app tier (private subnet) trên TCP 8080, app tier gọi RDS (private subnet) trên TCP 5432. Cần cấu hình security group theo least privilege. Hai cấu hình nào ĐÚNG? (Chọn 2)","options":["App tier SG cho phép inbound 8080 với source là security group của web tier","RDS SG cho phép inbound 5432 với source là security group của app tier","App tier SG cho phép inbound 8080 từ 0.0.0.0/0 để đơn giản hóa","RDS SG cho phép inbound 5432 từ CIDR của toàn VPC","Web tier SG cho phép inbound 5432 từ RDS security group"],"correctIndices":[0,1],"explanation":"Tham chiếu security group làm source giúp least privilege và tự co giãn theo instance.\n✓ App tier nhận 8080 từ SG web tier — đúng, chỉ web tier gọi được app tier.\n✓ RDS nhận 5432 từ SG app tier — đúng, chỉ app tier gọi được DB.\n✗ App tier mở 8080 từ 0.0.0.0/0 — quá rộng, vi phạm least privilege.\n✗ RDS mở 5432 cho toàn VPC CIDR — rộng hơn cần thiết, không least privilege.\n✗ Web tier nhận 5432 từ RDS SG — sai chiều luồng; web tier không nhận kết nối DB.","domain":1,"mock":1},
  {"id":"saa-m2-012","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"multi","question":"Khi so sánh security group và network ACL trong VPC, các phát biểu nào ĐÚNG? (Chọn 2)","options":["Security group là stateful; NACL là stateless và cần rule cho cả chiều đi và về","NACL hỗ trợ explicit deny; security group chỉ hỗ trợ allow","Security group đánh giá rule theo thứ tự số nhỏ tới lớn rồi dừng","NACL gắn vào ENI của từng instance","Security group áp dụng ở cấp subnet cho mọi instance trong subnet"],"correctIndices":[0,1],"explanation":"Khác biệt cốt lõi: state (stateful vs stateless) và khả năng deny.\n✓ SG stateful, NACL stateless cần cả hai chiều — đúng.\n✓ NACL có explicit deny, SG chỉ allow — đúng.\n✗ SG đánh giá theo thứ tự số rồi dừng — sai, đó là NACL; SG đánh giá tập hợp allow.\n✗ NACL gắn ENI instance — sai, NACL gắn subnet; SG gắn ENI/instance.\n✗ SG áp cấp subnet — sai, NACL mới áp cấp subnet.","domain":1,"mock":2},
  {"id":"saa-m3-012","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty SaaS muốn cung cấp dịch vụ của mình cho khách hàng truy cập riêng tư từ VPC của khách hàng mà không đi qua Internet, không cần VPC peering và che giấu hạ tầng. Giải pháp nào phù hợp?","options":["Tạo VPC endpoint service (PrivateLink) sau một Network Load Balancer cho khách hàng tạo interface endpoint","Thiết lập VPC peering với từng VPC khách hàng","Phơi bày dịch vụ qua một public ALB và hạn chế bằng security group","Dùng Transit Gateway kết nối tất cả VPC khách hàng vào VPC của mình"],"correctIndices":[0],"explanation":"PrivateLink endpoint service sau NLB cho phép khách hàng truy cập riêng tư qua interface endpoint, không cần peering, ẩn hạ tầng.\n✓ Endpoint service (PrivateLink) sau NLB — đúng, truy cập riêng tư một chiều, không lộ mạng.\n✗ VPC peering từng khách hàng — lộ CIDR hai bên, không scale, dễ trùng IP.\n✗ Public ALB — đi qua Internet, không riêng tư theo yêu cầu.\n✗ Transit Gateway nối mọi VPC khách — kết nối mạng rộng, lộ hạ tầng và phức tạp quản trị.","domain":1,"mock":3},
  {"id":"saa-m1-013","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một subnet được coi là 'public' khi nào trong VPC design?","options":["Khi route table của subnet có route 0.0.0.0/0 trỏ tới Internet gateway","Khi subnet có NAT gateway trong cùng AZ","Khi instance trong subnet có private IP thuộc dải public","Khi NACL của subnet cho phép inbound từ 0.0.0.0/0"],"correctIndices":[0],"explanation":"Subnet là public khi route table của nó có default route ra Internet gateway.\n✓ Route 0.0.0.0/0 tới IGW — đúng định nghĩa public subnet.\n✗ Có NAT gateway cùng AZ — NAT phục vụ egress cho private subnet, không làm subnet thành public.\n✗ Private IP thuộc dải public — IP nội bộ không quyết định tính public của subnet.\n✗ NACL cho inbound 0.0.0.0/0 — chỉ là rule lọc, không tạo đường ra Internet.","domain":1,"mock":1},
  {"id":"saa-m2-013","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ứng dụng dùng Interface VPC endpoint cho Secrets Manager. Team muốn đảm bảo chỉ một số IAM principal nhất định được dùng endpoint này và chỉ truy cập một secret cụ thể. Cơ chế nào kiểm soát điều đó tại endpoint?","options":["VPC endpoint policy giới hạn principal và resource (ARN của secret) được phép qua endpoint","Security group của endpoint chặn theo IAM principal","NACL trên subnet của endpoint lọc theo secret ARN","Resource policy của S3 bucket chứa secret"],"correctIndices":[0],"explanation":"VPC endpoint policy là IAM resource policy gắn trên endpoint, kiểm soát principal/action/resource đi qua endpoint.\n✓ VPC endpoint policy — đúng, giới hạn principal và ARN secret qua endpoint.\n✗ Security group endpoint — chỉ lọc IP/port, không hiểu IAM principal hay secret ARN.\n✗ NACL theo secret ARN — NACL chỉ làm việc IP/port.\n✗ S3 bucket policy — Secrets Manager không lưu secret trong bucket của khách; không liên quan.","domain":1,"mock":2},
  {"id":"saa-m3-013","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ứng dụng đa Region cần chống lại tấn công DDoS L3/L4 thể tích lớn và cũng cần lọc bot xấu ở L7 trên CloudFront. Giải pháp tích hợp nào tối ưu nhất?","options":["Shield Advanced cho bảo vệ DDoS L3/L4 kết hợp AWS WAF Bot Control trên CloudFront","Chỉ dùng AWS WAF với rate-based rules cho cả DDoS thể tích và bot","Chỉ dùng Network Firewall đặt trước CloudFront","Shield Standard kết hợp NACL chặn IP bot"],"correctIndices":[0],"explanation":"DDoS L3/L4 thể tích lớn cần Shield Advanced, còn lọc bot L7 cần WAF Bot Control; kết hợp là tối ưu.\n✓ Shield Advanced + WAF Bot Control — đúng, mỗi lớp xử lý đúng loại tấn công.\n✗ Chỉ WAF rate-based — không thay thế bảo vệ DDoS thể tích L3/L4 của Shield Advanced.\n✗ Chỉ Network Firewall trước CloudFront — không đặt được trước CloudFront và không phải dịch vụ DDoS thể tích.\n✗ Shield Standard + NACL — không đủ chống DDoS quy mô lớn và bot L7.","domain":1,"mock":3},
  {"id":"saa-m1-014","courseId":"SAA-C03","lesson":"ch3-02-network-security","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một Lambda function cần đọc secret từ Secrets Manager và cần truy cập riêng tư không qua Internet. Lambda đã được cấu hình chạy trong VPC (private subnet). Cần thêm gì để Lambda lấy secret thành công mà không cần NAT?","options":["Tạo Interface VPC endpoint cho Secrets Manager trong VPC và cho phép SG của Lambda tới endpoint","Tạo Gateway VPC endpoint cho Secrets Manager","Gắn Internet gateway vào private subnet của Lambda","Bật public IP cho Lambda ENI"],"correctIndices":[0],"explanation":"Lambda trong VPC không có route Internet sẽ cần Interface endpoint (PrivateLink) cho Secrets Manager để gọi API riêng tư.\n✓ Interface endpoint cho Secrets Manager + SG cho phép — đúng, truy cập riêng tư không cần NAT.\n✗ Gateway endpoint cho Secrets Manager — gateway endpoint chỉ hỗ trợ S3/DynamoDB.\n✗ IGW vào private subnet — biến subnet thành public và không phải cách Lambda truy cập riêng tư API.\n✗ Public IP cho Lambda ENI — Lambda ENI trong VPC không hỗ trợ public IP theo cách này.","domain":1,"mock":1},
  {"id":"saa-m2-014","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty lưu trữ các báo cáo tài chính trong một S3 bucket. Yêu cầu compliance bắt buộc dữ liệu phải được encrypted at rest, nhưng team không muốn quản lý bất kỳ key material nào và muốn least operational overhead. Giải pháp nào phù hợp nhất?","options":["Bật server-side encryption với Amazon S3 managed keys (SSE-S3)","Dùng SSE-C, nơi client cung cấp encryption key trong mỗi request","Mã hoá file phía client bằng OpenSSL trước khi upload lên S3","Triển khai CloudHSM cluster và tự mã hoá dữ liệu trước khi lưu"],"correctIndices":[0],"explanation":"SSE-S3 mã hoá at rest hoàn toàn do AWS quản lý, không cần thao tác key, đáp ứng least operational overhead.\n✓ SSE-S3: AWS quản lý toàn bộ key, bật một lần, đáp ứng compliance encryption at rest mà không cần vận hành key.\n✗ SSE-C: client phải tự lưu giữ và truyền key trong mỗi request, tăng overhead đáng kể.\n✗ Mã hoá phía client bằng OpenSSL: phải tự quản lý key và quy trình, nhiều thao tác thủ công.\n✗ CloudHSM: dùng cho yêu cầu HSM chuyên dụng/FIPS 140-2 Level 3, quá phức tạp và tốn kém cho nhu cầu này.","domain":1,"mock":2},
  {"id":"saa-m3-014","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ngân hàng cần mã hoá dữ liệu trong S3 và yêu cầu kiểm soát chi tiết: có audit log mỗi lần key được dùng, tự định nghĩa key rotation policy, và giới hạn IAM principal nào được phép dùng key. Giải pháp nào đáp ứng tốt nhất?","options":["SSE-KMS với customer managed key (CMK) trong AWS KMS","SSE-S3 với Amazon S3 managed keys","SSE-KMS với AWS managed key (aws/s3)","SSE-C với key do ứng dụng cung cấp"],"correctIndices":[0],"explanation":"Customer managed key trong KMS cho phép tự định nghĩa key policy, rotation và audit chi tiết qua CloudTrail.\n✓ SSE-KMS với customer managed key: kiểm soát key policy, rotation tuỳ chỉnh, và mỗi lần dùng key được ghi vào CloudTrail.\n✗ SSE-S3: không cho phép kiểm soát key policy hay audit từng lần dùng key.\n✗ AWS managed key (aws/s3): không thể chỉnh sửa key policy hay rotation, audit hạn chế hơn.\n✗ SSE-C: client phải tự quản lý key, không có audit tập trung trong KMS/CloudTrail.","domain":1,"mock":3},
  {"id":"saa-m1-015","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng cần mã hoá khối lượng lớn dữ liệu (hàng terabyte) tại client trước khi gửi lên S3. Để tránh gọi KMS cho từng object nhỏ và giảm chi phí API, kiến trúc nào áp dụng đúng nguyên lý envelope encryption?","options":["Gọi KMS GenerateDataKey để lấy data key, dùng plaintext data key mã hoá dữ liệu local, lưu encrypted data key cùng object","Gửi toàn bộ terabyte dữ liệu trực tiếp tới KMS Encrypt API để mã hoá","Dùng một KMS key riêng cho mỗi object và gọi Encrypt cho từng file","Lưu plaintext data key trong S3 metadata để giải mã sau này"],"correctIndices":[0],"explanation":"Envelope encryption: KMS chỉ sinh/mã hoá data key, còn dữ liệu lớn được mã hoá local bằng data key, giảm tải KMS.\n✓ GenerateDataKey + mã hoá local: dùng data key để mã hoá khối lượng lớn, chỉ lưu encrypted data key, đúng envelope encryption và tiết kiệm gọi KMS.\n✗ Gửi terabyte tới KMS Encrypt: KMS giới hạn 4KB mỗi request, không thể mã hoá file lớn trực tiếp.\n✗ KMS key riêng mỗi object: tốn kém và không cần thiết, không phải nguyên lý envelope.\n✗ Lưu plaintext data key: phá vỡ bảo mật vì ai đọc được object cũng giải mã được.","domain":1,"mock":1},
  {"id":"saa-m2-015","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một tổ chức trong ngành y tế phải tuân thủ FIPS 140-2 Level 3 cho việc quản lý cryptographic key, yêu cầu single-tenant hardware và toàn quyền kiểm soát HSM. Họ cũng cần dùng key này để mã hoá dữ liệu trong các ứng dụng tự xây. Giải pháp nào phù hợp nhất?","options":["AWS CloudHSM cluster, ứng dụng tích hợp qua PKCS#11/JCE","AWS KMS customer managed key với automatic rotation","AWS KMS AWS managed key","AWS Secrets Manager để lưu trữ và rotate key material"],"correctIndices":[0],"explanation":"CloudHSM cung cấp HSM single-tenant đạt FIPS 140-2 Level 3 với toàn quyền kiểm soát, tích hợp qua chuẩn PKCS#11/JCE.\n✓ CloudHSM: single-tenant dedicated HSM, FIPS 140-2 Level 3, khách hàng toàn quyền quản lý, tích hợp ứng dụng qua PKCS#11/JCE/OpenSSL.\n✗ KMS customer managed key: KMS là dịch vụ multi-tenant, không đáp ứng yêu cầu single-tenant hardware toàn quyền kiểm soát.\n✗ KMS AWS managed key: thậm chí còn ít kiểm soát hơn, không đáp ứng yêu cầu.\n✗ Secrets Manager: dùng để lưu secrets/credentials, không phải HSM cho cryptographic operations.","domain":1,"mock":2},
  {"id":"saa-m3-015","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một public-facing web app cần đảm bảo encryption in transit cho người dùng cuối. Team muốn cung cấp và tự động gia hạn TLS certificate cho Application Load Balancer với least operational overhead. Giải pháp nào tốt nhất?","options":["Cấp certificate công khai từ AWS Certificate Manager (ACM) và gắn vào HTTPS listener của ALB","Mua certificate từ third-party CA và tự cài, đặt lịch nhắc gia hạn thủ công","Tự tạo self-signed certificate và phân phối cho client","Dùng CloudHSM để sinh certificate rồi import vào ALB"],"correctIndices":[0],"explanation":"ACM cấp public certificate miễn phí và tự động gia hạn, tích hợp trực tiếp với ALB, least operational overhead.\n✓ ACM public certificate trên ALB: cấp miễn phí, auto-renew, tích hợp native với HTTPS listener.\n✗ Third-party CA tự cài: phải gia hạn thủ công, nhiều overhead và rủi ro hết hạn.\n✗ Self-signed certificate: trình duyệt người dùng cuối sẽ cảnh báo không tin cậy.\n✗ CloudHSM sinh certificate: phức tạp và không cần thiết, ACM đã giải quyết trọn vẹn.","domain":1,"mock":3},
  {"id":"saa-m1-016","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một S3 bucket vô tình bị cấu hình cho phép public read và đã làm lộ dữ liệu. Solutions Architect muốn ngăn chặn ở mức tổ chức để không bucket nào trong account có thể public, bất kể ACL hay bucket policy được đặt sai. Cách nào đảm bảo tốt nhất?","options":["Bật S3 Block Public Access ở cấp account","Thêm explicit deny vào từng bucket policy","Bật S3 Versioning trên toàn bộ bucket","Dùng Amazon Macie để phát hiện bucket public và cảnh báo"],"correctIndices":[0],"explanation":"Block Public Access ở cấp account override mọi ACL/policy cho phép public, ngăn chặn tập trung và chắc chắn.\n✓ Account-level Block Public Access: chặn mọi cấu hình public ở mọi bucket hiện tại và tương lai, bất kể ACL/policy.\n✗ Explicit deny từng bucket: phải làm thủ công cho mỗi bucket và dễ bỏ sót bucket mới.\n✗ S3 Versioning: chống ghi đè/xoá, không liên quan tới public access.\n✗ Macie: phát hiện và cảnh báo nhưng không chủ động chặn public access.","domain":1,"mock":1},
  {"id":"saa-m2-016","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty dùng SSE-KMS với customer managed key cho S3 bucket lưu hàng triệu object được đọc với tần suất rất cao. Họ nhận thấy chi phí KMS API request tăng mạnh và đôi khi bị throttling. Giải pháp tối ưu chi phí mà vẫn dùng KMS là gì?","options":["Bật S3 Bucket Keys để giảm số lần gọi KMS","Chuyển sang SSE-C để loại bỏ hoàn toàn gọi KMS","Yêu cầu tăng KMS request quota và chấp nhận chi phí cao hơn","Chuyển toàn bộ object sang S3 One Zone-IA để giảm chi phí"],"correctIndices":[0],"explanation":"S3 Bucket Keys tạo một bucket-level key giảm mạnh số lần gọi KMS GenerateDataKey/Decrypt, giảm chi phí và throttling.\n✓ S3 Bucket Keys: giảm tới ~99% số request tới KMS, hạ chi phí và tránh throttling mà vẫn dùng SSE-KMS.\n✗ SSE-C: chuyển gánh nặng quản lý key sang client và thay đổi toàn bộ ứng dụng, không phải giải pháp tối ưu.\n✗ Tăng quota: chỉ giải quyết throttling chứ không giảm chi phí, ngược lại còn tốn hơn.\n✗ One Zone-IA: thay đổi storage class/độ bền, không liên quan tới chi phí KMS.","domain":1,"mock":2},
  {"id":"saa-m3-016","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty có hàng petabyte dữ liệu trong S3 và lo ngại có thể chứa PII (số thẻ, số an sinh xã hội) chưa được phân loại. Họ cần một dịch vụ managed tự động khám phá và phân loại dữ liệu nhạy cảm. Dịch vụ nào phù hợp?","options":["Amazon Macie","AWS Config","Amazon Inspector","AWS Glue DataBrew"],"correctIndices":[0],"explanation":"Amazon Macie dùng ML/pattern matching để tự động khám phá và phân loại dữ liệu nhạy cảm (PII) trong S3.\n✓ Macie: managed service chuyên discover và classify PII/dữ liệu nhạy cảm trong S3, đúng nhu cầu.\n✗ AWS Config: theo dõi cấu hình resource và compliance, không phân loại nội dung dữ liệu.\n✗ Inspector: quét lỗ hổng bảo mật của EC2/ECR/Lambda, không phân loại dữ liệu S3.\n✗ Glue DataBrew: chuẩn bị/làm sạch dữ liệu cho analytics, không phải công cụ phát hiện PII compliance.","domain":1,"mock":3},
  {"id":"saa-m1-017","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một tổ chức cần bản sao dữ liệu S3 ở một region khác để đáp ứng yêu cầu compliance về data residency và disaster recovery. Dữ liệu được mã hoá bằng SSE-KMS với customer managed key. Cấu hình nào đảm bảo cross-region replication hoạt động đúng?","options":["Bật S3 Cross-Region Replication và cấu hình replica dùng một KMS key ở destination region","Bật S3 Cross-Region Replication; KMS key ở source region tự động dùng được ở destination","Dùng S3 Same-Region Replication rồi copy thủ công sang region khác","Tắt mã hoá ở destination bucket để replication không bị lỗi key"],"correctIndices":[0],"explanation":"KMS key mang tính region-specific, nên replica ở destination cần một KMS key của chính region đó để mã hoá lại.\n✓ CRR với KMS key ở destination region: bắt buộc vì single-Region KMS key không dùng được cross-region; cấu hình replica key cho phép re-encrypt object tại destination.\n✗ Dùng lại source key ở destination: không hợp lệ vì single-Region KMS key bị giới hạn theo region.\n✗ Same-Region Replication + copy thủ công: tăng overhead và không phải giải pháp CRR tự động.\n✗ Tắt mã hoá ở destination: vi phạm yêu cầu compliance encryption at rest.","domain":1,"mock":1},
  {"id":"saa-m2-017","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một bucket policy cần cho phép một IAM role trong account khác đọc object, nhưng phải từ chối mọi request không dùng HTTPS để đảm bảo encryption in transit. Điều kiện policy nào nên dùng để enforce TLS?","options":["Thêm statement Deny khi điều kiện aws:SecureTransport là false","Thêm statement Allow khi điều kiện s3:x-amz-server-side-encryption tồn tại","Dùng điều kiện aws:SourceIp để giới hạn IP","Dùng điều kiện s3:x-amz-acl bằng private"],"correctIndices":[0],"explanation":"aws:SecureTransport=false xác định request không qua HTTPS; Deny trên điều kiện này enforce encryption in transit.\n✓ Deny khi aws:SecureTransport là false: chặn mọi request HTTP, bắt buộc dùng TLS/HTTPS.\n✗ s3:x-amz-server-side-encryption: liên quan encryption at rest chứ không phải in transit.\n✗ aws:SourceIp: giới hạn theo IP, không enforce TLS.\n✗ s3:x-amz-acl=private: kiểm soát ACL object, không liên quan transport security.","domain":1,"mock":2},
  {"id":"saa-m3-017","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty đa quốc gia muốn dùng cùng một logical encryption key để mã hoá S3 object ở nhiều region (us-east-1, eu-west-1) nhằm đơn giản hoá quản lý key cho dữ liệu được replicate, tránh phải re-encrypt khi failover giữa region. Giải pháp KMS nào phù hợp nhất?","options":["AWS KMS multi-Region keys","AWS KMS customer managed key riêng cho từng region không liên quan nhau","CloudHSM cluster trải rộng nhiều region","Import cùng một key material thủ công vào các single-region key độc lập"],"correctIndices":[0],"explanation":"KMS multi-Region keys là các replica có cùng key ID và key material, cho phép giải mã cross-region mà không cần re-encrypt.\n✓ Multi-Region keys: primary và replica chia sẻ key material, ciphertext mã hoá ở một region có thể giải mã ở region khác, lý tưởng cho DR/replication.\n✗ Customer managed key riêng từng region: key material khác nhau nên phải re-encrypt khi chuyển region.\n✗ CloudHSM nhiều region: không cung cấp khái niệm shared multi-Region key kiểu KMS và tăng độ phức tạp.\n✗ Import key material thủ công vào key độc lập: dễ lỗi và không được KMS quản lý như multi-Region keys.","domain":1,"mock":3},
  {"id":"saa-m1-018","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một team muốn đảm bảo dữ liệu trong Amazon EBS volume gắn vào EC2 instance được encrypted at rest với least operational overhead. Cách nào đơn giản nhất?","options":["Bật EBS encryption khi tạo volume, dùng KMS key để mã hoá","Tự cài đĩa mã hoá phần mềm bên trong OS cho mỗi instance","Mã hoá từng file trong ứng dụng trước khi ghi xuống đĩa","Dùng instance store và mã hoá thủ công bằng dm-crypt"],"correctIndices":[0],"explanation":"EBS encryption tích hợp với KMS, mã hoá transparent at rest và in transit giữa instance và volume, gần như không cần thao tác.\n✓ EBS encryption với KMS: bật một lần khi tạo volume, mã hoá/giải mã transparent, least operational overhead.\n✗ Đĩa mã hoá phần mềm trong OS: phải tự quản lý cấu hình và key trên từng instance.\n✗ Mã hoá từng file trong app: thay đổi mã ứng dụng và tự quản lý key, nhiều overhead.\n✗ Instance store + dm-crypt: dữ liệu ephemeral và phải tự vận hành mã hoá, phức tạp.","domain":1,"mock":1},
  {"id":"saa-m2-018","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"multi","question":"Một công ty fintech cần thiết kế lớp data protection cho S3 bucket chứa hồ sơ khách hàng. Họ muốn: kiểm soát key, audit truy cập key, ngăn public access, và enforce mã hoá khi upload. Chọn HAI hành động giúp đạt mục tiêu này.","options":["Dùng SSE-KMS với customer managed key và bật key rotation","Bật S3 Block Public Access ở cấp account và bucket","Lưu trữ dữ liệu ở storage class S3 One Zone-IA","Tắt S3 Versioning để giảm số lượng object","Cho phép truy cập bucket qua ACL public-read cho tiện chia sẻ"],"correctIndices":[0,1],"explanation":"Customer managed KMS key cho kiểm soát+audit key, Block Public Access ngăn lộ dữ liệu công khai.\n✓ SSE-KMS customer managed key + rotation: kiểm soát key policy và audit qua CloudTrail.\n✓ Block Public Access ở account và bucket: chặn mọi cấu hình public, bảo vệ hồ sơ nhạy cảm.\n✗ One Zone-IA: liên quan độ bền/chi phí lưu trữ, không phải mục tiêu data protection.\n✗ Tắt Versioning: làm giảm khả năng phục hồi, không liên quan mục tiêu.\n✗ ACL public-read: trực tiếp gây lộ dữ liệu, sai hoàn toàn yêu cầu.","domain":1,"mock":2},
  {"id":"saa-m3-018","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ứng dụng cần lưu database credentials và tự động rotate chúng định kỳ mà không cần thay đổi code khi rotate, đồng thời mã hoá credentials at rest bằng KMS. Dịch vụ nào phù hợp nhất?","options":["AWS Secrets Manager","AWS Systems Manager Parameter Store (Standard, không rotation)","AWS KMS lưu trực tiếp credentials","Lưu credentials trong file mã hoá trên EBS volume"],"correctIndices":[0],"explanation":"Secrets Manager mã hoá secret bằng KMS và hỗ trợ automatic rotation tích hợp (đặc biệt với RDS) mà app chỉ cần fetch khi cần.\n✓ Secrets Manager: lưu credentials mã hoá bằng KMS, có built-in automatic rotation, app lấy giá trị mới mà không sửa code.\n✗ Parameter Store Standard: không có automatic rotation tích hợp sẵn cho database credentials.\n✗ KMS lưu credentials: KMS quản lý key chứ không lưu trữ secret payload.\n✗ File mã hoá trên EBS: phải tự xây cơ chế rotation và phân phối, nhiều overhead.","domain":1,"mock":3},
  {"id":"saa-m1-019","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một bucket dùng SSE-KMS với customer managed key. Một IAM user có quyền s3:GetObject nhưng khi tải object lại bị AccessDenied. Nguyên nhân khả dĩ nhất và cách khắc phục là gì?","options":["User thiếu quyền kms:Decrypt trên KMS key; cần cấp quyền trong key policy/IAM","Bucket chưa bật Versioning nên object không đọc được","S3 Block Public Access đang chặn user nội bộ","Object đang ở Glacier Deep Archive nên cần restore"],"correctIndices":[0],"explanation":"Với SSE-KMS, để giải mã object user cần cả s3:GetObject lẫn kms:Decrypt trên key dùng để mã hoá.\n✓ Thiếu kms:Decrypt: dù có GetObject, không có quyền giải mã KMS key sẽ bị AccessDenied; cấp kms:Decrypt khắc phục.\n✗ Versioning: không ảnh hưởng quyền đọc object hiện tại.\n✗ Block Public Access: chỉ chặn truy cập public, không chặn IAM principal có quyền hợp lệ.\n✗ Glacier Deep Archive: sẽ báo lỗi InvalidObjectState, không phải AccessDenied.","domain":1,"mock":1},
  {"id":"saa-m2-019","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty media phân phối nội dung qua CloudFront và cần đảm bảo encryption in transit từ viewer đến edge và từ edge đến origin (S3). Họ muốn least operational overhead cho certificate. Cấu hình nào đúng?","options":["Dùng ACM certificate (trong us-east-1) cho CloudFront, đặt Viewer Protocol Policy là Redirect HTTP to HTTPS và origin protocol là HTTPS","Dùng self-signed certificate trên CloudFront và cho phép HTTP cho viewer","Để CloudFront chỉ phục vụ HTTP để giảm độ trễ, mã hoá ở tầng ứng dụng","Mua certificate third-party và cài thủ công trên từng edge location"],"correctIndices":[0],"explanation":"ACM certificate (us-east-1 cho CloudFront) cùng Redirect HTTP to HTTPS và HTTPS tới origin đảm bảo end-to-end TLS, auto-renew.\n✓ ACM + Redirect HTTPS + HTTPS origin: bảo đảm encryption in transit cả viewer-edge và edge-origin, certificate tự gia hạn.\n✗ Self-signed + cho phép HTTP: không an toàn và viewer không được bảo vệ TLS.\n✗ Chỉ HTTP: vi phạm yêu cầu encryption in transit.\n✗ Third-party cài thủ công trên edge: không khả thi và nhiều overhead; CloudFront dùng ACM.","domain":1,"mock":2},
  {"id":"saa-m3-019","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty muốn dùng key material của riêng họ (do tổ chức tự sinh trong HSM nội bộ) cho KMS thay vì để KMS sinh, nhằm đáp ứng yêu cầu kiểm soát nguồn gốc key (key provenance) trong khi vẫn dùng SSE-KMS cho S3. Giải pháp nào đúng?","options":["Tạo KMS key với origin EXTERNAL và import key material do tổ chức tự sinh","Dùng AWS managed key và bật automatic rotation","Dùng SSE-S3 vì AWS sẽ tự quản lý key material","Lưu key material trong Secrets Manager và tham chiếu từ S3"],"correctIndices":[0],"explanation":"KMS hỗ trợ Bring Your Own Key (BYOK) qua key có origin EXTERNAL, cho phép import key material tự sinh để đáp ứng key provenance.\n✓ KMS key origin EXTERNAL + import key material: cho phép dùng key tự sinh, đáp ứng yêu cầu kiểm soát nguồn gốc key mà vẫn dùng SSE-KMS.\n✗ AWS managed key: AWS sinh và quản lý hoàn toàn, không kiểm soát được provenance.\n✗ SSE-S3: hoàn toàn do AWS quản lý key, không đáp ứng BYOK.\n✗ Secrets Manager: lưu secrets, không phải cơ chế cung cấp KMS encryption key material cho S3.","domain":1,"mock":3},
  {"id":"saa-m1-020","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một tổ chức cần đảm bảo rằng tất cả object upload vào một S3 bucket bắt buộc phải dùng SSE-KMS, từ chối bất kỳ upload nào không có header mã hoá đúng. Cách enforce hiệu quả nhất là gì?","options":["Bucket policy Deny PutObject khi s3:x-amz-server-side-encryption không phải aws:kms","Bật S3 default encryption và tin rằng client luôn gửi đúng header","Dùng Macie để phát hiện object chưa mã hoá sau khi upload","Bật S3 Versioning để giữ lịch sử object chưa mã hoá"],"correctIndices":[0],"explanation":"Bucket policy với điều kiện Deny dựa trên header server-side-encryption enforce bắt buộc SSE-KMS ngay tại thời điểm upload.\n✓ Deny PutObject khi không phải aws:kms: chủ động từ chối mọi upload không dùng SSE-KMS, enforce ở thời điểm ghi.\n✗ Default encryption đơn thuần: tự áp dụng mã hoá nhưng không từ chối các request cố tình chỉ định kiểu khác; bucket policy mạnh hơn để enforce KMS cụ thể.\n✗ Macie phát hiện sau: chỉ phát hiện chứ không ngăn upload không mã hoá.\n✗ Versioning: không liên quan tới enforce encryption.","domain":1,"mock":1},
  {"id":"saa-m2-020","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một công ty bị yêu cầu compliance: dữ liệu PII trong S3 phải được phân loại, mã hoá at rest bằng key có thể audit, và mọi truy cập object phải qua HTTPS. Chọn HAI biện pháp trực tiếp đáp ứng các yêu cầu này (ngoài việc bật mã hoá SSE-KMS đã có sẵn).","options":["Bật Amazon Macie để khám phá và phân loại dữ liệu PII","Thêm bucket policy Deny khi aws:SecureTransport là false","Chuyển dữ liệu sang S3 Standard-IA để giảm chi phí","Tắt CloudTrail để giảm log noise","Dùng SSE-C để client tự giữ key"],"correctIndices":[0,1],"explanation":"Macie xử lý yêu cầu phân loại PII; bucket policy chặn non-HTTPS enforce encryption in transit.\n✓ Macie: tự động khám phá và phân loại PII, đáp ứng yêu cầu data classification.\n✓ Deny khi aws:SecureTransport là false: bắt buộc mọi truy cập object qua HTTPS.\n✗ Standard-IA: liên quan chi phí lưu trữ, không đáp ứng yêu cầu compliance nêu ra.\n✗ Tắt CloudTrail: làm mất khả năng audit, đi ngược yêu cầu.\n✗ SSE-C: key do client giữ, không audit tập trung được như yêu cầu key auditable.","domain":1,"mock":2},
  {"id":"saa-m3-020","courseId":"SAA-C03","lesson":"ch3-03-data-protection","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng analytics cần chia sẻ dataset đã được phân loại nhạy cảm chỉ với một số AWS account đối tác cụ thể, dữ liệu mã hoá bằng customer managed KMS key. Để đối tác đọc được object, cần cấu hình gì ngoài bucket policy cho phép?","options":["Cập nhật KMS key policy để cấp quyền kms:Decrypt cho các account đối tác","Bật S3 Block Public Access để dữ liệu tự động chia sẻ an toàn","Đổi sang SSE-S3 để bỏ qua yêu cầu quyền KMS","Bật cross-region replication tới bucket của đối tác"],"correctIndices":[0],"explanation":"Với SSE-KMS, đối tác cần quyền kms:Decrypt trên key; do đó key policy phải cấp quyền cho các account đó.\n✓ Cập nhật KMS key policy cấp kms:Decrypt cho account đối tác: bắt buộc để họ giải mã object ngoài quyền S3.\n✗ Block Public Access: ngăn public, không cấp quyền chia sẻ cho account cụ thể.\n✗ Đổi SSE-S3: thay đổi mô hình bảo mật và mất kiểm soát key, không đúng yêu cầu.\n✗ Cross-region replication: sao chép dữ liệu chứ không giải quyết quyền giải mã cho đối tác.","domain":1,"mock":3},
  {"id":"saa-m1-021","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một ứng dụng web ghi các đơn hàng vào một tier xử lý backend. Khi backend tạm thời quá tải, đơn hàng bị mất. Kiến trúc sư muốn decouple hai tier để đơn hàng được giữ lại và xử lý sau. Dịch vụ nào phù hợp nhất?","options":["Đặt một Amazon SQS queue giữa web tier và backend tier","Cho web tier gọi trực tiếp backend qua Application Load Balancer","Lưu đơn hàng vào Amazon S3 rồi cho backend poll danh sách object","Dùng Amazon Kinesis Data Firehose để stream đơn hàng tới backend"],"correctIndices":[0],"explanation":"SQS làm buffer bền giữa producer và consumer, hấp thụ tải đột biến và cho phép xử lý bất đồng bộ (queue-based load leveling).\n✓ SQS queue giữa hai tier — đúng, message được giữ bền cho tới khi backend xử lý xong, decouple hoàn toàn.\n✗ Gọi trực tiếp qua ALB — vẫn là kết nối đồng bộ, backend quá tải vẫn làm mất/timeout request.\n✗ S3 + poll list object — S3 không phải hàng đợi message, polling list kém hiệu quả và không có visibility/retry semantics.\n✗ Kinesis Firehose — dùng để load streaming data vào kho lưu trữ, không phải hàng đợi công việc xử lý từng đơn.","domain":2,"mock":1},
  {"id":"saa-m2-021","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một hệ thống xử lý giao dịch tài chính yêu cầu các message phải được xử lý đúng MỘT lần và đúng THỨ TỰ chúng được gửi. Loại Amazon SQS queue nào đáp ứng yêu cầu này?","options":["FIFO queue","Standard queue","Standard queue với long polling","Standard queue có bật server-side encryption"],"correctIndices":[0],"explanation":"FIFO queue đảm bảo thứ tự và exactly-once processing, đúng yêu cầu giao dịch tài chính.\n✓ FIFO queue — đúng, đảm bảo ordering và exactly-once processing.\n✗ Standard queue — chỉ best-effort ordering và at-least-once delivery (có thể trùng).\n✗ Standard + long polling — long polling chỉ giảm empty response, không đảm bảo thứ tự.\n✗ Standard + encryption — mã hóa không liên quan tới ordering hay duplication.","domain":2,"mock":2},
  {"id":"saa-m3-021","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một consumer đọc message từ Amazon SQS queue và mất tới 8 phút để xử lý mỗi message. Hiện một số message bị xử lý nhiều lần vì worker khác nhận lại chúng trước khi worker đầu tiên hoàn tất. Cách least operational overhead để khắc phục là gì?","options":["Tăng visibility timeout của queue lên lớn hơn thời gian xử lý tối đa","Chuyển sang FIFO queue để loại bỏ trùng lặp","Giảm số lượng consumer xuống còn một worker duy nhất","Bật long polling với WaitTimeSeconds là 20"],"correctIndices":[0],"explanation":"Message bị nhận lại do visibility timeout ngắn hơn thời gian xử lý; tăng timeout là cách trực tiếp và đơn giản nhất.\n✓ Tăng visibility timeout > thời gian xử lý — đúng, message ẩn đủ lâu để worker hoàn tất trước khi worker khác thấy lại.\n✗ Chuyển FIFO — FIFO chống trùng khi gửi, nhưng vấn đề ở đây là timeout xử lý, không phải duplicate khi produce; còn giới hạn throughput.\n✗ Giảm còn một worker — làm mất khả năng scale và không giải quyết gốc rễ.\n✗ Long polling — chỉ giảm empty receive, không liên quan tới việc message bị reprocess.","domain":2,"mock":3},
  {"id":"saa-m1-022","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng dùng Amazon SQS. Một số message bị lỗi (poison message) và liên tục được xử lý lại, làm nghẽn consumer và che lấp message hợp lệ. Giải pháp nào xử lý các message hỏng này mà ít vận hành nhất?","options":["Cấu hình một Dead-Letter Queue (DLQ) với maxReceiveCount phù hợp","Tăng visibility timeout để giảm số lần message được nhận lại","Xóa và tạo lại queue mỗi khi phát hiện message lỗi","Bật FIFO queue để loại bỏ message trùng lặp"],"correctIndices":[0],"explanation":"DLQ tự động chuyển message vượt quá số lần nhận tối đa ra khỏi queue chính để cô lập và phân tích sau.\n✓ DLQ với maxReceiveCount — đúng, message lỗi quá ngưỡng tự động chuyển sang DLQ, queue chính không còn nghẽn.\n✗ Tăng visibility timeout — chỉ làm message hỏng quay lại chậm hơn, không cô lập chúng.\n✗ Xóa/tạo lại queue — thủ công, mất cả message hợp lệ, rủi ro cao.\n✗ FIFO — chống trùng lặp, không xử lý vấn đề poison message liên tục fail.","domain":2,"mock":1},
  {"id":"saa-m2-022","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Khi một ảnh được tải lên, hệ thống cần kích hoạt đồng thời nhiều xử lý độc lập: tạo thumbnail, đánh chỉ mục tìm kiếm, và gửi thông báo. Mỗi xử lý có queue riêng. Kiến trúc decoupled fan-out nào phù hợp nhất?","options":["Publish một message tới Amazon SNS topic, subscribe nhiều SQS queue vào topic đó","Cho ứng dụng ghi tuần tự cùng một message vào ba SQS queue","Dùng một SQS queue duy nhất với ba consumer group khác nhau","Dùng Amazon Kinesis Data Streams với một shard cho mỗi xử lý"],"correctIndices":[0],"explanation":"SNS fan-out cho phép một message được phát tới nhiều SQS queue subscriber cùng lúc, mỗi tier xử lý độc lập.\n✓ SNS topic + nhiều SQS subscriber — đúng, mẫu fan-out chuẩn, một publish tới nhiều queue, decoupled và resilient.\n✗ Ghi tuần tự vào ba queue — producer phải biết mọi consumer, coupling cao, lỗi giữa chừng gây thiếu sót.\n✗ Một queue, ba consumer group — SQS không có khái niệm consumer group; mỗi message chỉ được một consumer xử lý.\n✗ Kinesis một shard mỗi xử lý — Kinesis dùng cho streaming, không phải fan-out pub/sub tới các queue độc lập.","domain":2,"mock":2},
  {"id":"saa-m3-022","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty muốn route các event giữa nhiều SaaS application và AWS service dựa trên nội dung event, với khả năng lọc và biến đổi event, mà không phải tự viết code routing. Dịch vụ nào phù hợp nhất?","options":["Amazon EventBridge với rule và event pattern","Amazon SQS với nhiều queue lọc thủ công","Amazon SNS với message filtering policy đơn giản","AWS Step Functions điều phối luồng giữa các service"],"correctIndices":[0],"explanation":"EventBridge là event bus serverless hỗ trợ tích hợp SaaS partner và content-based routing qua rule/event pattern.\n✓ EventBridge với rule/event pattern — đúng, route theo nội dung, tích hợp sẵn SaaS partner, lọc và biến đổi event, ít code.\n✗ SQS lọc thủ công — SQS là hàng đợi point-to-point, không có routing theo nội dung và không tích hợp SaaS.\n✗ SNS filtering — lọc theo message attribute đơn giản, không có schema registry/SaaS integration mạnh như EventBridge.\n✗ Step Functions — điều phối workflow trạng thái, không phải event router giữa nhiều nguồn SaaS.","domain":2,"mock":3},
  {"id":"saa-m1-023","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một REST API nhận lưu lượng không thể đoán trước, từ 0 tới hàng nghìn request mỗi giây. Công ty muốn kiến trúc serverless, chỉ trả tiền khi có request và tự scale. Lựa chọn nào tối ưu nhất?","options":["Amazon API Gateway tích hợp với AWS Lambda","Application Load Balancer phân phối tới một Auto Scaling group EC2","Amazon EC2 đặt sau Network Load Balancer với scaling theo lịch","Amazon API Gateway tích hợp với một EC2 instance cố định"],"correctIndices":[0],"explanation":"API Gateway + Lambda là kiến trúc serverless, scale tự động từ 0 lên hàng nghìn request và chỉ tính phí theo lượng dùng.\n✓ API Gateway + Lambda — đúng, fully serverless, scale tự động, pay-per-use, ít vận hành nhất.\n✗ ALB + Auto Scaling EC2 — chạy được nhưng luôn có instance tối thiểu, không scale-to-zero, vận hành nhiều hơn.\n✗ EC2 sau NLB scaling theo lịch — scaling theo lịch không hợp với tải khó đoán; tốn phí khi nhàn rỗi.\n✗ API Gateway + EC2 cố định — EC2 cố định không tự scale và không serverless.","domain":2,"mock":1},
  {"id":"saa-m2-023","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng cần một load balancer hoạt động ở tầng TCP/UDP (Layer 4), xử lý hàng triệu request mỗi giây với độ trễ cực thấp và hỗ trợ static IP. Loại load balancer nào phù hợp nhất?","options":["Network Load Balancer (NLB)","Application Load Balancer (ALB)","Gateway Load Balancer (GWLB)","Classic Load Balancer (CLB)"],"correctIndices":[0],"explanation":"NLB hoạt động ở Layer 4, cung cấp throughput cực cao, độ trễ thấp và static IP per AZ.\n✓ NLB — đúng, Layer 4, ultra-low latency, hàng triệu request/giây, hỗ trợ static/Elastic IP.\n✗ ALB — Layer 7, định tuyến theo HTTP/HTTPS, không tối ưu cho TCP/UDP thuần và không có static IP gốc.\n✗ GWLB — dùng cho triển khai virtual appliance bảo mật (firewall), không phải cân bằng tải ứng dụng thông thường.\n✗ CLB — thế hệ cũ, không khuyến nghị cho thiết kế mới, thiếu nhiều tính năng.","domain":2,"mock":2},
  {"id":"saa-m3-023","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng cần route request tới các target group khác nhau dựa trên đường dẫn URL (ví dụ /api tới một nhóm, /images tới nhóm khác) và hostname. Loại load balancer nào phù hợp?","options":["Application Load Balancer (ALB)","Network Load Balancer (NLB)","Gateway Load Balancer (GWLB)","AWS Global Accelerator"],"correctIndices":[0],"explanation":"ALB hỗ trợ content-based routing theo path và host header ở Layer 7.\n✓ ALB — đúng, định tuyến theo URL path và hostname, lý tưởng cho microservices HTTP/HTTPS.\n✗ NLB — Layer 4, không hiểu HTTP path/host nên không route theo nội dung được.\n✗ GWLB — dùng cho appliance bảo mật, không route theo URL path.\n✗ Global Accelerator — cải thiện routing toàn cầu qua AWS backbone, không phải path-based routing trong một region.","domain":2,"mock":3},
  {"id":"saa-m1-024","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một fleet EC2 web server đứng sau Application Load Balancer lưu session người dùng trong bộ nhớ cục bộ của từng instance. Khi Auto Scaling thu hồi instance, người dùng bị đăng xuất. Cách tốt nhất để làm tier này stateless và resilient là gì?","options":["Lưu session ở một store dùng chung như Amazon ElastiCache hoặc DynamoDB","Bật sticky sessions trên ALB để giữ người dùng ở cùng một instance","Tăng cooldown của Auto Scaling để instance ít bị thu hồi hơn","Lưu session vào ổ EBS gắn vào từng instance"],"correctIndices":[0],"explanation":"Đưa session ra store ngoài (ElastiCache/DynamoDB) làm các instance stateless, có thể thay thế tự do mà không mất session.\n✓ Session store dùng chung — đúng, tier stateless, instance thay thế thoải mái, người dùng không bị đăng xuất.\n✗ Sticky sessions — vẫn gắn người dùng vào một instance; instance bị thu hồi là mất session.\n✗ Tăng cooldown — chỉ trì hoãn vấn đề, không làm tier stateless.\n✗ Session trên EBS — EBS gắn theo instance; instance mất thì session mất, không dùng chung được.","domain":2,"mock":1},
  {"id":"saa-m2-024","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một worker tier xử lý job từ SQS queue. Đội ngũ muốn số lượng EC2 worker tự động tăng khi backlog message lớn và giảm khi backlog nhỏ, để cân bằng chi phí và tốc độ xử lý. Cách triển khai tốt nhất là gì?","options":["Auto Scaling group dùng target tracking dựa trên metric ApproximateNumberOfMessagesVisible (qua backlog per instance)","Auto Scaling theo lịch cố định mỗi giờ tăng giảm số instance","Scale thủ công dựa trên báo cáo backlog hằng ngày","Auto Scaling theo CPUUtilization trung bình của fleet"],"correctIndices":[0],"explanation":"Scaling theo độ sâu queue (backlog per instance) phản ánh đúng lượng công việc tồn đọng, là chỉ số chuẩn để scale worker tier.\n✓ Auto Scaling theo số message visible / backlog per instance — đúng, scale theo khối lượng công việc thực tế trong queue.\n✗ Scaling theo lịch — không phản ứng với biến động backlog thực tế.\n✗ Scale thủ công hằng ngày — chậm, nhiều vận hành, không kịp xử lý đột biến.\n✗ Scale theo CPU — CPU không nhất thiết tương quan với backlog; queue có thể đầy mà CPU vẫn thấp.","domain":2,"mock":2},
  {"id":"saa-m3-024","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty đang chuyển một ứng dụng monolith sang microservices chạy trong container. Họ không muốn quản lý EC2 instance hay cluster của control plane container, chỉ muốn deploy container và để AWS lo phần hạ tầng. Lựa chọn nào ít operational overhead nhất?","options":["Amazon ECS với AWS Fargate launch type","Amazon ECS với EC2 launch type","Amazon EKS với self-managed node group EC2","Chạy container trực tiếp trên EC2 với Docker tự cài"],"correctIndices":[0],"explanation":"Fargate là compute serverless cho container, loại bỏ việc quản lý EC2 và scaling node.\n✓ ECS + Fargate — đúng, không quản lý server/node, chỉ định nghĩa task, AWS lo hạ tầng, ít vận hành nhất.\n✗ ECS + EC2 launch type — vẫn phải quản lý và patch các EC2 container instance.\n✗ EKS + self-managed node — phải tự quản lý và vận hành node group EC2.\n✗ Docker tự cài trên EC2 — vận hành nhiều nhất, không có orchestration được quản lý.","domain":2,"mock":3},
  {"id":"saa-m1-025","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng dùng SNS để gửi thông báo. Đội phát triển muốn đảm bảo nếu việc gửi tới một subscriber endpoint thất bại liên tục thì message không bị mất mà được lưu để xử lý lại. Cách tốt nhất là gì?","options":["Cấu hình một dead-letter queue (SQS) cho SNS subscription","Tăng số lần retry mặc định của SNS lên tối đa","Chuyển sang SQS standard queue thay cho SNS","Bật message filtering để bỏ qua message lỗi"],"correctIndices":[0],"explanation":"SNS subscription hỗ trợ DLQ (một SQS queue) để giữ các message giao không thành công sau khi hết retry.\n✓ DLQ cho SNS subscription — đúng, message giao thất bại được chuyển vào SQS DLQ, không bị mất.\n✗ Tăng retry — retry có giới hạn; nếu endpoint hỏng lâu thì message vẫn mất khi không có DLQ.\n✗ Chuyển hẳn sang SQS — mất khả năng fan-out pub/sub của SNS; không phải giải pháp cho nhu cầu này.\n✗ Message filtering — chỉ quyết định message nào được gửi, không cứu message giao thất bại.","domain":2,"mock":1},
  {"id":"saa-m2-025","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một hệ thống đặt vé dùng SQS FIFO queue để đảm bảo thứ tự xử lý theo từng sự kiện. Hiện throughput bị giới hạn vì tất cả message dùng chung một MessageGroupId. Cách tăng throughput mà vẫn giữ thứ tự trong phạm vi mỗi sự kiện là gì?","options":["Dùng MessageGroupId riêng cho mỗi sự kiện để các nhóm được xử lý song song","Chuyển sang standard queue để có throughput không giới hạn","Tăng số consumer đọc cùng một message group","Bật high throughput mode nhưng giữ một MessageGroupId duy nhất"],"correctIndices":[0],"explanation":"FIFO chỉ đảm bảo thứ tự trong cùng một MessageGroupId; tách nhóm theo sự kiện cho phép xử lý song song giữa các nhóm trong khi vẫn giữ thứ tự nội bộ mỗi nhóm.\n✓ MessageGroupId riêng mỗi sự kiện — đúng, các group được xử lý song song, tăng throughput, vẫn giữ ordering trong từng sự kiện.\n✗ Chuyển standard queue — mất đảm bảo thứ tự, vi phạm yêu cầu.\n✗ Nhiều consumer cùng một group — message trong một group vẫn được xử lý tuần tự, không tăng được throughput của group đó.\n✗ High throughput mode với một group duy nhất — vẫn nghẽn vì cùng một MessageGroupId xử lý tuần tự.","domain":2,"mock":2},
  {"id":"saa-m3-025","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một pipeline cần ingest và xử lý real-time hàng trăm nghìn clickstream event mỗi giây, cho phép nhiều consumer độc lập đọc lại cùng dữ liệu trong cửa sổ thời gian, và xử lý theo thứ tự trong từng partition. Dịch vụ nào phù hợp nhất?","options":["Amazon Kinesis Data Streams","Amazon SQS standard queue","Amazon SNS topic với nhiều subscriber","Amazon SQS FIFO queue"],"correctIndices":[0],"explanation":"Kinesis Data Streams được thiết kế cho ingestion real-time throughput cao, cho phép nhiều consumer đọc lại dữ liệu trong retention window và giữ thứ tự theo partition key (shard).\n✓ Kinesis Data Streams — đúng, throughput cao, replay được, nhiều consumer độc lập, ordering theo shard.\n✗ SQS standard — message bị xóa sau khi consume, không replay được, không nhiều consumer đọc lại cùng message.\n✗ SNS — fan-out push, không lưu trữ để replay theo cửa sổ thời gian.\n✗ SQS FIFO — giữ thứ tự nhưng throughput thấp hơn nhiều và không hỗ trợ replay/nhiều consumer đọc lại.","domain":2,"mock":3},
  {"id":"saa-m1-026","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một kiến trúc sư đang thiết kế một tier xử lý đơn hàng bất đồng bộ resilient bằng SQS. Những thực hành nào sau đây giúp tier này highly available và fault tolerant? (Chọn 2)","options":["Đặt consumer trong một Auto Scaling group trải trên nhiều Availability Zones","Cấu hình DLQ để cô lập các message liên tục xử lý thất bại","Đặt visibility timeout về 0 để message luôn sẵn sàng được nhận","Cho một consumer EC2 duy nhất xử lý toàn bộ queue để đảm bảo thứ tự","Tắt retry để tránh xử lý message nhiều lần"],"correctIndices":[0,1],"explanation":"Consumer trải nhiều AZ trong Auto Scaling group đảm bảo HA, và DLQ cô lập poison message để hệ thống không nghẽn.\n✓ Auto Scaling consumer trên nhiều AZ — đúng, mất một AZ vẫn còn consumer khác xử lý, highly available.\n✓ DLQ cô lập message lỗi — đúng, tăng độ bền và ngăn message hỏng làm nghẽn xử lý.\n✗ Visibility timeout = 0 — sai, message bị nhiều worker nhận đồng thời, gây xử lý trùng nghiêm trọng.\n✗ Một consumer duy nhất — sai, single point of failure, không HA.\n✗ Tắt retry — sai, SQS không hoạt động theo cách đó; mất khả năng thử lại làm giảm độ bền.","domain":2,"mock":1},
  {"id":"saa-m2-026","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng three-tier có web tier, application tier và database tier. Đội bảo mật muốn application tier có thể scale độc lập với web tier và không bị ảnh hưởng khi tải web tăng đột biến. Cách decouple hai tier tốt nhất là gì?","options":["Chèn một SQS queue giữa web tier và application tier để xử lý bất đồng bộ","Đặt cả hai tier trong cùng một Auto Scaling group","Tăng kích thước instance của application tier","Cho web tier gọi application tier trực tiếp qua synchronous HTTP"],"correctIndices":[0],"explanation":"SQS giữa hai tier giúp chúng scale độc lập và hấp thụ đột biến tải mà không ảnh hưởng lẫn nhau.\n✓ SQS giữa web và application tier — đúng, decoupled, mỗi tier scale độc lập, đột biến web không làm sập app tier.\n✗ Cùng một Auto Scaling group — gắn chặt hai tier, không scale độc lập được.\n✗ Tăng kích thước instance — vertical scaling có giới hạn, không giải quyết coupling.\n✗ Gọi synchronous HTTP — kết nối chặt, đột biến web tier trực tiếp dội sang app tier.","domain":2,"mock":2},
  {"id":"saa-m3-026","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ứng dụng dùng SNS fan-out tới nhiều SQS queue. Một microservice mới chỉ quan tâm tới event có thuộc tính 'eventType' bằng 'order_created', không muốn nhận các loại event khác để tránh xử lý lãng phí. Cách tối ưu nhất để chỉ nhận đúng loại event này là gì?","options":["Áp dụng SNS subscription filter policy trên subscription của queue đó","Cho consumer đọc mọi message rồi tự lọc và xóa message không liên quan","Tạo một SNS topic riêng cho mỗi loại event","Dùng EventBridge thay thế toàn bộ kiến trúc SNS hiện có"],"correctIndices":[0],"explanation":"SNS message filtering cho phép subscription chỉ nhận message khớp filter policy, giảm xử lý thừa mà không đổi kiến trúc.\n✓ SNS subscription filter policy — đúng, chỉ message khớp 'order_created' được đẩy vào queue, tối ưu và ít thay đổi nhất.\n✗ Đọc rồi tự lọc — vẫn nhận và xử lý mọi message, lãng phí tài nguyên và chi phí.\n✗ Topic riêng mỗi loại event — tăng phức tạp quản lý và producer phải biết route, kém linh hoạt.\n✗ Thay toàn bộ bằng EventBridge — over-engineering cho một nhu cầu lọc đơn giản đã có sẵn trong SNS.","domain":2,"mock":3},
  {"id":"saa-m1-027","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty muốn lập lịch chạy một tác vụ làm sạch dữ liệu mỗi ngày lúc 2 giờ sáng bằng cách kích hoạt một Lambda function, theo cách serverless và ít vận hành. Dịch vụ nào phù hợp nhất để kích hoạt theo lịch?","options":["Amazon EventBridge Scheduler (rule theo cron)","Một EC2 instance chạy cron job gọi Lambda","Amazon SQS với delay queue đặt 24 giờ","AWS Step Functions polling liên tục cho tới 2 giờ sáng"],"correctIndices":[0],"explanation":"EventBridge Scheduler kích hoạt target theo biểu thức cron/rate, serverless và không cần hạ tầng quản lý.\n✓ EventBridge Scheduler theo cron — đúng, serverless, kích hoạt Lambda đúng lịch, ít vận hành nhất.\n✗ EC2 chạy cron — phải duy trì và patch một instance chỉ để chạy lịch, vận hành nhiều.\n✗ SQS delay queue 24 giờ — delay tối đa của SQS là 15 phút, không dùng cho lịch hằng ngày được.\n✗ Step Functions polling — lãng phí và phức tạp, không phải công cụ lập lịch.","domain":2,"mock":1},
  {"id":"saa-m2-027","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng xử lý video upload. Khi có upload, công ty muốn Lambda function được gọi tự động, nhưng nếu lượng upload đột biến vượt giới hạn concurrency của Lambda thì các yêu cầu không được mất. Kiến trúc nào đảm bảo điều này tốt nhất?","options":["Đưa sự kiện vào SQS queue, cho Lambda đọc từ queue như event source","Cho S3 gọi trực tiếp Lambda đồng bộ với mỗi upload","Tăng reserved concurrency của Lambda lên mức tối đa của account","Cho API Gateway gọi Lambda đồng bộ cho mỗi upload"],"correctIndices":[0],"explanation":"SQS làm buffer trước Lambda; khi vượt concurrency, message vẫn nằm trong queue và được xử lý dần, không mất yêu cầu.\n✓ SQS làm buffer + Lambda đọc từ queue — đúng, queue hấp thụ đột biến, Lambda xử lý theo concurrency mà không mất event.\n✗ S3 gọi Lambda đồng bộ — S3 thực ra gọi Lambda bất đồng bộ, nhưng khi throttle kéo dài và hết retry vẫn có thể mất event nếu không có buffer/DLQ.\n✗ Tăng reserved concurrency — vẫn có trần account-level, đột biến lớn vẫn có thể throttle và mất.\n✗ API Gateway đồng bộ — gọi đồng bộ vẫn bị throttle khi vượt concurrency, không buffer.","domain":2,"mock":2},
  {"id":"saa-m3-027","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một microservice cần gọi đồng bộ một service khác qua nhiều bước có điều kiện rẽ nhánh, retry, và xử lý lỗi rõ ràng, với khả năng quan sát từng bước. Đội muốn tránh nhồi logic điều phối vào trong code Lambda. Dịch vụ nào phù hợp nhất?","options":["AWS Step Functions để điều phối workflow","Amazon SQS chuỗi nhiều queue nối tiếp nhau","Amazon SNS fan-out tới các Lambda từng bước","Amazon EventBridge bus với nhiều rule nối tiếp"],"correctIndices":[0],"explanation":"Step Functions cung cấp workflow có state, rẽ nhánh, retry/catch tích hợp và observability từng bước, tách logic điều phối khỏi code.\n✓ Step Functions — đúng, điều phối có điều kiện, retry/error handling khai báo, theo dõi từng state.\n✗ Chuỗi SQS nối tiếp — phải tự viết logic rẽ nhánh/retry và khó quan sát toàn workflow.\n✗ SNS fan-out — phát song song, không phù hợp cho luồng tuần tự có điều kiện và state.\n✗ EventBridge nhiều rule nối tiếp — khó biểu diễn rẽ nhánh/retry phức tạp và theo dõi trạng thái từng bước.","domain":2,"mock":3},
  {"id":"saa-m1-028","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty chạy microservices trên Amazon EKS và cần một load balancer phân phối lưu lượng HTTP tới các pod, hỗ trợ định tuyến theo path và tích hợp tự động với Kubernetes Ingress. Lựa chọn nào phù hợp nhất?","options":["Application Load Balancer (qua AWS Load Balancer Controller)","Network Load Balancer cho lưu lượng HTTP nội bộ","Classic Load Balancer cấu hình thủ công","Gateway Load Balancer đặt trước cluster"],"correctIndices":[0],"explanation":"ALB qua AWS Load Balancer Controller tích hợp với Kubernetes Ingress, định tuyến HTTP theo path tới các pod.\n✓ ALB qua Load Balancer Controller — đúng, Layer 7, path-based routing, tích hợp Ingress chuẩn cho EKS.\n✗ NLB cho HTTP — NLB là Layer 4, không định tuyến theo path; thường dùng cho Service type LoadBalancer L4.\n✗ CLB thủ công — lỗi thời, không tích hợp Ingress hiện đại tốt.\n✗ GWLB — dành cho appliance bảo mật, không phải ingress ứng dụng.","domain":2,"mock":1},
  {"id":"saa-m2-028","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng publish event tới EventBridge. Đôi khi target (một API endpoint) tạm thời không khả dụng và một số event giao thất bại sau khi retry. Công ty không muốn mất các event này. Cách tốt nhất để giữ lại chúng là gì?","options":["Cấu hình dead-letter queue (SQS) cho EventBridge target","Tăng số retry attempt của EventBridge lên vô hạn","Chuyển event qua SNS thay vì EventBridge","Ghi log mọi event ra CloudWatch Logs để gửi lại thủ công"],"correctIndices":[0],"explanation":"EventBridge target hỗ trợ DLQ (SQS) để giữ event không giao được sau khi hết retry.\n✓ DLQ (SQS) cho EventBridge target — đúng, event thất bại được giữ trong DLQ để xử lý lại, không mất.\n✗ Retry vô hạn — không khả thi/khuyến nghị; vẫn có thể mất sau ngưỡng tối đa.\n✗ Chuyển sang SNS — không tự giải quyết vấn đề mất event và làm lại kiến trúc không cần thiết.\n✗ Log ra CloudWatch rồi gửi lại thủ công — nhiều vận hành, dễ sai sót, không tự động.","domain":2,"mock":2},
  {"id":"saa-m3-028","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một ứng dụng đặt sau Application Load Balancer cần ALB ngừng gửi lưu lượng tới các instance không lành mạnh để duy trì độ sẵn sàng. Tính năng nào của ALB đảm nhiệm việc này?","options":["Health checks tới target group","Sticky sessions","Cross-zone load balancing","SSL/TLS termination"],"correctIndices":[0],"explanation":"Health check của ALB liên tục kiểm tra target và chỉ route tới các target lành mạnh.\n✓ Health checks — đúng, ALB tự động ngừng gửi traffic tới instance unhealthy, duy trì availability.\n✗ Sticky sessions — giữ người dùng ở cùng target, không liên quan loại bỏ target hỏng.\n✗ Cross-zone load balancing — phân phối đều giữa các AZ, không phải phát hiện target hỏng.\n✗ SSL/TLS termination — giải mã TLS tại LB, không liên quan health.","domain":2,"mock":3},
  {"id":"saa-m1-029","courseId":"SAA-C03","lesson":"resilient-01-decoupling","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một hệ thống IoT nhận hàng triệu message nhỏ mỗi phút từ thiết bị, cần buffer để load leveling trước khi xử lý bằng một fleet worker có thể scale, với chi phí thấp và không yêu cầu thứ tự nghiêm ngặt. Lựa chọn decoupling nào tối ưu nhất về chi phí và đơn giản?","options":["Amazon SQS standard queue làm buffer, worker tier scale theo độ sâu queue","Amazon SQS FIFO queue để đảm bảo thứ tự tuyệt đối","Amazon Kinesis Data Streams với hàng nghìn shard provisioned","Kết nối thiết bị trực tiếp tới worker EC2 qua TCP"],"correctIndices":[0],"explanation":"Không cần thứ tự nghiêm ngặt nên SQS standard là lựa chọn rẻ và đơn giản nhất để buffer và load-level, kết hợp worker scale theo độ sâu queue.\n✓ SQS standard + worker scale theo queue depth — đúng, throughput gần như không giới hạn, chi phí thấp, đơn giản, đúng nhu cầu.\n✗ FIFO queue — đắt hơn và giới hạn throughput hơn, không cần vì không yêu cầu thứ tự.\n✗ Kinesis nghìn shard provisioned — phức tạp và tốn kém hơn cho nhu cầu buffer đơn giản không cần replay.\n✗ Kết nối trực tiếp TCP tới worker — coupling chặt, không có buffer, mất message khi worker quá tải.","domain":2,"mock":1},
  {"id":"saa-m2-029","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một ứng dụng web chạy trên Amazon RDS for PostgreSQL trong một Single-AZ deployment. Doanh nghiệp yêu cầu database phải tự động chuyển đổi (failover) sang một instance dự phòng khi AZ chứa database gặp sự cố, mà không cần thay đổi connection string của ứng dụng. Giải pháp nào đáp ứng yêu cầu với ít thao tác vận hành nhất?","options":["Bật Multi-AZ deployment cho RDS instance để AWS duy trì một standby instance đồng bộ ở AZ khác và tự động failover","Tạo một Read Replica ở AZ khác và cấu hình ứng dụng tự động promote replica khi primary lỗi","Lên lịch snapshot mỗi giờ và khôi phục thủ công sang AZ khác khi xảy ra sự cố","Triển khai thêm một RDS instance ở AZ khác và dùng một Lambda function tùy chỉnh để đồng bộ dữ liệu"],"correctIndices":[0],"explanation":"Multi-AZ deployment của RDS cung cấp standby đồng bộ ở AZ khác và tự động failover qua DNS endpoint, không đổi connection string.\n✓ Bật Multi-AZ: AWS quản lý standby đồng bộ và failover tự động qua cùng một endpoint, ít vận hành nhất.\n✗ Read Replica là replication bất đồng bộ phục vụ scale đọc, không tự động failover và promote cần thao tác thủ công, có thể mất dữ liệu.\n✗ Snapshot khôi phục thủ công có RTO/RPO cao và nhiều thao tác.\n✗ Lambda tự đồng bộ là giải pháp tự xây dựng, nhiều vận hành và rủi ro.","domain":2,"mock":2},
  {"id":"saa-m3-029","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty thương mại điện tử toàn cầu chạy ứng dụng trên hai Region: us-east-1 và eu-west-1, mỗi Region có một Application Load Balancer. Họ muốn người dùng được định tuyến tới Region có độ trễ thấp nhất, đồng thời tự động chuyển hướng khỏi Region nào không khỏe mạnh. Cấu hình Route 53 nào phù hợp nhất?","options":["Latency-based routing kết hợp với Route 53 health checks gắn vào từng record của hai Region","Geolocation routing dựa trên quốc gia người dùng mà không cần health checks","Weighted routing với trọng số bằng nhau cho hai Region","Simple routing trỏ tới cả hai ALB và để client tự chọn"],"correctIndices":[0],"explanation":"Latency-based routing chọn Region độ trễ thấp nhất, còn health checks loại bỏ endpoint không khỏe mạnh.\n✓ Latency-based + health checks: định tuyến theo độ trễ thấp nhất và tự động bỏ qua Region không khỏe mạnh.\n✗ Geolocation định tuyến theo vị trí địa lý chứ không theo độ trễ, và không có health checks thì không failover.\n✗ Weighted phân chia theo tỷ lệ cố định, không tối ưu độ trễ.\n✗ Simple routing không hỗ trợ health checks và trả nhiều giá trị ngẫu nhiên, không đảm bảo độ trễ thấp nhất.","domain":2,"mock":3},
  {"id":"saa-m1-030","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng tài chính yêu cầu RPO gần bằng 0 và RTO chỉ vài phút cho database quan hệ, với khả năng đọc với độ trễ thấp ở nhiều Region và khả năng promote một Region thứ cấp lên primary trong dưới 1 phút khi xảy ra thảm họa Region. Giải pháp nào phù hợp nhất?","options":["Amazon Aurora Global Database với một primary Region và các secondary Region read-only","Amazon RDS Multi-AZ deployment trong một Region duy nhất","Amazon RDS cross-Region Read Replicas được promote thủ công khi cần","Sao chép EBS snapshots của database sang Region khác mỗi 15 phút"],"correctIndices":[0],"explanation":"Aurora Global Database replicate với độ trễ thường dưới 1 giây và cho phép promote secondary Region trong khoảng 1 phút.\n✓ Aurora Global Database: replication storage-level rất nhanh, RPO gần 0, RTO thấp, đọc low-latency ở secondary Region.\n✗ RDS Multi-AZ chỉ bảo vệ trong một Region, không chống thảm họa cấp Region.\n✗ RDS cross-Region Read Replica có độ trễ replication cao hơn và promote chậm hơn Aurora Global.\n✗ EBS snapshot mỗi 15 phút cho RPO tới 15 phút và RTO cao, không đạt yêu cầu.","domain":2,"mock":1},
  {"id":"saa-m2-030","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một startup muốn chiến lược disaster recovery cho hệ thống dựa trên EC2 và RDS với chi phí thấp nhất, chấp nhận RTO vài giờ và RPO vài giờ. Hiện tại họ chưa chạy bất kỳ tài nguyên nào ở Region dự phòng. Chiến lược DR nào phù hợp nhất với yêu cầu chi phí?","options":["Backup and Restore: sao lưu định kỳ (AMI, snapshot) sang Region khác và chỉ khôi phục tài nguyên khi có thảm họa","Pilot Light: luôn duy trì database replica đang chạy và core services tối thiểu ở Region dự phòng","Warm Standby: duy trì một bản sao thu nhỏ của toàn bộ hệ thống luôn chạy ở Region dự phòng","Multi-Site active-active: chạy đầy đủ hệ thống ở cả hai Region và phân tải giữa chúng"],"correctIndices":[0],"explanation":"Backup and Restore rẻ nhất vì không duy trì hạ tầng chạy ở Region dự phòng, phù hợp RTO/RPO vài giờ.\n✓ Backup and Restore: chỉ trả phí lưu trữ backup, dựng tài nguyên khi cần, chi phí thấp nhất với RTO/RPO chấp nhận được.\n✗ Pilot Light duy trì core đang chạy nên đắt hơn và không cần thiết khi RTO vài giờ.\n✗ Warm Standby chạy bản thu nhỏ liên tục, chi phí cao hơn nhiều.\n✗ Multi-Site active-active đắt nhất, dành cho RTO/RPO gần 0.","domain":2,"mock":2},
  {"id":"saa-m3-030","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng web stateless chạy sau Application Load Balancer với Auto Scaling group. Hiện tại tất cả EC2 instances nằm trong một Availability Zone duy nhất. Kiến trúc sư muốn tăng khả năng chịu lỗi (fault tolerance) để ứng dụng vẫn hoạt động khi một AZ gặp sự cố. Hành động nào đạt mục tiêu hiệu quả nhất?","options":["Cấu hình Auto Scaling group trải trên nhiều subnet thuộc các AZ khác nhau và bật cross-zone load balancing","Tăng kích thước instance type để mỗi instance chịu tải lớn hơn","Bật detailed CloudWatch monitoring cho các instance trong AZ hiện tại","Đặt một Network Load Balancer trước Application Load Balancer hiện tại"],"correctIndices":[0],"explanation":"Trải instances trên nhiều AZ là cách cơ bản để chịu được lỗi một AZ.\n✓ ASG đa AZ + cross-zone LB: nếu một AZ lỗi, các instance ở AZ khác vẫn phục vụ, đạt fault tolerance.\n✗ Tăng instance type không giúp gì khi cả AZ chứa toàn bộ instance bị mất.\n✗ Detailed monitoring chỉ tăng độ chi tiết metric, không tăng khả năng chịu lỗi.\n✗ Đặt NLB trước ALB không giải quyết vấn đề tập trung trong một AZ.","domain":2,"mock":3},
  {"id":"saa-m1-031","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty cần triển khai trang bảo trì (maintenance page) lưu trên S3 để hiển thị cho người dùng khi ứng dụng chính ở một Region không khỏe mạnh. Họ muốn Route 53 tự động chuyển traffic từ ứng dụng chính sang trang bảo trì. Cấu hình routing nào phù hợp nhất?","options":["Failover routing với primary record (ALB) gắn health check và secondary record trỏ tới S3 static website","Weighted routing chia 90% cho ALB và 10% cho S3 static website","Multivalue answer routing trả về cả ALB và S3 endpoint","Latency-based routing giữa ALB và S3 static website"],"correctIndices":[0],"explanation":"Failover routing được thiết kế đúng cho mô hình active-passive primary/secondary.\n✓ Failover routing với health check: khi primary ALB không khỏe mạnh, Route 53 trả về secondary trỏ S3, hiển thị trang bảo trì.\n✗ Weighted vẫn gửi 10% traffic sang S3 ngay cả khi ALB khỏe mạnh, không đúng mục tiêu.\n✗ Multivalue answer trả nhiều giá trị song song, không phải mô hình failover passive.\n✗ Latency-based không xét trạng thái active-passive.","domain":2,"mock":1},
  {"id":"saa-m2-031","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một ứng dụng global yêu cầu một database NoSQL có khả năng đọc-ghi với độ trễ thấp ở nhiều Region đồng thời (active-active), tự động sao chép giữa các Region, và chịu được lỗi toàn bộ một Region mà ứng dụng vẫn ghi được ở Region khác. Những phát biểu nào ĐÚNG về giải pháp phù hợp? (Chọn 2)","options":["Amazon DynamoDB Global Tables cung cấp replication active-active multi-Region, cho phép đọc-ghi ở mỗi Region tham gia","DynamoDB Global Tables sử dụng cơ chế last-writer-wins để giải quyết xung đột ghi đồng thời","Aurora Global Database cho phép ghi đồng thời ở mọi secondary Region với độ trễ thấp","Phải tự xây dựng một Lambda pipeline để sao chép dữ liệu giữa các bảng DynamoDB ở mỗi Region","DynamoDB Global Tables yêu cầu cấu hình thủ công cross-Region replication qua Kinesis cho mỗi item"],"correctIndices":[0,1],"explanation":"DynamoDB Global Tables là giải pháp NoSQL multi-Region active-active được quản lý hoàn toàn, dùng last-writer-wins để xử lý xung đột.\n✓ Global Tables hỗ trợ đọc-ghi ở mọi Region tham gia, replication tự động.\n✓ Cơ chế giải quyết xung đột là last-writer-wins dựa trên timestamp.\n✗ Aurora Global Database chỉ có một writer Region; secondary là read-only, không phải active-active ghi.\n✗ Không cần tự xây Lambda pipeline vì replication là tính năng tích hợp.\n✗ Không cần cấu hình Kinesis thủ công; Global Tables tự sao chép.","domain":2,"mock":2},
  {"id":"saa-m3-031","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một tổ chức cần một giải pháp tập trung để lên lịch và quản lý backup cho nhiều dịch vụ: EBS volumes, RDS databases, DynamoDB tables và EFS file systems, đồng thời áp dụng chính sách lưu trữ (retention) thống nhất và sao chép backup sang Region khác. Dịch vụ nào phù hợp nhất với ít thao tác vận hành nhất?","options":["AWS Backup với backup plan và cross-Region copy","Viết script tùy chỉnh dùng AWS CLI để tạo snapshot cho từng dịch vụ","Amazon Data Lifecycle Manager chỉ cho EBS snapshots","Bật versioning trên S3 và copy thủ công các backup vào đó"],"correctIndices":[0],"explanation":"AWS Backup là dịch vụ quản lý backup tập trung cho nhiều dịch vụ với policy và cross-Region copy tích hợp.\n✓ AWS Backup: một backup plan duy nhất quản lý EBS, RDS, DynamoDB, EFS, áp retention thống nhất và copy cross-Region, ít vận hành nhất.\n✗ Script CLI tùy chỉnh nhiều vận hành và dễ lỗi.\n✗ Data Lifecycle Manager chỉ quản lý EBS/AMI snapshots, không bao phủ RDS/DynamoDB/EFS.\n✗ S3 versioning không tạo backup cho các dịch vụ trên.","domain":2,"mock":3},
  {"id":"saa-m1-032","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty media lưu nội dung quan trọng trong một S3 bucket ở us-east-1. Để tuân thủ quy định, một bản sao của tất cả object phải tồn tại ở một Region khác và được cập nhật tự động khi có object mới. Giải pháp nào phù hợp nhất?","options":["Bật S3 Cross-Region Replication (CRR) tới một bucket đích ở Region khác","Bật S3 Versioning và chạy một AWS Lambda định kỳ để copy object sang Region khác","Dùng S3 Transfer Acceleration để tăng tốc tải object lên","Bật S3 Same-Region Replication (SRR) sang một bucket khác trong cùng Region"],"correctIndices":[0],"explanation":"S3 Cross-Region Replication tự động sao chép object sang bucket ở Region khác khi có thay đổi.\n✓ CRR: sao chép tự động, bất đồng bộ object mới sang Region khác, đáp ứng tuân thủ multi-Region.\n✗ Lambda định kỳ copy là tự xây dựng, không tự động tức thì và nhiều vận hành.\n✗ Transfer Acceleration chỉ tăng tốc upload, không tạo bản sao Region khác.\n✗ SRR sao chép trong cùng Region, không đáp ứng yêu cầu Region khác.","domain":2,"mock":1},
  {"id":"saa-m2-032","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một hệ thống thương mại điện tử dùng Warm Standby ở Region thứ cấp. Trong DR test, đội vận hành nhận thấy khi failover, ứng dụng cần xử lý lưu lượng production đầy đủ nhưng Region thứ cấp chỉ chạy số lượng instance tối thiểu. Cách nào giúp Region thứ cấp đáp ứng tải production nhanh nhất khi failover, đồng thời giữ chi phí thấp khi chưa failover?","options":["Giữ Auto Scaling group ở Region thứ cấp với số instance tối thiểu và cấu hình scaling policy để mở rộng nhanh khi traffic chuyển tới","Chạy sẵn toàn bộ số lượng instance production ở Region thứ cấp 24/7","Tắt hoàn toàn Region thứ cấp và chỉ tạo instance khi xảy ra thảm họa","Chuyển sang Multi-AZ trong một Region duy nhất thay vì dùng Region thứ cấp"],"correctIndices":[0],"explanation":"Warm Standby duy trì bản thu nhỏ luôn chạy và mở rộng (scale out) khi failover, cân bằng tốc độ và chi phí.\n✓ ASG tối thiểu + scaling policy: chạy nhỏ để tiết kiệm, tự mở rộng nhanh đáp ứng tải production khi failover.\n✗ Chạy full 24/7 biến nó thành active-active/hot, chi phí cao không cần thiết.\n✗ Tắt hoàn toàn là Backup and Restore/Pilot Light, RTO cao hơn, mâu thuẫn với Warm Standby.\n✗ Multi-AZ một Region không bảo vệ thảm họa cấp Region.","domain":2,"mock":2},
  {"id":"saa-m3-032","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một ứng dụng dùng Amazon Aurora MySQL. Nhóm phát triển muốn giảm tải đọc khỏi instance ghi chính và tăng tính sẵn sàng bằng cách cho phép một replica được tự động promote thành primary khi primary lỗi, tất cả trong cùng một Region. Giải pháp nào phù hợp nhất?","options":["Thêm Aurora Replicas trong cùng Aurora cluster trải trên nhiều AZ","Tạo một RDS Read Replica truyền thống ở Region khác","Bật S3 Cross-Region Replication cho dữ liệu database","Chuyển sang DynamoDB Global Tables"],"correctIndices":[0],"explanation":"Aurora Replicas trong cluster vừa phục vụ đọc vừa là ứng viên failover tự động trong Region.\n✓ Aurora Replicas đa AZ: phục vụ đọc và được Aurora tự động promote khi primary lỗi, HA trong Region.\n✗ RDS Read Replica cross-Region không tự promote và nhằm DR/đọc cross-Region.\n✗ S3 CRR không liên quan đến failover database.\n✗ DynamoDB Global Tables là NoSQL, thay đổi cả mô hình dữ liệu, không cần thiết.","domain":2,"mock":3},
  {"id":"saa-m1-033","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một dịch vụ API nhận traffic từ người dùng ở nhiều quốc gia. Vì lý do tuân thủ dữ liệu, người dùng ở Đức phải luôn được phục vụ bởi endpoint ở eu-central-1, người dùng ở Nhật bởi ap-northeast-1, còn lại đi tới us-east-1. Cấu hình Route 53 nào phù hợp nhất?","options":["Geolocation routing với record riêng cho Đức, Nhật và một default record cho phần còn lại","Latency-based routing giữa ba Region","Weighted routing chia đều cho ba Region","Failover routing với us-east-1 là primary"],"correctIndices":[0],"explanation":"Geolocation routing định tuyến dựa trên vị trí địa lý của người dùng, phù hợp yêu cầu tuân thủ dữ liệu theo quốc gia.\n✓ Geolocation routing: ánh xạ quốc gia tới Region cụ thể và có default record cho các vị trí khác, đáp ứng tuân thủ.\n✗ Latency-based chọn theo độ trễ, không đảm bảo người Đức luôn tới eu-central-1.\n✗ Weighted chia ngẫu nhiên theo tỷ lệ, vi phạm tuân thủ.\n✗ Failover là active-passive, không phục vụ theo địa lý.","domain":2,"mock":1},
  {"id":"saa-m2-033","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ngân hàng triển khai chiến lược Pilot Light. Database primary ở Region chính được sao chép liên tục sang Region dự phòng, nhưng application/web tier ở Region dự phòng KHÔNG chạy thường xuyên. Khi xảy ra thảm họa, điều gì PHẢI xảy ra để khôi phục dịch vụ, và đây là đặc trưng nào của Pilot Light?","options":["Khởi chạy (provision và scale up) application/web tier từ AMI/template ở Region dự phòng rồi điều hướng traffic; đặc trưng là core data luôn chạy còn app tier được dựng khi cần","Không cần làm gì vì toàn bộ tier đã chạy sẵn; đặc trưng là active-active","Phục hồi toàn bộ từ snapshot kể cả database; đặc trưng là Backup and Restore","Chỉ cần đổi DNS vì bản thu nhỏ của toàn hệ thống đã chạy liên tục; đặc trưng là Warm Standby"],"correctIndices":[0],"explanation":"Pilot Light giữ phần lõi dữ liệu luôn được replicate/chạy, còn app tier chỉ được dựng khi cần.\n✓ Khởi chạy app tier từ template + điều hướng traffic: đúng bản chất Pilot Light với data core luôn sẵn sàng.\n✗ Không phải active-active vì app tier không chạy sẵn đầy đủ.\n✗ Không phải Backup and Restore vì database đã được replicate liên tục, không khôi phục từ snapshot.\n✗ Không phải Warm Standby vì Warm Standby chạy bản thu nhỏ của toàn hệ thống liên tục, còn Pilot Light app tier thường tắt.","domain":2,"mock":2},
  {"id":"saa-m3-033","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng quan trọng yêu cầu RTO và RPO gần như bằng 0, người dùng toàn cầu phải luôn được phục vụ bởi cả hai Region cùng lúc, và nếu một Region mất hoàn toàn thì Region kia phải tiếp tục xử lý 100% traffic ngay lập tức. Chiến lược DR nào phù hợp nhất?","options":["Multi-Site active-active với cả hai Region cùng phục vụ traffic qua Route 53","Warm Standby với Region thứ cấp chạy bản thu nhỏ","Pilot Light với chỉ database được replicate","Backup and Restore với cross-Region snapshot"],"correctIndices":[0],"explanation":"Multi-Site active-active đạt RTO/RPO gần 0 vì cả hai Region đều phục vụ traffic thật.\n✓ Multi-Site active-active: cả hai Region xử lý production đồng thời, mất một Region thì Region kia tiếp nhận ngay, RTO/RPO gần 0.\n✗ Warm Standby cần scale out khi failover nên RTO không bằng 0.\n✗ Pilot Light cần dựng app tier khi thảm họa, RTO cao hơn.\n✗ Backup and Restore có RTO/RPO cao nhất.","domain":2,"mock":3},
  {"id":"saa-m1-034","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng dùng Amazon RDS Multi-AZ. Kiến trúc sư cần xác nhận hành vi khi AZ chứa primary instance gặp sự cố. Phát biểu nào mô tả ĐÚNG nhất điều xảy ra?","options":["RDS tự động failover sang standby instance ở AZ khác và cập nhật DNS endpoint để trỏ tới instance mới","RDS tự động tạo một Read Replica mới và promote nó thành primary","Ứng dụng phải tự đổi connection string sang IP của standby instance","RDS yêu cầu khôi phục thủ công từ snapshot gần nhất"],"correctIndices":[0],"explanation":"Multi-AZ duy trì standby đồng bộ và tự động failover bằng cách trỏ lại DNS endpoint.\n✓ Tự động failover + cập nhật DNS: ứng dụng dùng cùng endpoint nên không cần đổi cấu hình.\n✗ Standby trong Multi-AZ không phải là Read Replica; nó không phục vụ đọc và đã tồn tại sẵn.\n✗ Không cần đổi connection string vì endpoint không đổi.\n✗ Không cần khôi phục thủ công từ snapshot.","domain":2,"mock":1},
  {"id":"saa-m2-034","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một kiến trúc sư đang thiết kế Route 53 health checks cho một ứng dụng failover multi-Region. Những phát biểu nào ĐÚNG về Route 53 health checks? (Chọn 2)","options":["Route 53 health check có thể giám sát một endpoint qua HTTP/HTTPS/TCP và đánh dấu không khỏe mạnh khi vượt ngưỡng lỗi","Calculated health check có thể kết hợp trạng thái của nhiều health check con để quyết định trạng thái tổng","Route 53 health check chỉ hoạt động với các endpoint nằm trong AWS, không hỗ trợ endpoint on-premises","Health check phải được gắn vào một CloudFront distribution thì mới có hiệu lực","Route 53 chỉ failover khi toàn bộ Region của AWS bị down, không phản ứng với lỗi endpoint đơn lẻ"],"correctIndices":[0,1],"explanation":"Route 53 health checks giám sát endpoint qua nhiều giao thức và có thể tổng hợp qua calculated health check.\n✓ Health check hỗ trợ HTTP/HTTPS/TCP với failure threshold để xác định trạng thái.\n✓ Calculated (parent) health check kết hợp nhiều child health check.\n✗ Health check hỗ trợ cả endpoint ngoài AWS (on-premises có IP công khai).\n✗ Không bắt buộc gắn vào CloudFront; gắn vào DNS record.\n✗ Failover phản ứng với lỗi endpoint, không cần cả Region down.","domain":2,"mock":2},
  {"id":"saa-m3-034","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng phân tích chạy ở us-east-1 và đọc dữ liệu từ một Aurora MySQL cluster. Đội phân tích ở châu Âu cần đọc cùng dữ liệu với độ trễ thấp mà không làm tăng tải lên writer ở us-east-1, và yêu cầu sẵn sàng promote châu Âu thành primary khi cần DR. Giải pháp nào phù hợp nhất?","options":["Aurora Global Database với secondary Region ở châu Âu phục vụ đọc low-latency","Tạo nhiều Aurora Replicas trong us-east-1","Bật S3 Cross-Region Replication cho các export dữ liệu","Dùng DynamoDB Global Tables để nhân bản dữ liệu phân tích"],"correctIndices":[0],"explanation":"Aurora Global Database cung cấp secondary Region đọc low-latency và khả năng promote cho DR.\n✓ Aurora Global Database: secondary Region châu Âu phục vụ đọc nhanh, không tải lên writer chính, và promote được khi DR.\n✗ Aurora Replicas trong us-east-1 không giảm độ trễ cho người dùng châu Âu.\n✗ S3 CRR cho export không cung cấp truy vấn quan hệ low-latency theo thời gian thực.\n✗ DynamoDB Global Tables đổi mô hình dữ liệu sang NoSQL, không phù hợp truy vấn quan hệ hiện có.","domain":2,"mock":3},
  {"id":"saa-m1-035","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một Application Load Balancer phân phối traffic tới các EC2 instance trong một target group. Kiến trúc sư muốn ALB tự động ngừng gửi request tới các instance không phản hồi đúng và chỉ gửi tới các instance khỏe mạnh. Tính năng nào của ALB thực hiện điều này?","options":["Health checks của target group","Sticky sessions","Cross-zone load balancing","SSL/TLS termination"],"correctIndices":[0],"explanation":"Health checks của target group xác định instance khỏe mạnh để định tuyến traffic.\n✓ Health checks: ALB ngừng gửi request tới target không qua health check và chỉ phục vụ instance khỏe mạnh.\n✗ Sticky sessions giữ phiên người dùng trên cùng target, không kiểm tra sức khỏe.\n✗ Cross-zone load balancing phân bố đều giữa các AZ, không kiểm tra sức khỏe.\n✗ SSL/TLS termination xử lý mã hóa, không liên quan tới sức khỏe target.","domain":2,"mock":1},
  {"id":"saa-m2-035","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty đang chuyển dần traffic sang một phiên bản mới của ứng dụng đặt ở một nhóm server riêng. Họ muốn gửi 10% traffic tới phiên bản mới và 90% tới phiên bản cũ, sau đó tăng dần tỷ lệ. Cấu hình Route 53 nào phù hợp nhất?","options":["Weighted routing với trọng số 10 cho phiên bản mới và 90 cho phiên bản cũ","Latency-based routing giữa hai phiên bản","Failover routing với phiên bản mới là primary","Geolocation routing dựa trên quốc gia người dùng"],"correctIndices":[0],"explanation":"Weighted routing cho phép chia traffic theo tỷ lệ tùy chỉnh, lý tưởng cho canary/blue-green dần dần.\n✓ Weighted routing 10/90: phân phối traffic theo tỷ lệ và tăng dần bằng cách đổi trọng số.\n✗ Latency-based định tuyến theo độ trễ, không kiểm soát tỷ lệ traffic.\n✗ Failover là active-passive, không chia tỷ lệ.\n✗ Geolocation chia theo vị trí, không theo tỷ lệ phần trăm.","domain":2,"mock":2},
  {"id":"saa-m3-035","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một hệ thống xử lý đơn hàng dùng một SQS queue giữa web tier và worker tier chạy trên EC2 trong một Auto Scaling group đa AZ. Trong giờ cao điểm, worker không theo kịp và một số đơn bị mất khi instance bị terminate giữa chừng. Thay đổi nào cải thiện fault tolerance của quá trình xử lý đơn tốt nhất?","options":["Đảm bảo worker chỉ xóa message khỏi queue sau khi xử lý thành công và cấu hình visibility timeout phù hợp để message được xử lý lại nếu worker lỗi","Chuyển SQS sang một database RDS Multi-AZ để lưu đơn hàng","Tăng instance type của worker để xử lý nhanh hơn","Bật cross-zone load balancing trên ALB của web tier"],"correctIndices":[0],"explanation":"SQS đảm bảo không mất message nếu worker xóa message chỉ sau khi xử lý xong và visibility timeout đủ dài.\n✓ Xóa message sau xử lý + visibility timeout đúng: nếu worker chết giữa chừng, message trở lại queue và được xử lý lại, không mất đơn.\n✗ Thay SQS bằng RDS bỏ đi decoupling và không tự nhiên giải quyết retry.\n✗ Tăng instance type giảm tồn đọng nhưng không ngăn mất đơn khi instance bị terminate.\n✗ Cross-zone LB cải thiện cân bằng web tier, không liên quan tới mất message ở worker.","domain":2,"mock":3},
  {"id":"saa-m1-036","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng yêu cầu shared file storage được nhiều EC2 instance ở các AZ khác nhau cùng mount, và phải vẫn sẵn sàng khi một AZ gặp sự cố. Giải pháp lưu trữ nào phù hợp nhất?","options":["Amazon EFS với mount target ở mỗi AZ","Một EBS volume gắn vào instance ở một AZ và chia sẻ qua NFS tự cấu hình","Instance store volume trên mỗi instance","Amazon S3 mount qua một single-AZ gateway tùy chỉnh"],"correctIndices":[0],"explanation":"EFS là file system được chia sẻ, dữ liệu được lưu dư thừa trên nhiều AZ.\n✓ EFS với mount target mỗi AZ: nhiều instance đa AZ cùng truy cập, vẫn sẵn sàng khi một AZ lỗi.\n✗ EBS gắn một AZ là single point of failure và (theo chuẩn) chỉ gắn một instance.\n✗ Instance store là ephemeral, mất dữ liệu khi instance dừng và không chia sẻ.\n✗ S3 qua gateway single-AZ tạo điểm lỗi đơn và không phải POSIX file system tự nhiên.","domain":2,"mock":1},
  {"id":"saa-m2-036","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một doanh nghiệp cần đảm bảo các backup do AWS Backup tạo ra được giữ ở một tài khoản riêng và một Region khác để chống lại việc xóa do lỗi hoặc tấn công ransomware vào tài khoản chính. Tính năng nào phù hợp nhất?","options":["Cross-account và cross-Region backup copy của AWS Backup vào một vault có Vault Lock","Bật S3 Versioning trên bucket chứa backup trong cùng tài khoản","Lên lịch snapshot EBS thủ công và copy bằng tay khi cần","Sử dụng RDS Multi-AZ để có thêm bản sao database"],"correctIndices":[0],"explanation":"AWS Backup hỗ trợ copy backup sang Region và tài khoản khác, kết hợp Vault Lock chống xóa.\n✓ Cross-account + cross-Region copy với Vault Lock: cách ly backup khỏi tài khoản chính và bảo vệ bất biến (immutable).\n✗ S3 Versioning trong cùng tài khoản không cách ly khỏi tấn công vào tài khoản đó.\n✗ Snapshot thủ công nhiều vận hành và không tự bảo vệ cross-account.\n✗ Multi-AZ là HA trong Region, không phải cơ chế backup cách ly.","domain":2,"mock":2},
  {"id":"saa-m3-036","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ứng dụng chạy active-active trên hai Region với DynamoDB Global Tables. Trong một sự cố mạng, cùng một item được cập nhật gần như đồng thời ở cả hai Region với hai giá trị khác nhau. Kết quả cuối cùng được xác định như thế nào và đây là cân nhắc thiết kế nào?","options":["Global Tables áp dụng last-writer-wins dựa trên timestamp; ứng dụng phải thiết kế chấp nhận eventual consistency cross-Region và khả năng mất một bản ghi xung đột","Global Tables khóa item trên toàn bộ Region để đảm bảo strong consistency cross-Region, không có xung đột","Global Tables từ chối cả hai ghi và trả lỗi cho ứng dụng","Global Tables gộp (merge) hai giá trị thành một bản ghi kết hợp tự động"],"correctIndices":[0],"explanation":"DynamoDB Global Tables giải quyết xung đột bằng last-writer-wins và chỉ đảm bảo eventual consistency cross-Region.\n✓ Last-writer-wins theo timestamp + eventual consistency: ứng dụng phải chấp nhận một bản ghi xung đột có thể bị ghi đè.\n✗ Global Tables không cung cấp strong consistency cross-Region hay khóa toàn cục.\n✗ Nó không từ chối cả hai ghi.\n✗ Nó không tự merge hai giá trị; chỉ giữ giá trị ghi sau cùng.","domain":2,"mock":3},
  {"id":"saa-m1-037","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một kiến trúc sư so sánh giữa triển khai Multi-AZ và Multi-Region cho một workload. Phát biểu nào mô tả ĐÚNG nhất sự khác biệt quan trọng giữa hai cách tiếp cận?","options":["Multi-AZ bảo vệ chống lỗi cấp data center/AZ với độ trễ thấp trong một Region; Multi-Region bảo vệ chống thảm họa cấp Region nhưng phức tạp và tốn kém hơn","Multi-AZ và Multi-Region cung cấp mức bảo vệ tương đương, chỉ khác tên gọi","Multi-Region luôn rẻ hơn Multi-AZ vì dùng ít tài nguyên hơn","Multi-AZ bảo vệ chống thảm họa toàn Region còn Multi-Region chỉ bảo vệ trong một AZ"],"correctIndices":[0],"explanation":"Multi-AZ là HA trong một Region; Multi-Region là DR/khả năng chịu lỗi ở quy mô Region với chi phí và độ phức tạp cao hơn.\n✓ Multi-AZ chống lỗi AZ với độ trễ thấp; Multi-Region chống thảm họa Region nhưng phức tạp/tốn kém hơn.\n✗ Hai cách không tương đương về phạm vi bảo vệ.\n✗ Multi-Region thường tốn kém hơn, không rẻ hơn.\n✗ Mô tả đảo ngược vai trò là sai.","domain":2,"mock":1},
  {"id":"saa-m2-037","courseId":"SAA-C03","lesson":"resilient-02-ha-fault-tolerance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng web cần điểm vào (entry point) tĩnh, hiệu năng cao trên toàn cầu và tự động failover nhanh giữa các endpoint ở nhiều Region khi một Region không khỏe mạnh, với failover ở mức nhanh hơn DNS TTL của Route 53. Dịch vụ nào phù hợp nhất?","options":["AWS Global Accelerator với endpoint ở nhiều Region và health checks","Route 53 weighted routing giữa các Region","Amazon CloudFront với một origin duy nhất","Một Network Load Balancer duy nhất trong một Region"],"correctIndices":[0],"explanation":"Global Accelerator cung cấp anycast IP tĩnh và failover nhanh không phụ thuộc DNS TTL.\n✓ Global Accelerator: IP tĩnh toàn cầu, định tuyến qua mạng AWS, failover nhanh dựa trên health check, nhanh hơn DNS-based.\n✗ Route 53 weighted phụ thuộc DNS TTL nên failover chậm hơn và không cung cấp IP tĩnh anycast.\n✗ CloudFront với một origin không cung cấp failover multi-Region cho origin.\n✗ NLB một Region không bảo vệ thảm họa Region.","domain":2,"mock":2},
  {"id":"saa-m1-038","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một ứng dụng web lưu hàng triệu ảnh trên Amazon S3. Người dùng trên toàn cầu phàn nàn việc upload ảnh từ các châu lục xa region của bucket rất chậm. Giải pháp nào cải thiện tốc độ upload với least operational overhead?","options":["Bật S3 Transfer Acceleration trên bucket để upload qua edge location của CloudFront","Tạo bucket S3 ở mỗi region và dùng S3 Cross-Region Replication để đồng bộ","Đặt một CloudFront distribution phía trước bucket và cho người dùng upload qua CloudFront","Dùng AWS Global Accelerator để định tuyến traffic upload đến bucket"],"correctIndices":[0],"explanation":"S3 Transfer Acceleration tận dụng mạng edge của CloudFront để tăng tốc upload xuyên lục địa chỉ bằng một thiết lập bật/tắt.\n✓ Transfer Acceleration định tuyến upload qua edge location gần người dùng rồi đi trên backbone AWS, least operational overhead.\n✗ Tạo bucket mỗi region kèm replication phức tạp và bản thân upload ban đầu vẫn phải đi xa.\n✗ CloudFront tối ưu cho phân phối (download/cache), không phải kênh tăng tốc upload trực tiếp như Transfer Acceleration.\n✗ Global Accelerator dùng cho endpoint như ALB/NLB/EC2, không hỗ trợ upload trực tiếp vào S3 bucket.","domain":3,"mock":1},
  {"id":"saa-m2-038","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một hệ thống phân tích log ghi dữ liệu vào S3 với tốc độ rất cao và liên tục đạt giới hạn request. Mọi object hiện đều dùng cùng một prefix dạng logs/. Kiến trúc sư cần tăng throughput request mà không đổi sang dịch vụ khác. Cách nào hiệu quả nhất?","options":["Phân tán key object qua nhiều prefix (ví dụ logs/2026/01/, logs/2026/02/) để tăng số request song song","Bật S3 Versioning để mỗi version được xử lý bởi partition riêng","Chuyển bucket sang storage class S3 Intelligent-Tiering để tăng throughput","Gom nhiều log thành một object lớn duy nhất rồi ghi mỗi giờ một lần"],"correctIndices":[0],"explanation":"S3 scale theo prefix: mỗi prefix hỗ trợ tới 3.500 PUT/COPY/POST/DELETE và 5.500 GET/HEAD mỗi giây, nên phân tán key qua nhiều prefix sẽ nhân throughput.\n✓ Nhiều prefix tạo nhiều partition song song, tăng tổng số request mỗi giây.\n✗ Versioning không liên quan đến giới hạn request rate theo prefix.\n✗ Intelligent-Tiering tối ưu chi phí lưu trữ, không thay đổi request rate.\n✗ Gom thành một object lớn ghi mỗi giờ làm mất tính real-time và không phải cách scale request rate.","domain":3,"mock":2},
  {"id":"saa-m3-037","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một cụm HPC chạy mô phỏng khí động học cần đọc/ghi dataset hàng trăm TB với throughput hàng trăm GB/s và độ trễ submillisecond, dữ liệu nguồn đang nằm trên S3. Loại lưu trữ nào phù hợp nhất?","options":["Amazon FSx for Lustre liên kết với S3 bucket chứa dataset","Amazon EFS với throughput mode là Elastic","Amazon EBS gp3 volume gắn vào từng node tính toán","Amazon FSx for Windows File Server với SSD storage"],"correctIndices":[0],"explanation":"FSx for Lustre là file system song song hiệu năng cao chuyên cho HPC, đạt throughput tới hàng trăm GB/s và độ trễ submillisecond, lại tích hợp trực tiếp với S3.\n✓ FSx for Lustre xử lý workload HPC dữ liệu lớn và lazy-load/đồng bộ dữ liệu từ S3.\n✗ EFS phù hợp file sharing NFS chung nhưng không đạt mức throughput/latency của Lustre cho HPC.\n✗ EBS gp3 gắn từng node là block storage không chia sẻ, không cung cấp file system song song dùng chung cho cả cụm.\n✗ FSx for Windows File Server phục vụ workload Windows/SMB, không phải HPC Linux hiệu năng cao.","domain":3,"mock":3},
  {"id":"saa-m1-039","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng .NET cũ chạy trên fleet EC2 Windows cần một shared file system hỗ trợ SMB, NTFS permissions và tích hợp Active Directory để nhiều instance cùng truy cập. Giải pháp managed nào phù hợp nhất?","options":["Amazon FSx for Windows File Server","Amazon EFS mount qua NFS trên các instance Windows","Amazon FSx for Lustre","Một EC2 Windows tự cấu hình làm file server chia sẻ EBS volume"],"correctIndices":[0],"explanation":"FSx for Windows File Server là dịch vụ managed cung cấp SMB, NTFS và tích hợp AD đúng nhu cầu workload Windows.\n✓ FSx for Windows File Server hỗ trợ native SMB protocol, NTFS ACL và join Active Directory.\n✗ EFS chỉ hỗ trợ NFS, không phù hợp SMB/NTFS cho ứng dụng Windows.\n✗ FSx for Lustre dành cho HPC Linux, không cung cấp SMB cho Windows.\n✗ Tự dựng file server trên EC2 tăng operational overhead về vận hành, HA và backup.","domain":3,"mock":1},
  {"id":"saa-m2-039","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một cơ sở dữ liệu quan hệ self-managed trên EC2 cần volume EBS cung cấp IOPS và throughput ổn định ở mức cao, đồng thời cho phép điều chỉnh IOPS và throughput độc lập với dung lượng để tối ưu chi phí. Loại volume nào phù hợp nhất?","options":["EBS gp3","EBS gp2","EBS st1 (Throughput Optimized HDD)","EBS sc1 (Cold HDD)"],"correctIndices":[0],"explanation":"gp3 cho phép cấu hình IOPS và throughput độc lập với dung lượng, hiệu năng baseline ổn định và rẻ hơn gp2.\n✓ gp3 tách rời IOPS/throughput khỏi size, cost-effective cho database cần hiệu năng dự đoán được.\n✗ gp2 gắn IOPS theo dung lượng (3 IOPS/GB) và phụ thuộc burst credit, kém linh hoạt và đắt hơn gp3.\n✗ st1 là HDD throughput-optimized cho sequential, không phù hợp database transactional cần IOPS.\n✗ sc1 là HDD lạnh chi phí thấp cho dữ liệu ít truy cập, không đáp ứng hiệu năng database.","domain":3,"mock":2},
  {"id":"saa-m3-038","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một cơ sở dữ liệu giao dịch quan trọng yêu cầu một EBS volume duy nhất cung cấp tới hàng trăm nghìn IOPS, sub-millisecond latency, và durability rất cao với SLA 99.999%. Workload đang chạy trên instance Nitro thế hệ mới. Loại lưu trữ nào đáp ứng tốt nhất?","options":["EBS io2 Block Express","EBS io1 với multi-attach","Instance store NVMe SSD","EBS gp3 cấu hình IOPS tối đa"],"correctIndices":[0],"explanation":"io2 Block Express là EBS hiệu năng cao nhất, đạt tới 256.000 IOPS mỗi volume, sub-millisecond latency và durability 99.999% — đúng yêu cầu database giao dịch quan trọng.\n✓ io2 Block Express cung cấp IOPS rất cao (tới 256.000/volume), latency thấp và durability 99.999% trên instance Nitro.\n✗ io1 giới hạn 64.000 IOPS/volume, thấp hơn nhiều so với hàng trăm nghìn IOPS, và multi-attach không cần thiết khi chỉ cần một volume.\n✗ Instance store NVMe nhanh nhưng ephemeral, mất dữ liệu khi stop instance, không durable cho database quan trọng.\n✗ gp3 tối đa 80.000 IOPS/volume, không đủ cho yêu cầu hàng trăm nghìn IOPS.","domain":3,"mock":3},
  {"id":"saa-m1-040","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một ứng dụng cần scratch storage cho dữ liệu tạm trong quá trình xử lý: hiệu năng I/O cực cao, độ trễ thấp nhất, và chấp nhận mất dữ liệu khi instance dừng. Đồng thời cần kết quả cuối được lưu durable. Chọn HAI lựa chọn đúng cho kiến trúc này.","options":["Dùng instance store NVMe SSD làm vùng scratch tạm trong khi xử lý","Ghi kết quả cuối cùng ra Amazon S3 để lưu trữ durable","Dùng EBS sc1 làm vùng scratch để tiết kiệm chi phí","Lưu dữ liệu tạm trên Amazon S3 Glacier để truy xuất nhanh","Dùng EFS Infrequent Access cho dữ liệu scratch I/O cao"],"correctIndices":[0,1],"explanation":"Instance store cho I/O nhanh nhất và phù hợp dữ liệu ephemeral, còn S3 đảm bảo lưu trữ durable cho kết quả cuối.\n✓ Instance store NVMe cung cấp I/O cực cao, latency thấp, lý tưởng cho scratch chấp nhận mất khi instance dừng.\n✓ Amazon S3 lưu kết quả cuối với durability 11 số 9, đúng yêu cầu durable.\n✗ sc1 là HDD lạnh chi phí thấp nhưng hiệu năng rất thấp, không hợp scratch I/O cao.\n✗ S3 Glacier là lưu trữ lạnh truy xuất chậm, hoàn toàn sai cho dữ liệu tạm cần tốc độ.\n✗ EFS IA tối ưu chi phí dữ liệu ít truy cập, không phải scratch I/O cao.","domain":3,"mock":1},
  {"id":"saa-m2-040","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một cụm máy học cần đọc dataset training khổng lồ từ S3 liên tục cho nhiều epoch. Yêu cầu throughput đọc cực cao chia sẻ giữa hàng trăm GPU instance, và sau khi train xong kết quả phải đồng bộ ngược về S3. Workload chỉ chạy theo đợt rồi xóa. Giải pháp tối ưu nhất?","options":["Amazon FSx for Lustre với S3 data repository association, dùng scratch file system","Tải toàn bộ dataset xuống EBS gp3 trên mỗi GPU instance","Mount S3 trực tiếp qua một gateway và đọc object từng lần","Amazon EFS với Max I/O performance mode dùng chung cho cả cụm"],"correctIndices":[0],"explanation":"FSx for Lustre scratch tích hợp S3 cho throughput đọc cực cao chia sẻ, đồng bộ kết quả ngược S3, và chi phí thấp cho workload tạm thời.\n✓ FSx for Lustre scratch kèm S3 repository association cung cấp throughput cao, lazy-load dataset và export kết quả về S3, phù hợp ML chạy theo đợt.\n✗ Tải dataset về EBS mỗi instance tốn dung lượng, không chia sẻ và lặp lại tốn kém với hàng trăm GPU.\n✗ Đọc trực tiếp từng object qua gateway không đạt throughput tổng hợp cần cho training nhiều epoch.\n✗ EFS Max I/O có latency cao hơn và throughput không sánh được với Lustre cho HPC/ML quy mô lớn.","domain":3,"mock":2},
  {"id":"saa-m3-039","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một nhóm Linux developer cần một shared file system có thể tự động co giãn dung lượng, gắn đồng thời từ nhiều EC2 instance qua NFS trên nhiều Availability Zone, với least operational overhead. Dịch vụ nào phù hợp nhất?","options":["Amazon EFS","Amazon EBS Multi-Attach io2","Amazon FSx for Windows File Server","Amazon S3 mount như một ổ đĩa"],"correctIndices":[0],"explanation":"EFS là file system NFS managed, tự động co giãn và truy cập đồng thời từ nhiều instance trên nhiều AZ với vận hành tối thiểu.\n✓ EFS hỗ trợ NFS, multi-AZ, elastic capacity và truy cập đồng thời, least operational overhead.\n✗ EBS Multi-Attach io2 chỉ trong một AZ và yêu cầu cluster-aware file system, phức tạp hơn nhiều.\n✗ FSx for Windows dùng SMB cho Windows, không phù hợp NFS trên Linux.\n✗ S3 là object storage, không phải file system POSIX để mount tự nhiên cho dev workflow.","domain":3,"mock":3},
  {"id":"saa-m1-041","courseId":"SAA-C03","lesson":"ch2-02-storage-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một workload phân tích đọc tuần tự (sequential) khối lượng lớn dữ liệu log lưu trên một EBS volume — như big data, MapReduce và data warehouse — và cần throughput cao trên mỗi đô la chứ không cần IOPS ngẫu nhiên cao. Loại EBS volume nào cost-effective nhất?","options":["EBS st1 (Throughput Optimized HDD)","EBS io2 (Provisioned IOPS SSD)","EBS gp3 (General Purpose SSD)","EBS sc1 (Cold HDD)"],"correctIndices":[0],"explanation":"st1 là HDD tối ưu throughput cho truy cập sequential khối lượng lớn với chi phí thấp, đúng cho big data/log analytics.\n✓ st1 cung cấp throughput cao chi phí thấp cho workload sequential như MapReduce, data warehouse.\n✗ io2 là SSD IOPS cao đắt tiền, lãng phí cho workload sequential không cần random IOPS.\n✗ gp3 là SSD đa dụng, đắt hơn st1 cho khối lượng lớn dữ liệu throughput-bound.\n✗ sc1 là Cold HDD throughput thấp hơn, dành cho dữ liệu ít truy cập, không tối ưu cho workload analytics thường xuyên.","domain":3,"mock":1},
  {"id":"saa-m2-041","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một nhóm phát triển chạy workload Java backend trên EC2 với CPU sử dụng ổn định. Họ muốn giảm chi phí compute mà không phải viết lại ứng dụng phức tạp, và ứng dụng đã được biên dịch lại để hỗ trợ kiến trúc ARM. Lựa chọn nào giúp giảm chi phí và tăng hiệu năng/giá tốt nhất?","options":["Chuyển sang các EC2 instance dùng AWS Graviton (ví dụ họ m7g/c7g)","Chuyển sang các EC2 instance họ m5 với nhiều vCPU hơn","Bật EC2 Detailed Monitoring để tối ưu CPU","Chuyển sang EC2 instance họ x2idn tối ưu bộ nhớ"],"correctIndices":[0],"explanation":"Graviton (ARM) cho price/performance tốt hơn và ứng dụng đã hỗ trợ ARM nên chuyển sang là tối ưu.\n✓ Graviton instances (m7g/c7g) — đúng, ARM-based, hiệu năng/giá cao hơn, ứng dụng đã hỗ trợ ARM nên dễ chuyển.\n✗ m5 nhiều vCPU hơn — tăng chi phí mà không khai thác lợi thế giá của Graviton.\n✗ Detailed Monitoring — chỉ tăng độ chi tiết metric, không giảm chi phí compute.\n✗ x2idn tối ưu bộ nhớ — đắt và phù hợp workload nặng RAM, không phải mục tiêu price/performance ở đây.","domain":3,"mock":2},
  {"id":"saa-m3-040","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng web có lưu lượng dao động trong ngày. Đội vận hành muốn Auto Scaling group tự động giữ mức trung bình CPU utilization của các instance quanh 50% với cấu hình đơn giản, ít thao tác vận hành nhất. Chính sách scaling nào phù hợp nhất?","options":["Target tracking scaling policy với metric Average CPU Utilization = 50%","Simple scaling policy dựa trên CloudWatch alarm khi CPU > 80%","Step scaling policy với nhiều bậc điều chỉnh theo CPU","Scheduled scaling theo khung giờ cố định"],"correctIndices":[0],"explanation":"Target tracking tự điều chỉnh số instance để giữ metric quanh giá trị mục tiêu, ít cấu hình và ít vận hành nhất.\n✓ Target tracking với CPU = 50% — đúng, AWS tự tính số instance cần để giữ metric quanh mục tiêu, đơn giản nhất.\n✗ Simple scaling theo alarm — cần cooldown thủ công, phản ứng chậm và nhiều cấu hình hơn.\n✗ Step scaling nhiều bậc — linh hoạt nhưng phức tạp, cần định nghĩa từng bậc thủ công.\n✗ Scheduled scaling — chỉ tốt khi tải có lịch dự đoán trước cố định, không bám theo CPU thực tế.","domain":3,"mock":3},
  {"id":"saa-m1-042","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một sàn thương mại điện tử biết trước rằng lưu lượng tăng theo chu kỳ rõ rệt mỗi sáng và mỗi tối dựa trên dữ liệu lịch sử nhiều tuần. Họ thấy target tracking phản ứng hơi trễ khi tải tăng đột ngột đầu mỗi đợt, gây độ trễ tạm thời. Giải pháp nào giúp chuẩn bị capacity TRƯỚC khi nhu cầu tăng, dựa trên mẫu lịch sử?","options":["Bật predictive scaling cho Auto Scaling group","Giảm cooldown của simple scaling xuống mức tối thiểu","Tăng giá trị target của target tracking lên 80% CPU","Chuyển toàn bộ sang Spot Instances để có thêm capacity"],"correctIndices":[0],"explanation":"Predictive scaling dùng machine learning phân tích lịch sử để dự báo và cấp capacity trước khi nhu cầu đến, giải quyết độ trễ phản ứng.\n✓ Predictive scaling — đúng, dựa trên mẫu lịch sử dự báo và tăng capacity trước khi tải đến.\n✗ Giảm cooldown simple scaling — vẫn phản ứng sau khi tải đã tăng, không chủ động.\n✗ Tăng target lên 80% CPU — khiến hệ thống chịu tải cao hơn trước khi scale, làm trễ tệ hơn.\n✗ Chuyển sang Spot — về giá/sẵn capacity, không giải quyết việc chuẩn bị trước theo mẫu lịch sử.","domain":3,"mock":1},
  {"id":"saa-m2-042","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một API serverless chạy trên Lambda phục vụ người dùng tương tác. Đội phát triển nhận thấy một số request đầu tiên sau thời gian không hoạt động có độ trễ cao do cold start. Họ cần đảm bảo một số lượng môi trường thực thi luôn được khởi tạo sẵn để giảm cold start, với least operational overhead. Giải pháp nào phù hợp nhất?","options":["Cấu hình Provisioned Concurrency cho function/alias","Tăng memory của Lambda lên mức tối đa","Tăng Reserved Concurrency cho function","Đặt Lambda trong VPC để cải thiện thời gian khởi tạo"],"correctIndices":[0],"explanation":"Provisioned Concurrency giữ sẵn các execution environment đã khởi tạo, loại bỏ cold start cho lượng concurrency đã cấp.\n✓ Provisioned Concurrency — đúng, giữ môi trường sẵn sàng, loại bỏ cold start cho phần đã provision.\n✗ Tăng memory tối đa — có thể giảm nhẹ thời gian init nhưng không loại bỏ cold start và tốn chi phí.\n✗ Reserved Concurrency — chỉ giới hạn/đảm bảo số concurrency tối đa, không làm ấm sẵn môi trường.\n✗ Đặt trong VPC — thường làm init lâu hơn chứ không nhanh hơn.","domain":3,"mock":2},
  {"id":"saa-m3-041","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một workload HPC (mô phỏng tính toán) chạy trên một cụm nhiều EC2 instance cần băng thông cao và độ trễ network cực thấp GIỮA CÁC NODE trong cùng một AZ để trao đổi dữ liệu liên tục. Cấu hình nào tối ưu nhất cho hiệu năng giao tiếp giữa các node?","options":["Cluster placement group kết hợp Elastic Fabric Adapter (EFA)","Spread placement group để phân tán các node trên nhiều hardware","Partition placement group trên nhiều rack","Đặt các instance ở nhiều AZ và bật Enhanced Networking"],"correctIndices":[0],"explanation":"Cluster placement group gom các instance gần nhau để có băng thông cao và độ trễ thấp, còn EFA cho phép giao tiếp HPC/MPI hiệu năng cao bỏ qua OS kernel.\n✓ Cluster placement group + EFA — đúng, gom node cùng AZ với độ trễ thấp, EFA tối ưu giao tiếp inter-node cho HPC.\n✗ Spread placement group — phân tán phần cứng để giảm rủi ro cùng lỗi, không tối ưu độ trễ giữa node.\n✗ Partition placement group — cho dữ liệu phân tán lớn (Hadoop/Kafka), không tối ưu low-latency inter-node.\n✗ Nhiều AZ + Enhanced Networking — đặt ở nhiều AZ làm tăng độ trễ giữa các node.","domain":3,"mock":3},
  {"id":"saa-m1-043","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty container hóa các microservice và muốn chạy chúng mà KHÔNG phải quản lý hay vá EC2 instance bên dưới, trả tiền theo tài nguyên container sử dụng, với least operational overhead. Họ đã chuẩn hóa trên Amazon ECS. Compute option nào phù hợp nhất?","options":["AWS Fargate làm launch type cho ECS","ECS trên EC2 launch type với Auto Scaling group tự quản lý","Chạy container trực tiếp trên EC2 với Docker tự cài","AWS Batch trên EC2 managed compute environment"],"correctIndices":[0],"explanation":"Fargate là serverless compute cho container: không quản lý server, trả theo tài nguyên task, ít vận hành nhất.\n✓ Fargate launch type — đúng, không cần quản lý/vá EC2, trả theo vCPU/memory của task.\n✗ ECS trên EC2 tự quản lý — vẫn phải vá và scale EC2, nhiều vận hành hơn.\n✗ Docker tự cài trên EC2 — quản lý thủ công nhiều nhất.\n✗ AWS Batch trên EC2 — hợp cho batch job, vẫn dựa trên EC2 cần quản lý hơn Fargate.","domain":3,"mock":1},
  {"id":"saa-m2-043","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"multi","question":"Một kiến trúc sư đang tối ưu chi phí và hiệu năng cho nhiều workload khác nhau. Hãy chọn HAI phát biểu ĐÚNG về việc lựa chọn EC2 instance family và compute.","options":["Compute optimized (họ C) phù hợp workload nặng CPU như batch processing và high-performance web servers","Memory optimized (họ R/X) phù hợp database in-memory và xử lý dữ liệu lớn trong RAM","General purpose (họ M/T) chỉ nên dùng cho workload cần GPU","Storage optimized (họ I/D) tối ưu cho workload cần thông lượng network giữa các node HPC","Burstable (họ T) không bao giờ phù hợp cho môi trường production"],"correctIndices":[0,1],"explanation":"Mỗi instance family được thiết kế cho một loại tài nguyên chủ đạo; chọn đúng họ giúp tối ưu hiệu năng/giá.\n✓ Compute optimized (C) cho workload nặng CPU — đúng, lý tưởng cho batch và web server hiệu năng cao.\n✓ Memory optimized (R/X) cho in-memory DB — đúng, nhiều RAM cho dữ liệu lớn trong bộ nhớ.\n✗ General purpose chỉ cho GPU — sai, GPU thuộc accelerated computing (họ P/G), không phải M/T.\n✗ Storage optimized cho HPC network — sai, I/D tối ưu I/O đĩa cao, không phải network HPC.\n✗ Burstable không bao giờ dùng production — sai, họ T hợp lý cho workload tải thấp/biến động trong production.","domain":3,"mock":2},
  {"id":"saa-m3-042","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một function Lambda gọi đồng thời rất nhiều và đôi khi làm cạn kiệt account concurrency, gây throttling cho các function khác trong cùng account. Đội kiến trúc muốn GIỚI HẠN số concurrency tối đa mà function 'gây ồn' này có thể dùng, để bảo vệ các function còn lại. Giải pháp nào đúng?","options":["Đặt Reserved Concurrency cho function gây ồn","Đặt Provisioned Concurrency cho function gây ồn","Tăng account-level concurrency limit qua support","Bật DLQ (dead-letter queue) cho function gây ồn"],"correctIndices":[0],"explanation":"Reserved Concurrency vừa đảm bảo vừa GIỚI HẠN concurrency tối đa của một function, ngăn nó chiếm hết pool chung của account.\n✓ Reserved Concurrency — đúng, đặt trần concurrency cho function đó nên không làm cạn pool chung.\n✗ Provisioned Concurrency — chỉ làm ấm môi trường sẵn, không đặt trần dùng tài nguyên.\n✗ Tăng account limit — chỉ tăng tổng quota, function ồn vẫn có thể chiếm hết.\n✗ DLQ — xử lý event thất bại, không liên quan giới hạn concurrency.","domain":3,"mock":3},
  {"id":"saa-m1-044","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng phân tích chạy theo lô lớn, có thể chịu được việc instance bị gián đoạn và tự khởi động lại task. Đội muốn giảm chi phí compute tối đa cho phần workload không khẩn cấp này. Phương án cost-effective nhất cho phần workload chịu gián đoạn?","options":["Dùng Spot Instances cho phần workload chịu gián đoạn","Dùng On-Demand Instances để đảm bảo không gián đoạn","Mua Reserved Instances 3 năm cho toàn bộ workload","Dùng Dedicated Hosts cho phần workload này"],"correctIndices":[0],"explanation":"Spot Instances rẻ nhất (tới ~90%) và phù hợp workload chịu được gián đoạn như batch/analytics có thể retry.\n✓ Spot Instances — đúng, rẻ nhất cho workload fault-tolerant chịu được interruption.\n✗ On-Demand — không gián đoạn nhưng đắt hơn nhiều, lãng phí cho workload chịu được gián đoạn.\n✗ RI 3 năm cho toàn bộ — cam kết dài hạn không hợp với phần tải biến động/không khẩn.\n✗ Dedicated Hosts — đắt nhất, dùng cho yêu cầu license/compliance, không tối ưu chi phí.","domain":3,"mock":1},
  {"id":"saa-m2-044","courseId":"SAA-C03","lesson":"ch2-01-compute-performance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một đội muốn triển khai ứng dụng web đơn giản và chỉ cần upload mã nguồn, để AWS tự lo provisioning EC2, load balancer, Auto Scaling và health monitoring, với least operational overhead nhưng vẫn giữ quyền truy cập vào tài nguyên bên dưới khi cần. Dịch vụ nào phù hợp nhất?","options":["AWS Elastic Beanstalk","Tự dựng EC2 với Auto Scaling group và ALB thủ công","AWS Lambda với API Gateway","Amazon Lightsail với blueprint thủ công"],"correctIndices":[0],"explanation":"Elastic Beanstalk tự lo provisioning, load balancing, scaling và monitoring từ mã nguồn upload lên, nhưng vẫn cho truy cập tài nguyên EC2/ASG bên dưới.\n✓ Elastic Beanstalk — đúng, chỉ cần upload code, AWS tự dựng và quản lý nền tảng, vẫn truy cập được tài nguyên.\n✗ Tự dựng EC2/ASG/ALB — nhiều thao tác vận hành nhất.\n✗ Lambda + API Gateway — serverless nhưng đòi tái cấu trúc ứng dụng theo function, không phải web app truyền thống.\n✗ Lightsail — đơn giản nhưng hạn chế tích hợp và ít kiểm soát tài nguyên AWS bên dưới hơn.","domain":3,"mock":2},
  {"id":"saa-m3-043","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một ứng dụng thương mại điện tử chạy trên Amazon RDS for MySQL (Single-AZ). Trang dashboard báo cáo và các truy vấn analytics đọc nhiều dữ liệu đang làm CPU của database tăng cao, ảnh hưởng đến các giao dịch ghi của khách hàng. Lượng ghi vẫn ổn định, chỉ có lượng đọc tăng đột biến. Giải pháp nào giúp giảm tải đọc khỏi instance chính với LEAST operational overhead?","options":["Tạo một hoặc nhiều RDS Read Replicas và định tuyến các truy vấn báo cáo/analytics chỉ-đọc sang đó","Bật Multi-AZ deployment để standby instance phục vụ các truy vấn đọc","Tăng kích thước instance lên loại lớn hơn để xử lý cả đọc và ghi","Chuyển toàn bộ database sang Amazon Redshift để xử lý truy vấn báo cáo"],"correctIndices":[0],"explanation":"Read Replicas tách tải đọc khỏi primary, lý tưởng cho báo cáo chỉ-đọc mà không ảnh hưởng giao dịch ghi.\n✓ Read Replicas phục vụ truy vấn đọc song song, giảm CPU primary, đúng use case offload read.\n✗ Standby của Multi-AZ KHÔNG phục vụ đọc — chỉ dùng cho failover, không giảm tải.\n✗ Tăng kích thước instance tốn kém hơn và vẫn dồn đọc/ghi vào một instance.\n✗ Migrate sang Redshift là thay đổi lớn, OLAP riêng, operational overhead cao cho nhu cầu chỉ offload read.","domain":3,"mock":3},
  {"id":"saa-m1-045","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một nền tảng gaming dùng DynamoDB lưu bảng xếp hạng (leaderboard) với hàng triệu lượt đọc mỗi giây trong giờ cao điểm. Hồ sơ người chơi được đọc lặp lại liên tục và yêu cầu độ trễ ở mức microsecond. Hiện tại độ trễ single-digit millisecond của DynamoDB chưa đủ nhanh. Giải pháp nào đáp ứng yêu cầu với ít thay đổi ứng dụng nhất?","options":["Thêm DynamoDB Accelerator (DAX) làm in-memory cache trước DynamoDB","Đặt Amazon ElastiCache for Redis trước DynamoDB và tự quản lý logic cache","Bật DynamoDB Global Tables để giảm độ trễ đọc","Chuyển bảng sang chế độ provisioned capacity với auto scaling"],"correctIndices":[0],"explanation":"DAX là in-memory cache được thiết kế riêng cho DynamoDB, cung cấp độ trễ microsecond với API tương thích nên gần như không phải sửa code.\n✓ DAX cho độ trễ microsecond cho read-heavy, tích hợp gốc với DynamoDB, ít thay đổi ứng dụng.\n✗ ElastiCache for Redis cũng cache được nhưng phải tự viết logic cache-aside, operational overhead cao hơn DAX.\n✗ Global Tables giải quyết replication đa region, không giảm độ trễ đọc xuống microsecond.\n✗ Provisioned capacity điều chỉnh throughput chứ không hạ độ trễ xuống mức microsecond.","domain":3,"mock":1},
  {"id":"saa-m2-045","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng serverless dùng AWS Lambda kết nối tới Amazon RDS for PostgreSQL. Khi lưu lượng tăng đột biến, hàng nghìn Lambda concurrent mở quá nhiều kết nối database khiến RDS cạn connection và lỗi 'too many connections'. Giải pháp nào giải quyết vấn đề với LEAST operational overhead?","options":["Triển khai Amazon RDS Proxy để pool và chia sẻ các kết nối database","Tăng tham số max_connections trên RDS và scale up instance","Tự xây dựng connection pooler chạy trên EC2 trước RDS","Thêm Read Replica để phân tán kết nối qua nhiều endpoint"],"correctIndices":[0],"explanation":"RDS Proxy là managed connection pooler được thiết kế riêng cho workload serverless/Lambda với nhiều kết nối ngắn.\n✓ RDS Proxy pool và tái sử dụng kết nối, hấp thụ burst của Lambda, fully managed nên ít vận hành.\n✗ Tăng max_connections và scale up chỉ trì hoãn vấn đề, mỗi kết nối vẫn tốn bộ nhớ và không bền vững.\n✗ Tự xây pooler trên EC2 thêm hạ tầng phải quản lý, vá lỗi, HA — operational overhead lớn.\n✗ Read Replica giúp offload đọc nhưng không giải quyết cạn kiệt kết nối, ghi vẫn dồn vào primary.","domain":3,"mock":2},
  {"id":"saa-m3-044","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty cần lưu trữ và truy vấn dữ liệu quan hệ với throughput đọc cao, khả năng tự động phục hồi sau lỗi, và muốn database tự scale storage. Họ cần tương thích MySQL nhưng yêu cầu hiệu năng cao hơn RDS for MySQL tiêu chuẩn và khả năng có tới 15 read replica với độ trễ replication thấp. Lựa chọn nào phù hợp nhất?","options":["Amazon Aurora MySQL-Compatible Edition","Amazon RDS for MySQL với Multi-AZ và 5 read replica","Amazon DynamoDB với MySQL compatibility layer","Amazon Redshift với storage tự động mở rộng"],"correctIndices":[0],"explanation":"Aurora cung cấp throughput cao gấp nhiều lần RDS MySQL, tới 15 read replica với độ trễ thấp, storage tự scale và self-healing.\n✓ Aurora MySQL hỗ trợ tới 15 low-latency read replicas, storage tự mở rộng, tương thích MySQL — khớp mọi yêu cầu.\n✗ RDS for MySQL chỉ tối đa 5 read replicas và độ trễ replication cao hơn Aurora.\n✗ DynamoDB là NoSQL key-value, không phải database quan hệ tương thích MySQL.\n✗ Redshift là kho dữ liệu OLAP, không phù hợp workload giao dịch quan hệ throughput cao.","domain":3,"mock":3},
  {"id":"saa-m1-046","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ứng dụng web dùng DynamoDB với primary key là CustomerId. Sản phẩm cần truy vấn đơn hàng theo OrderStatus và theo OrderDate, đây là các thuộc tính không phải key. Hiện tại ứng dụng đang Scan toàn bảng rồi lọc, gây tốn kém và chậm. Cách tối ưu nhất để hỗ trợ các truy vấn này?","options":["Tạo Global Secondary Index (GSI) với OrderStatus và OrderDate làm key để Query trực tiếp","Tạo Local Secondary Index (LSI) trên OrderStatus và OrderDate","Bật DynamoDB Streams và xây bảng phụ được denormalize theo OrderStatus","Tăng provisioned read capacity để các thao tác Scan chạy nhanh hơn"],"correctIndices":[0],"explanation":"GSI cho phép truy vấn theo các thuộc tính không phải primary key gốc với partition/sort key khác, thay thế Scan tốn kém bằng Query hiệu quả.\n✓ GSI tạo index với OrderStatus/OrderDate làm key, cho phép Query thay vì Scan toàn bảng — nhanh và rẻ hơn nhiều.\n✗ LSI phải dùng chung partition key với bảng gốc (CustomerId) và chỉ tạo được lúc tạo bảng — không hỗ trợ truy vấn theo OrderStatus độc lập.\n✗ DynamoDB Streams + bảng phụ giải quyết được nhưng phức tạp, tốn công maintain so với GSI native.\n✗ Tăng read capacity chỉ làm Scan tốn kém hơn về chi phí, không giải quyết bản chất kém hiệu quả của Scan.","domain":3,"mock":1},
  {"id":"saa-m2-046","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng cần cache session của người dùng. Yêu cầu: dữ liệu session phải tồn tại qua các lần khởi động lại node (persistence), hỗ trợ replication để high availability, và có thể dùng các cấu trúc dữ liệu phong phú như sorted set cho leaderboard. Engine ElastiCache nào phù hợp?","options":["Amazon ElastiCache for Redis","Amazon ElastiCache for Memcached","Amazon DynamoDB với TTL","Amazon RDS for PostgreSQL với in-memory tablespace"],"correctIndices":[0],"explanation":"Redis hỗ trợ persistence, replication/Multi-AZ với automatic failover và các kiểu dữ liệu nâng cao như sorted set — Memcached thì không.\n✓ ElastiCache for Redis có persistence, replication HA và cấu trúc dữ liệu phong phú (sorted set, hash) — đáp ứng mọi yêu cầu.\n✗ Memcached là cache đơn giản multi-threaded, KHÔNG có persistence, replication hay cấu trúc dữ liệu nâng cao.\n✗ DynamoDB với TTL lưu được session nhưng không phải in-memory cache, độ trễ cao hơn và không có sorted set native.\n✗ RDS PostgreSQL không phải caching layer in-memory, không phù hợp cho session caching tốc độ cao.","domain":3,"mock":2},
  {"id":"saa-m3-045","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một mạng xã hội cần lưu mối quan hệ giữa người dùng (bạn bè, theo dõi) và chạy các truy vấn duyệt nhiều bậc như 'bạn của bạn' hay đường nối ngắn nhất giữa hai người. Các truy vấn này liên kết sâu nhiều cấp. Database nào được tối ưu cho workload này?","options":["Amazon Neptune","Amazon Aurora PostgreSQL với nhiều bảng join","Amazon DynamoDB với adjacency list pattern","Amazon Timestream"],"correctIndices":[0],"explanation":"Neptune là graph database được tối ưu riêng cho dữ liệu quan hệ liên kết cao và các truy vấn duyệt đồ thị nhiều bậc.\n✓ Neptune là managed graph database, xử lý hiệu quả truy vấn 'bạn của bạn' và đường đi ngắn nhất bằng Gremlin/SPARQL.\n✗ Aurora PostgreSQL có thể join nhưng truy vấn duyệt nhiều bậc đệ quy trở nên rất chậm và phức tạp khi độ sâu tăng.\n✗ DynamoDB adjacency list mô phỏng được graph nhưng truy vấn duyệt nhiều bậc rất khó và kém hiệu quả.\n✗ Timestream là time-series database, không phù hợp với dữ liệu quan hệ đồ thị.","domain":3,"mock":3},
  {"id":"saa-m1-047","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một database Aurora MySQL phục vụ ứng dụng read-heavy với lưu lượng đọc dao động mạnh theo thời gian trong ngày. Kiến trúc sư muốn vừa tự động scale dung lượng đọc theo nhu cầu vừa đảm bảo ứng dụng luôn kết nối tới một endpoint ổn định cho các truy vấn đọc. Nên áp dụng những giải pháp nào? (Chọn 2)","options":["Bật Aurora Auto Scaling cho Aurora Replicas dựa trên CPU hoặc số kết nối","Cấu hình ứng dụng dùng Aurora reader endpoint để cân bằng tải đọc qua các replica","Định tuyến mọi truy vấn đọc trực tiếp tới Aurora writer endpoint","Tạo thủ công số Aurora Replica cố định tối đa và để chạy 24/7","Dùng một instance endpoint duy nhất của một replica cụ thể cho tất cả truy vấn đọc"],"correctIndices":[0,1],"explanation":"Aurora Auto Scaling tự thêm/bớt replica theo tải, còn reader endpoint cung cấp một điểm kết nối ổn định tự cân bằng qua các replica đang có.\n✓ Aurora Auto Scaling tự điều chỉnh số replica theo nhu cầu đọc dao động — cost-effective và đàn hồi.\n✓ Reader endpoint là endpoint ổn định, tự load balance qua tất cả replica kể cả khi auto scaling thêm/bớt node.\n✗ Writer endpoint chỉ trỏ tới primary, dồn tải đọc vào instance ghi — phản tác dụng offload.\n✗ Chạy số replica tối đa cố định 24/7 lãng phí chi phí trong giờ thấp điểm.\n✗ Instance endpoint của một replica cụ thể không cân bằng tải và mất kết nối khi replica đó bị thay thế.","domain":3,"mock":1},
  {"id":"saa-m2-047","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một startup ra mắt ứng dụng mới và không thể dự đoán traffic; có thể tăng vọt bất ngờ rồi giảm về gần 0. Họ muốn dùng DynamoDB nhưng lo lắng về việc throttling khi chưa provisioned đủ capacity và lãng phí khi provisioned quá mức. Giải pháp nào phù hợp nhất mà LEAST operational overhead?","options":["Dùng DynamoDB on-demand capacity mode","Dùng provisioned capacity với auto scaling cấu hình thủ công","Provisioned capacity cao cố định để tránh throttling","Dùng provisioned capacity thấp kết hợp DynamoDB Accelerator (DAX)"],"correctIndices":[0],"explanation":"On-demand mode tự động phục vụ throughput theo lưu lượng thực tế, không cần dự đoán capacity, lý tưởng cho traffic không đoán trước.\n✓ On-demand tự scale tức thời theo traffic, chỉ trả tiền theo request thực dùng — không cần planning, ít vận hành nhất.\n✗ Provisioned + auto scaling cần cấu hình ngưỡng và có độ trễ scaling, không lý tưởng cho burst đột ngột không đoán trước.\n✗ Provisioned cao cố định lãng phí lớn khi traffic giảm về 0.\n✗ DAX là caching layer giảm độ trễ đọc, không giải quyết vấn đề capacity planning cho ghi/burst.","domain":3,"mock":2},
  {"id":"saa-m3-046","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một database RDS for MySQL có một truy vấn báo cáo nặng chạy mỗi đêm trên Read Replica. Nhóm phân tích phàn nàn rằng đôi khi dữ liệu trên replica trễ vài giây so với primary. Để hiểu và xử lý đúng, kiến trúc sư nên nhận định điều gì về RDS Read Replicas?","options":["Read Replicas dùng asynchronous replication nên có thể có replica lag; phù hợp khi ứng dụng chấp nhận eventual consistency cho đọc","Read Replicas dùng synchronous replication nên dữ liệu luôn nhất quán tuyệt đối với primary","Replica lag chỉ xảy ra khi cấu hình sai và có thể loại bỏ hoàn toàn bằng cách bật Multi-AZ","Read Replicas đảm bảo strong consistency giống như đọc từ primary endpoint"],"correctIndices":[0],"explanation":"RDS Read Replicas dùng asynchronous replication, nên replica lag là hành vi bình thường và phù hợp cho workload đọc chấp nhận eventual consistency.\n✓ Asynchronous replication vốn dĩ có thể gây lag — đúng bản chất, phù hợp báo cáo chấp nhận eventual consistency.\n✗ Read Replicas KHÔNG dùng synchronous replication; chỉ Multi-AZ standby mới synchronous.\n✗ Replica lag là vốn có của async replication, không thể loại bỏ hoàn toàn; Multi-AZ phục vụ HA chứ không loại bỏ lag của read replica.\n✗ Read Replicas cho eventual consistency, không đảm bảo strong consistency như primary.","domain":3,"mock":3},
  {"id":"saa-m1-048","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty host static website (HTML, CSS, JS, ảnh) trên Amazon S3, phục vụ user toàn cầu. User ở châu Á và châu Âu phàn nàn trang load chậm vì bucket nằm ở us-east-1. Giải pháp nào cải thiện latency hiệu quả nhất với LEAST operational overhead?","options":["Tạo Amazon CloudFront distribution với S3 bucket làm origin, dùng OAC để bảo vệ origin","Bật S3 Cross-Region Replication sang nhiều region và để user tự chọn region gần nhất","Đặt Global Accelerator phía trước S3 bucket để định tuyến qua AWS backbone","Chuyển sang Route 53 latency-based routing trỏ về cùng một S3 bucket ở us-east-1"],"correctIndices":[0],"explanation":"CloudFront cache static content tại hàng trăm edge location gần user, giảm latency mà không cần vận hành nhiều bản sao.\n✓ CloudFront + S3 origin: cache static tại edge, OAC bảo mật origin, fully managed, ít overhead nhất.\n✗ Cross-Region Replication nhiều region: tốn storage, phức tạp, và user phải tự chọn region — không tự động tối ưu.\n✗ Global Accelerator: hoạt động ở L4 cho TCP/UDP và KHÔNG cache; static content nên dùng CDN.\n✗ Route 53 latency-based về cùng 1 bucket us-east-1: vẫn chỉ 1 origin ở US, không giảm khoảng cách vật lý.","domain":3,"mock":1},
  {"id":"saa-m2-048","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một nền tảng game multiplayer chạy trên EC2 fleet sau NLB, dùng giao thức UDP, triển khai ở hai region (us-east-1 và ap-southeast-1). Yêu cầu: client cần một địa chỉ IP cố định để whitelist, latency thấp ổn định, và failover giữa region trong vòng dưới một giây khi một region down. Giải pháp nào phù hợp nhất?","options":["AWS Global Accelerator với hai static anycast IP, endpoint trỏ tới NLB ở mỗi region","Amazon CloudFront với NLB làm origin và Origin Failover group giữa hai region","Route 53 failover routing với TTL 30 giây trỏ tới NLB của từng region","Route 53 latency-based routing kết hợp health check trên hai NLB"],"correctIndices":[0],"explanation":"Global Accelerator cung cấp static anycast IP, định tuyến UDP qua AWS backbone với latency thấp và failover dưới một giây — đúng mọi yêu cầu.\n✓ Global Accelerator: 2 static IP để whitelist, hỗ trợ UDP (L4), failover region dưới 1s không phụ thuộc DNS.\n✗ CloudFront: là CDN L7 HTTP/HTTPS, không hỗ trợ UDP game traffic và không cho static IP.\n✗ Route 53 failover TTL 30s: phụ thuộc DNS cache của client/resolver, failover thường chậm hơn nhiều giây, không cho static IP.\n✗ Route 53 latency-based: vẫn là DNS, không đảm bảo failover dưới 1s và không cấp static IP cho whitelist.","domain":3,"mock":2},
  {"id":"saa-m3-047","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"EC2 instance trong private subnet đang tải về và đẩy lên hàng terabyte dữ liệu mỗi tháng từ/đến Amazon S3 trong cùng region. Hóa đơn NAT Gateway tăng vọt do data processing charge. Giải pháp nào giảm chi phí này một cách cost-effective nhất mà không ảnh hưởng kết nối tới các dịch vụ khác?","options":["Tạo S3 Gateway VPC endpoint và thêm route tới endpoint trong route table của private subnet","Tạo Interface VPC endpoint (PrivateLink) cho S3 ở mỗi AZ","Di chuyển EC2 instance ra public subnet và gán Elastic IP","Thay NAT Gateway bằng NAT instance EC2 tự quản lý loại nhỏ hơn"],"correctIndices":[0],"explanation":"S3 Gateway endpoint miễn phí và định tuyến traffic S3 qua mạng AWS private, loại bỏ hoàn toàn data processing charge của NAT cho phần traffic này.\n✓ S3 Gateway endpoint: free, traffic S3 không đi qua NAT nữa, vẫn giữ NAT cho các dịch vụ khác — cost-effective nhất.\n✗ Interface endpoint cho S3: tính phí theo giờ per-AZ cộng phí data; Gateway endpoint cho S3 thì miễn phí nên rẻ hơn.\n✗ Đưa EC2 ra public subnet: phá vỡ mô hình bảo mật private và vẫn tính internet egress.\n✗ NAT instance tự quản lý: vẫn tốn data transfer, thêm operational overhead, không giải quyết gốc.","domain":3,"mock":3},
  {"id":"saa-m1-049","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một tổ chức có 12 VPC trên nhiều account, cần kết nối mạng full-mesh để các VPC có thể giao tiếp với nhau và với mạng on-premises. Họ muốn tránh sự bùng nổ số lượng peering connection và quản lý routing tập trung. Giải pháp nào tốt nhất?","options":["AWS Transit Gateway làm hub trung tâm, chia sẻ qua Resource Access Manager (RAM)","VPC peering full-mesh giữa tất cả 12 VPC","AWS PrivateLink expose từng VPC như một endpoint service riêng","Một VPN connection riêng giữa từng cặp VPC qua Site-to-Site VPN"],"correctIndices":[0],"explanation":"Transit Gateway là hub-and-spoke hỗ trợ transitive routing, scale tuyến tính thay vì bậc hai, và quản lý routing tập trung.\n✓ Transit Gateway + RAM: hub trung tâm, transitive routing, kết nối luôn on-prem, chia sẻ cross-account dễ dàng.\n✗ VPC peering full-mesh: 12 VPC cần 66 peering, không transitive, quản lý route bùng nổ — không scale.\n✗ PrivateLink: chỉ expose một service cụ thể một chiều, không phải kết nối mạng full-mesh hai chiều.\n✗ VPN giữa từng cặp VPC: cực kỳ phức tạp, tốn kém, không phải pattern kết nối nội bộ VPC.","domain":3,"mock":1},
  {"id":"saa-m2-049","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một doanh nghiệp tài chính cần kết nối data center on-premises với AWS để truyền dữ liệu liên tục 2 Gbps với latency thấp và ổn định, và yêu cầu compliance không cho phép traffic đi qua public internet. Giải pháp nào đáp ứng tốt nhất?","options":["AWS Direct Connect với dedicated connection 10 Gbps","Site-to-Site VPN với hai tunnel để tăng throughput tổng","Site-to-Site VPN qua AWS Global Accelerator để tối ưu route","Nhiều Site-to-Site VPN connection gộp lại bằng ECMP để đạt 2 Gbps"],"correctIndices":[0],"explanation":"Direct Connect cung cấp kết nối riêng với throughput, latency ổn định và không đi qua internet — đáp ứng cả hiệu năng lẫn compliance.\n✓ Direct Connect 10 Gbps: throughput trên 2 Gbps consistent, latency predictable, traffic không qua public internet, thỏa compliance.\n✗ VPN hai tunnel: mỗi tunnel khoảng 1.25 Gbps và đi qua public internet, không thỏa yêu cầu compliance.\n✗ VPN qua Global Accelerator: vẫn dựa trên internet ở chặng cuối tới on-prem, không thỏa yêu cầu no internet.\n✗ Nhiều VPN + ECMP: phức tạp, jitter cao, và vẫn đi qua public internet.","domain":3,"mock":2},
  {"id":"saa-m3-048","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ứng dụng HPC chạy MPI giữa hàng trăm EC2 instance đòi hỏi inter-node communication latency cực thấp và network throughput cao trong cùng một cluster. Kết hợp nào tối ưu nhất cho yêu cầu network performance này?","options":["Cluster placement group kết hợp Elastic Fabric Adapter (EFA) trên các instance hỗ trợ","Spread placement group kết hợp Elastic Network Adapter (ENA) tiêu chuẩn","Partition placement group trải trên nhiều AZ để tăng độ bền","Cluster placement group kết hợp jumbo frames qua NAT Gateway"],"correctIndices":[0],"explanation":"Cluster placement group đặt instance gần nhau trong cùng AZ cho latency thấp nhất, còn EFA cho phép OS-bypass tối ưu MPI/HPC.\n✓ Cluster placement group + EFA: low latency intra-cluster, high throughput, EFA tối ưu cho MPI HPC — đúng nhất.\n✗ Spread placement group + ENA: spread đặt instance tách xa nhau để giảm tương quan lỗi, ngược mục tiêu low latency.\n✗ Partition group trải nhiều AZ: cross-AZ latency cao hơn, tối ưu cho big data/HA chứ không cho HPC latency thấp.\n✗ Jumbo frames qua NAT Gateway: jumbo frames không hoạt động ra internet/NAT, và NAT không liên quan inter-node MPI.","domain":3,"mock":3},
  {"id":"saa-m1-050","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một ứng dụng web toàn cầu dùng CloudFront trước ALB. Đội vận hành thấy cache hit ratio chỉ 35% và muốn tăng tỉ lệ cache hit cũng như giảm tải lên origin. Những hành động nào sẽ giúp đạt mục tiêu này? (Chọn 2)","options":["Cấu hình cache policy để KHÔNG đưa các cookie và query string không cần thiết vào cache key","Bật Origin Shield để thêm một lớp cache tập trung trước origin","Giảm TTL của tất cả object xuống 0 để luôn lấy bản mới nhất","Chuyển CloudFront price class sang chỉ dùng các edge location đắt nhất","Tắt compression Gzip/Brotli để giảm thời gian xử lý tại edge"],"correctIndices":[0,1],"explanation":"Tăng cache hit ratio bằng cách giảm số biến thể cache key và thêm lớp cache tập trung gom request tới origin.\n✓ Không đưa cookie/query string thừa vào cache key: giảm số biến thể cache key, nhiều request trúng cùng object nên hit ratio tăng.\n✓ Origin Shield: lớp cache trung tâm gom request từ nhiều POP, giảm origin request đáng kể và tăng hiệu quả cache.\n✗ Giảm TTL về 0: ép mọi request đi tới origin, hit ratio giảm và tải origin tăng — ngược mục tiêu.\n✗ Đổi price class sang edge đắt nhất: chỉ ảnh hưởng vùng phục vụ/chi phí, không cải thiện hit ratio.\n✗ Tắt compression: không liên quan cache hit và còn làm payload lớn hơn, chậm hơn.","domain":3,"mock":1},
  {"id":"saa-m2-050","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một SaaS provider muốn cho phép hàng trăm khách hàng (mỗi khách hàng một VPC ở account riêng) truy cập an toàn vào một dịch vụ nội bộ chạy sau NLB trong VPC của provider. Yêu cầu: traffic không đi qua internet, khách hàng không cần biết hay route tới IP/CIDR của provider, và không xảy ra xung đột CIDR. Giải pháp nào phù hợp nhất?","options":["AWS PrivateLink: tạo endpoint service trên NLB của provider, khách hàng tạo interface endpoint trong VPC của họ","VPC peering giữa VPC của provider và từng VPC khách hàng","Transit Gateway kết nối tất cả VPC khách hàng với VPC của provider","Đặt dịch vụ sau ALB public và bảo vệ bằng security group theo IP khách hàng"],"correctIndices":[0],"explanation":"PrivateLink expose dịch vụ một chiều qua interface endpoint, không cần route hay biết IP của provider và tránh xung đột CIDR.\n✓ PrivateLink endpoint service + interface endpoint: traffic private qua ENI, khách hàng không cần route/IP provider, không lo CIDR overlap — đúng cho SaaS multi-tenant.\n✗ VPC peering: cần route tới CIDR của nhau, không transitive, dễ xung đột CIDR với hàng trăm khách hàng.\n✗ Transit Gateway: kết nối mạng đầy đủ hai chiều, lộ topology provider và vẫn có nguy cơ CIDR overlap.\n✗ ALB public + security group: traffic đi qua internet, vi phạm yêu cầu private.","domain":3,"mock":2},
  {"id":"saa-m3-049","courseId":"SAA-C03","lesson":"ch2-04-network-performance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty chạy hai bản sao của ứng dụng web ở us-east-1 và eu-west-1, mỗi bản sau một ALB riêng. Họ muốn user được tự động định tuyến tới region có độ trễ mạng thấp nhất từ vị trí của họ. Routing policy nào của Route 53 phù hợp nhất?","options":["Latency-based routing với health check trên mỗi ALB","Weighted routing chia 50/50 giữa hai region","Geolocation routing theo quốc gia của user","Simple routing trỏ tới cả hai ALB cùng lúc"],"correctIndices":[0],"explanation":"Latency-based routing định tuyến user tới region cho latency thấp nhất từ resolver của họ, kèm health check để bỏ qua region lỗi.\n✓ Latency-based routing: chọn region latency thấp nhất tự động, health check loại region down — đúng mục tiêu.\n✗ Weighted 50/50: chia theo tỉ lệ cố định, không tối ưu theo latency địa lý.\n✗ Geolocation: định tuyến theo ranh giới địa lý cố định, không phản ánh latency thực tế và dễ bỏ sót vùng chưa map.\n✗ Simple routing nhiều giá trị: trả về ngẫu nhiên không xét latency và không health check.","domain":3,"mock":3},
  {"id":"saa-m1-051","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty thu thập clickstream từ website thương mại điện tử và cần đẩy dữ liệu này vào một S3 data lake để phân tích batch hàng ngày. Họ không cần xử lý real-time, không muốn quản lý cluster, và muốn giải pháp có least operational overhead để load dữ liệu vào S3 với khả năng tự động buffer, nén và partition theo thời gian. Giải pháp nào phù hợp nhất?","options":["Amazon Data Firehose ghi trực tiếp vào S3 với buffering, nén (GZIP/Snappy) và dynamic partitioning theo thời gian","Amazon Kinesis Data Streams kết hợp consumer application chạy trên EC2 tự ghi dữ liệu vào S3","Amazon MSK với một custom Kafka Connect S3 sink connector tự vận hành","Amazon EMR cluster đọc dữ liệu từ một SQS queue rồi ghi xuống S3 theo lịch"],"correctIndices":[0],"explanation":"Firehose là dịch vụ fully managed để load streaming data vào S3 với buffering, nén và partition, đúng nhu cầu ingestion với least operational overhead.\n✓ Amazon Data Firehose fully managed, tự buffer/nén/partition và ghi vào S3, không cần quản lý hạ tầng.\n✗ Kinesis Data Streams cần tự viết và vận hành consumer trên EC2, tăng operational overhead.\n✗ MSK + Kafka Connect tự vận hành đòi hỏi quản lý cluster và connector, quá nặng cho nhu cầu này.\n✗ EMR cluster cho batch ETL là thừa thãi và tốn kém cho việc chỉ load dữ liệu vào S3.","domain":3,"mock":1},
  {"id":"saa-m2-051","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một nền tảng IoT nhận hàng triệu sự kiện mỗi giây từ cảm biến. Đội phân tích cần xử lý các sự kiện này theo thứ tự cho từng thiết bị (ordered per device), với độ trễ dưới một giây, và đồng thời cho phép nhiều consumer application khác nhau (real-time dashboard, anomaly detection) đọc lại cùng một luồng dữ liệu trong cửa sổ tối đa 7 ngày. Dịch vụ nào đáp ứng tốt nhất?","options":["Amazon Kinesis Data Streams với partition key theo deviceId, bật extended retention và Enhanced Fan-Out cho nhiều consumer","Amazon Data Firehose ghi vào S3 rồi các consumer query qua Athena","Amazon SQS Standard queue với nhiều consumer poll song song","Amazon SNS fan-out đến nhiều SQS Standard queue cho từng consumer application"],"correctIndices":[0],"explanation":"Kinesis Data Streams đảm bảo ordering theo shard (partition key), retention tới 7+ ngày và Enhanced Fan-Out cho nhiều consumer real-time với độ trễ thấp.\n✓ Kinesis Data Streams giữ thứ tự per-device qua partition key, hỗ trợ replay và nhiều consumer real-time với Enhanced Fan-Out.\n✗ Firehose chỉ ghi batch vào S3, không phải real-time sub-second và không hỗ trợ replay luồng theo thứ tự.\n✗ SQS Standard không đảm bảo ordering và không cho phép nhiều consumer độc lập đọc lại cùng message.\n✗ SNS+SQS Standard không giữ ordering nghiêm ngặt per device và không hỗ trợ replay dữ liệu lịch sử.","domain":3,"mock":2},
  {"id":"saa-m3-050","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty lưu hàng terabyte log JSON thô trong S3. Đội data muốn chạy các truy vấn SQL ad-hoc không thường xuyên trực tiếp trên dữ liệu này mà không cần load vào database hay vận hành cluster, và chỉ trả tiền theo dung lượng quét. Giải pháp nào tối ưu nhất về cost-effective và least operational overhead?","options":["Amazon Athena truy vấn trực tiếp dữ liệu S3, dùng AWS Glue Data Catalog làm schema","Amazon Redshift provisioned cluster với COPY toàn bộ dữ liệu từ S3 vào rồi truy vấn","Amazon EMR với Hive chạy liên tục để phục vụ truy vấn ad-hoc","Amazon RDS for PostgreSQL với dữ liệu được import định kỳ từ S3"],"correctIndices":[0],"explanation":"Athena là serverless query engine query trực tiếp trên S3, tính phí theo dữ liệu quét, lý tưởng cho truy vấn ad-hoc không thường xuyên.\n✓ Athena serverless, không cần cluster, trả tiền theo lượng quét, dùng Glue Data Catalog cho schema.\n✗ Redshift provisioned cluster chạy liên tục tốn chi phí cố định, thừa cho truy vấn không thường xuyên.\n✗ EMR Hive chạy liên tục đòi hỏi quản lý cluster và chi phí cao khi truy vấn thưa thớt.\n✗ RDS cần import dữ liệu và không phù hợp để quét hàng terabyte log thô trong S3.","domain":3,"mock":3},
  {"id":"saa-m1-052","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một tổ chức cần chạy job ETL theo lịch để làm sạch, biến đổi và join nhiều nguồn dữ liệu trong S3 trước khi nạp vào data lake dạng Parquet. Họ muốn giải pháp serverless, tự động khám phá schema và sinh code transformation, với least operational overhead. Dịch vụ nào phù hợp nhất?","options":["AWS Glue (Crawler khám phá schema + ETL jobs trên Spark serverless) ghi output Parquet vào S3","Amazon EMR cluster với Spark được khởi tạo theo lịch để chạy job ETL","AWS Lambda functions được orchestrate bằng Step Functions để transform toàn bộ dữ liệu","Amazon Managed Service for Apache Flink xử lý dữ liệu rồi ghi vào S3"],"correctIndices":[0],"explanation":"Glue là dịch vụ ETL serverless với crawler khám phá schema và job Spark managed, lý tưởng cho batch ETL nạp data lake.\n✓ Glue serverless, crawler tự phát hiện schema, ETL job sinh code và ghi Parquet, least operational overhead.\n✗ EMR cần cấu hình và quản lý cluster Spark, nhiều operational overhead hơn Glue.\n✗ Lambda có giới hạn 15 phút và bộ nhớ, không hợp cho ETL nặng join nhiều nguồn TB dữ liệu.\n✗ Managed Service for Apache Flink dành cho streaming real-time, không phải batch ETL theo lịch.","domain":3,"mock":1},
  {"id":"saa-m2-052","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty đang dùng Apache Kafka tự host on-premises và muốn migrate lên AWS. Họ phụ thuộc vào hệ sinh thái Kafka (Kafka Connect, Schema Registry, các producer/consumer dùng Kafka API) và không muốn viết lại ứng dụng. Họ cần một managed service giảm gánh nặng vận hành broker nhưng giữ tương thích Kafka API. Lựa chọn nào tối ưu nhất?","options":["Amazon MSK (Managed Streaming for Apache Kafka)","Amazon Kinesis Data Streams thay thế Kafka và viết lại ứng dụng dùng KPL/KCL","Tự cài đặt Kafka cluster trên EC2 với Auto Scaling group","Amazon SQS FIFO kết hợp Lambda để mô phỏng hành vi Kafka"],"correctIndices":[0],"explanation":"MSK là Kafka được AWS quản lý, giữ nguyên Kafka API và hệ sinh thái, cho phép migrate mà không viết lại ứng dụng.\n✓ MSK tương thích Kafka API, hỗ trợ Kafka Connect/Schema Registry, AWS quản lý broker, ít vận hành hơn self-managed.\n✗ Kinesis Data Streams không tương thích Kafka API, buộc viết lại toàn bộ producer/consumer.\n✗ Self-managed Kafka trên EC2 vẫn để công ty gánh patching, scaling và vận hành broker.\n✗ SQS FIFO không cung cấp Kafka API, không hỗ trợ Kafka Connect/Schema Registry và mô hình consumer group.","domain":3,"mock":2},
  {"id":"saa-m3-051","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một pipeline streaming dùng Kinesis Data Streams cần biến đổi nhẹ từng record (làm sạch, enrich) ngay trước khi lưu vào S3 cho data lake. Lượng dữ liệu lớn nhưng transform đơn giản, và đội muốn tránh quản lý hạ tầng tối đa. Kiến trúc nào tối ưu nhất về least operational overhead?","options":["Kết nối Amazon Data Firehose vào stream và bật tích hợp Lambda transformation để biến đổi record trước khi ghi S3","Triển khai một consumer application Spark Streaming trên EMR đọc stream, transform và ghi S3","Dùng KCL consumer chạy trên một Auto Scaling group EC2 để transform và ghi S3","Ghi raw vào S3 bằng Firehose rồi chạy Glue ETL batch định kỳ để transform sau"],"correctIndices":[0],"explanation":"Firehose với Lambda transformation cho phép biến đổi từng record inline rồi tự ghi vào S3, fully managed cho transform nhẹ trên stream.\n✓ Firehose + Lambda transformation managed hoàn toàn, biến đổi record inline và tự ghi S3, ít vận hành nhất.\n✗ Spark Streaming trên EMR đòi hỏi quản lý cluster, quá nặng cho transform đơn giản.\n✗ KCL consumer trên EC2 ASG buộc tự viết và vận hành ứng dụng, nhiều operational overhead.\n✗ Ghi raw rồi Glue batch thêm độ trễ và một bước xử lý, không tối ưu khi transform có thể làm inline.","domain":3,"mock":3},
  {"id":"saa-m1-053","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty cần xử lý batch khổng lồ (hàng petabyte) bằng các framework big data như Apache Spark, Hive và Presto, với các job chạy không liên tục. Họ muốn tối ưu chi phí bằng cách dùng Spot Instances cho compute và tách storage khỏi compute. Giải pháp nào phù hợp nhất?","options":["Amazon EMR với transient cluster đọc/ghi dữ liệu trên S3 (EMRFS) và dùng Spot Instances cho task nodes","AWS Glue ETL chạy tất cả job Spark petabyte-scale theo mô hình serverless","Amazon Athena chạy mọi job transformation big data trực tiếp trên S3","Amazon Redshift Spectrum xử lý toàn bộ workload Spark và Hive"],"correctIndices":[0],"explanation":"EMR transient cluster với EMRFS tách storage (S3) khỏi compute và dùng Spot cho task nodes, tối ưu chi phí cho big data batch petabyte-scale.\n✓ EMR hỗ trợ Spark/Hive/Presto, dùng S3 làm storage tách rời, Spot Instances giảm chi phí cho job không liên tục.\n✗ Glue serverless tiện nhưng kém kinh tế và linh hoạt hơn EMR cho workload petabyte cần Spot và nhiều framework.\n✗ Athena là query engine SQL, không chạy được job Spark/Hive tùy biến petabyte-scale.\n✗ Redshift Spectrum chỉ query S3 từ Redshift, không phải nền tảng chạy Spark/Hive.","domain":3,"mock":1},
  {"id":"saa-m2-053","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"multi","question":"Một đội đang thiết kế ingestion layer cho data lake trên S3. Họ cần (1) nhận nhiều nguồn streaming với khả năng delivery vào S3 fully managed, và (2) tối ưu chi phí query về sau bằng cách lưu dữ liệu ở định dạng cột nén. Hãy chọn HAI hành động phù hợp nhất.","options":["Dùng Amazon Data Firehose để delivery streaming data vào S3 với buffering và retry tự động","Cấu hình Firehose chuyển đổi record format sang Apache Parquet trước khi ghi vào S3","Lưu toàn bộ dữ liệu dưới dạng raw JSON không nén để Athena dễ đọc nhất","Dùng Kinesis Data Streams và tự viết consumer EC2 để ghi từng record nhỏ riêng lẻ vào S3","Bật S3 Versioning trên bucket để tối ưu chi phí query Athena"],"correctIndices":[0,1],"explanation":"Firehose managed delivery cộng với chuyển sang Parquet là cặp giải pháp đúng cho ingestion fully managed và query tiết kiệm.\n✓ Firehose delivery streaming vào S3 fully managed với buffering/retry, đáp ứng yêu cầu ingestion.\n✓ Firehose convert sang Parquet (cột, nén) giảm dữ liệu Athena phải quét, tối ưu chi phí query.\n✗ JSON thô không nén làm tăng dung lượng quét và chi phí Athena, không tối ưu.\n✗ Tự viết consumer EC2 ghi file nhỏ tăng operational overhead và gây small-file problem.\n✗ S3 Versioning không liên quan đến tối ưu chi phí query Athena.","domain":3,"mock":2},
  {"id":"saa-m3-052","courseId":"SAA-C03","lesson":"ch2-03-database-performance","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng cần xử lý các file CSV được upload không thường xuyên vào S3: mỗi khi có file mới, hệ thống phải parse, validate và ghi kết quả vào DynamoDB. Khối lượng xử lý mỗi file nhỏ và hoàn tất trong vài giây. Đội muốn giải pháp event-driven, serverless, chỉ trả tiền khi chạy. Kiến trúc nào phù hợp nhất?","options":["S3 Event Notification kích hoạt một Lambda function để parse file và ghi vào DynamoDB","Một EMR cluster chạy liên tục poll S3 để phát hiện file mới và xử lý","Một Glue job được lên lịch chạy mỗi 5 phút để quét bucket tìm file mới","Một EC2 instance chạy cron job kiểm tra S3 và xử lý file định kỳ"],"correctIndices":[0],"explanation":"S3 Event Notification + Lambda là kiến trúc event-driven serverless, chỉ chạy và tính phí khi có file mới upload.\n✓ S3 event trigger Lambda xử lý ngay khi có file, serverless, chỉ trả tiền theo lần chạy.\n✗ EMR cluster chạy liên tục tốn chi phí cố định và là overkill cho file nhỏ không thường xuyên.\n✗ Glue job theo lịch thêm độ trễ và chi phí khi không có file, kém hơn mô hình event-driven.\n✗ EC2 cron poll tốn chi phí thường trực và tăng operational overhead so với serverless.","domain":3,"mock":3},
  {"id":"saa-m1-054","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty lưu trữ dữ liệu trên S3 với mẫu truy cập không thể dự đoán: một số object được truy cập thường xuyên, số khác hầu như không bao giờ, và mẫu này thay đổi theo thời gian. Họ muốn tối ưu chi phí lưu trữ một cách tự động mà KHÔNG cần phân tích thủ công hay viết lifecycle policy phức tạp. Giải pháp nào phù hợp nhất?","options":["Dùng S3 Intelligent-Tiering để tự động chuyển object giữa các access tier theo mẫu truy cập","Dùng S3 Standard cho mọi object để đảm bảo hiệu năng","Tạo lifecycle policy chuyển tất cả object sang S3 Standard-IA sau 30 ngày","Chuyển toàn bộ dữ liệu sang S3 Glacier Flexible Retrieval để giảm chi phí"],"correctIndices":[0],"explanation":"Mẫu truy cập không dự đoán được và hay thay đổi là đúng kịch bản của Intelligent-Tiering, vốn tự dịch chuyển object giữa các tier mà không có phí truy xuất.\n✓ S3 Intelligent-Tiering — đúng, tự động tối ưu chi phí theo mẫu truy cập thay đổi, không cần phân tích thủ công.\n✗ S3 Standard cho mọi object — không tối ưu chi phí cho dữ liệu ít truy cập.\n✗ Lifecycle sang Standard-IA sau 30 ngày — giả định cứng nhắc, có thể tính phí truy xuất nếu object lại được truy cập nhiều.\n✗ Glacier Flexible Retrieval — gây độ trễ truy xuất cho object đang được truy cập thường xuyên.","domain":4,"mock":1},
  {"id":"saa-m2-054","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty media lưu các bản ghi log truy cập vào S3. Log được phân tích nhiều trong 30 ngày đầu, sau đó hiếm khi cần nhưng vẫn phải truy xuất trong vài giờ nếu có yêu cầu audit, và phải giữ tổng cộng 7 năm vì lý do compliance. Giải pháp lifecycle nào TỐI ƯU CHI PHÍ nhất?","options":["Giữ ở S3 Standard 30 ngày, chuyển sang S3 Glacier Flexible Retrieval, rồi sang S3 Glacier Deep Archive, xóa sau 7 năm","Giữ ở S3 Standard 30 ngày rồi chuyển thẳng sang S3 Glacier Deep Archive, xóa sau 7 năm","Giữ ở S3 Standard-IA toàn bộ 7 năm","Giữ ở S3 Standard 30 ngày rồi chuyển sang S3 Standard-IA cho 7 năm còn lại"],"correctIndices":[0],"explanation":"Cần truy xuất trong vài giờ ở giai đoạn đầu sau 30 ngày (phù hợp Glacier Flexible Retrieval), rồi mới hạ xuống Deep Archive rẻ nhất cho phần đuôi dài.\n✓ Standard → Glacier Flexible Retrieval → Deep Archive — đúng, cân bằng giữa yêu cầu truy xuất vài giờ và chi phí thấp nhất về dài hạn.\n✗ Standard → thẳng Deep Archive — Deep Archive cần tới ~12 giờ để truy xuất, không đáp ứng được nhu cầu audit vài giờ trong giai đoạn đầu.\n✗ Standard-IA toàn bộ 7 năm — đắt hơn nhiều so với Glacier cho dữ liệu hầu như không truy cập.\n✗ Standard → Standard-IA cho 7 năm — vẫn đắt hơn nhiều so với tier archive cho dữ liệu lưu trữ dài hạn.","domain":4,"mock":2},
  {"id":"saa-m3-053","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng tạo ra hàng triệu object nhỏ (mỗi object khoảng 50 KB) trên S3. Dữ liệu được truy cập không thường xuyên sau 30 ngày nhưng cần truy xuất tức thì (milliseconds) khi cần. Kiến trúc sư muốn giảm chi phí nhưng lo ngại về phí tối thiểu. Lựa chọn TỐI ƯU CHI PHÍ nhất là gì?","options":["Giữ object ở S3 Standard vì S3 Standard-IA có kích thước object tối thiểu tính phí 128 KB","Chuyển object sang S3 Standard-IA sau 30 ngày bằng lifecycle policy","Chuyển object sang S3 One Zone-IA sau 30 ngày bằng lifecycle policy","Chuyển object sang S3 Glacier Instant Retrieval sau 30 ngày"],"correctIndices":[0],"explanation":"Với hàng triệu object 50 KB, các tier IA tính phí lưu trữ tối thiểu theo 128 KB/object nên thực tế đắt hơn Standard; truy xuất tức thì còn loại Glacier truyền thống.\n✓ Giữ ở S3 Standard — đúng, vì object dưới 128 KB bị tính phí như 128 KB ở tier IA, khiến IA đắt hơn cho object rất nhỏ và nhiều.\n✗ Standard-IA sau 30 ngày — phí tối thiểu 128 KB/object làm chi phí thực tế cao hơn với object 50 KB.\n✗ One Zone-IA sau 30 ngày — cũng bị phí tối thiểu 128 KB và giảm độ bền (một AZ).\n✗ Glacier Instant Retrieval — cũng có kích thước tối thiểu tính phí 128 KB và phí truy xuất, không tối ưu cho object nhỏ truy cập tức thì.","domain":4,"mock":3},
  {"id":"saa-m1-055","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"multi","question":"Một tổ chức có hàng nghìn EBS volume và muốn giảm chi phí cũng như rủi ro của việc quản lý EBS snapshot thủ công (tạo snapshot định kỳ, giữ lại theo policy, xóa snapshot cũ). Họ muốn least operational overhead. Chọn HAI hành động giúp đạt mục tiêu này.","options":["Dùng Amazon Data Lifecycle Manager (DLM) để tự động tạo, giữ và xóa EBS snapshot theo policy","Cấu hình DLM lưu trữ các snapshot ít dùng vào EBS Snapshots Archive tier để giảm chi phí lưu trữ snapshot dài hạn","Viết một cron job trên mỗi EC2 instance gọi AWS CLI để tạo snapshot","Copy thủ công snapshot sang một Region khác mỗi tuần để giảm chi phí","Bật EBS encryption để giảm chi phí lưu trữ snapshot"],"correctIndices":[0,1],"explanation":"DLM tự động hóa vòng đời snapshot, và DLM có thể lưu trữ snapshot ít dùng vào EBS Snapshots Archive tier rẻ hơn cho dài hạn, cùng giảm chi phí lẫn vận hành.\n✓ Amazon Data Lifecycle Manager — đúng, tự động hóa toàn bộ vòng đời snapshot, giảm thao tác thủ công.\n✓ DLM lưu vào EBS Snapshots Archive tier — đúng, hạ chi phí lưu trữ snapshot giữ lâu mà ít truy cập.\n✗ Cron job trên mỗi instance — tăng operational overhead và dễ lỗi, trái mục tiêu.\n✗ Copy snapshot sang Region khác hằng tuần — phục vụ DR/độ bền chứ không giảm chi phí, còn tăng phí.\n✗ Bật EBS encryption — liên quan bảo mật, không ảnh hưởng chi phí lưu trữ snapshot.","domain":4,"mock":1},
  {"id":"saa-m2-055","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một nhóm phân tích cần quản trị tập trung toàn bộ hàng trăm bucket S3 trên nhiều tài khoản trong AWS Organizations. Họ muốn hiểu xu hướng dung lượng lưu trữ, phát hiện bucket có nhiều object không được truy cập và tìm cơ hội tiết kiệm chi phí (ví dụ object nên chuyển tier). Công cụ AWS nào phù hợp nhất?","options":["S3 Storage Lens với dashboard nâng cao (advanced metrics) ở cấp organization","Amazon CloudWatch metrics cho từng bucket riêng lẻ","S3 Inventory report kết hợp Amazon Athena cho mỗi bucket","AWS Cost Explorer lọc theo dịch vụ S3"],"correctIndices":[0],"explanation":"S3 Storage Lens cung cấp khả năng quan sát dung lượng và hoạt động trên toàn organization với khuyến nghị tối ưu chi phí, đúng nhu cầu tập trung đa tài khoản.\n✓ S3 Storage Lens advanced ở cấp organization — đúng, cung cấp insight toàn tổ chức và khuyến nghị tối ưu chi phí cho hàng trăm bucket.\n✗ CloudWatch metrics từng bucket — không tập trung, không có khuyến nghị tối ưu, tốn công gộp thủ công.\n✗ S3 Inventory + Athena mỗi bucket — cho danh sách object nhưng đòi nhiều thao tác phân tích thủ công, không có dashboard tổng hợp tổ chức.\n✗ Cost Explorer lọc S3 — cho biết chi phí tổng nhưng không phân tích chi tiết hành vi truy cập object để khuyến nghị chuyển tier.","domain":4,"mock":2},
  {"id":"saa-m3-054","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty dùng Amazon EFS để lưu file của ứng dụng. Phân tích cho thấy phần lớn file chỉ được truy cập trong vài ngày đầu rồi gần như không động đến nữa, nhưng đôi khi vẫn cần đọc lại với độ trễ chấp nhận được. Họ muốn giảm chi phí lưu trữ EFS mà KHÔNG thay đổi code ứng dụng. Cách TỐI ƯU CHI PHÍ nhất là gì?","options":["Bật EFS Lifecycle Management để tự động chuyển file ít truy cập sang EFS Infrequent Access (IA), và bật Intelligent-Tiering để tự đưa file trở lại khi được truy cập","Di chuyển toàn bộ file sang một EFS file system mới dùng One Zone storage class","Sao chép file ít dùng sang S3 Glacier bằng script tùy biến và xóa khỏi EFS","Tăng provisioned throughput của EFS để xử lý nhanh hơn rồi xóa file cũ"],"correctIndices":[0],"explanation":"EFS Lifecycle Management cùng Intelligent-Tiering tự chuyển file ít truy cập xuống tier IA rẻ hơn (và đưa lại Standard khi được truy cập), giảm chi phí mà không sửa code.\n✓ EFS Lifecycle Management + IA + Intelligent-Tiering — đúng, tự động dịch chuyển file theo mẫu truy cập, minh bạch với ứng dụng.\n✗ EFS One Zone — giảm chi phí nhưng hạ độ bền (một AZ) và đòi di chuyển dữ liệu, không phải giải pháp tier hóa theo truy cập.\n✗ Script copy sang Glacier rồi xóa khỏi EFS — tăng operational overhead lớn và thay đổi cách truy cập file, đòi sửa ứng dụng.\n✗ Tăng provisioned throughput — làm tăng chi phí, không liên quan tối ưu lưu trữ.","domain":4,"mock":3},
  {"id":"saa-m1-056","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty lưu các bản backup database vào S3, chỉ truy cập lại khi cần khôi phục thảm họa (rất hiếm). Yêu cầu RTO cho phép vài giờ để truy xuất backup. Backup này KHÔNG cần độ bền multi-AZ vì đã có bản gốc ở nơi khác. Mục tiêu là chi phí lưu trữ thấp nhất có thể. Storage class nào phù hợp nhất?","options":["S3 Glacier Flexible Retrieval","S3 One Zone-IA","S3 Glacier Deep Archive","S3 Glacier Instant Retrieval"],"correctIndices":[0],"explanation":"Truy xuất hiếm với RTO vài giờ khớp đúng Glacier Flexible Retrieval (standard retrieval 3-5 giờ), rẻ hơn nhiều so với các tier truy cập nhanh trong khi vẫn đáp ứng RTO.\n✓ Glacier Flexible Retrieval — đúng, chi phí lưu trữ rất thấp và truy xuất trong vài giờ đáp ứng RTO.\n✗ One Zone-IA — chi phí lưu trữ cao hơn nhiều Glacier và phù hợp dữ liệu cần truy xuất tức thì hơn.\n✗ Deep Archive — rẻ hơn nữa nhưng truy xuất ~12 giờ có thể vượt RTO vài giờ.\n✗ Glacier Instant Retrieval — tối ưu cho truy cập tức thì nhưng đắt hơn Flexible Retrieval cho dữ liệu chỉ cần trong vài giờ.","domain":4,"mock":1},
  {"id":"saa-m2-056","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một bucket S3 bật versioning. Theo thời gian, các noncurrent (previous) version của object tích lũy và làm tăng chi phí lưu trữ đáng kể, dù chúng hiếm khi cần. Cách TỐI ƯU CHI PHÍ với least operational overhead để quản lý các version cũ này là gì?","options":["Cấu hình lifecycle rule chuyển noncurrent version sang Glacier sau X ngày và xóa (expire) chúng sau Y ngày","Tắt versioning trên bucket để ngừng tạo version mới","Viết một Lambda chạy hằng đêm liệt kê và xóa noncurrent version cũ","Bật S3 Replication để chuyển version cũ sang bucket rẻ hơn"],"correctIndices":[0],"explanation":"Lifecycle rule dành riêng cho noncurrent version cho phép tự động hạ tier rồi expire chúng, giảm chi phí mà không cần code.\n✓ Lifecycle rule cho noncurrent version (chuyển Glacier + expire) — đúng, tự động hóa hoàn toàn việc tier hóa và dọn dẹp version cũ.\n✗ Tắt versioning — không xóa version cũ đã tồn tại và làm mất khả năng bảo vệ dữ liệu.\n✗ Lambda xóa hằng đêm — tăng operational overhead và rủi ro lỗi so với lifecycle dựng sẵn.\n✗ S3 Replication — sao chép thêm dữ liệu làm tăng chi phí, không xử lý việc dọn version cũ.","domain":4,"mock":2},
  {"id":"saa-m3-055","courseId":"SAA-C03","lesson":"ch4-02-storage-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một data lake trên S3 dùng S3 Intelligent-Tiering. Kiến trúc sư muốn tối ưu thêm chi phí cho dữ liệu archive sâu nhưng vẫn giữ tính tự động của Intelligent-Tiering, đồng thời nắm rõ các đặc tính chi phí. Chọn HAI phát biểu ĐÚNG về cách tối ưu và đặc tính chi phí của S3 Intelligent-Tiering.","options":["Có thể bật Archive Access và Deep Archive Access tier (opt-in) trong Intelligent-Tiering để tự động chuyển object không truy cập lâu xuống tier archive rẻ hơn","S3 Intelligent-Tiering tính một khoản phí monitoring/automation nhỏ theo từng object, nên cần cân nhắc khi có rất nhiều object nhỏ","S3 Intelligent-Tiering áp dụng phí truy xuất (retrieval fee) cho mọi lần đọc object ở Frequent Access tier","Object nhỏ hơn 128 KB luôn bị tính phí truy xuất khi được tự động chuyển xuống Deep Archive Access tier","Khi bật Deep Archive Access tier, object được truy xuất tức thì (milliseconds) giống như ở Frequent Access tier"],"correctIndices":[0,1],"explanation":"Intelligent-Tiering hỗ trợ opt-in Archive/Deep Archive Access tier để tự động hạ chi phí sâu hơn, và nó tính một khoản phí monitoring/automation nhỏ theo object cần cân nhắc.\n✓ Bật Archive/Deep Archive Access tier (opt-in) — đúng, cho phép tự động chuyển object lạnh xuống tier archive rẻ hơn trong Intelligent-Tiering.\n✓ Có phí monitoring/automation nhỏ trên mỗi object — đúng, đây là chi phí ngầm cần cân nhắc, đặc biệt với rất nhiều object nhỏ.\n✗ Tính phí truy xuất cho mọi lần đọc ở Frequent Access tier — sai, Intelligent-Tiering không tính phí truy xuất khi truy cập object giữa các access tier thông thường.\n✗ Object nhỏ hơn 128 KB luôn bị tính phí truy xuất khi xuống Deep Archive — sai, object nhỏ hơn 128 KB không được tự động chuyển xuống tier archive mà giữ ở Frequent Access.\n✗ Deep Archive Access truy xuất tức thì — sai, object ở Deep Archive Access cần tới hàng giờ để khôi phục, không tức thì.","domain":4,"mock":3},
  {"id":"saa-m1-057","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty chạy một workload phân tích batch trên EC2. Workload có thể bị gián đoạn bất cứ lúc nào, tự lưu checkpoint và chạy lại từ checkpoint khi cần. Công ty muốn giảm chi phí compute tối đa. Giải pháp nào là MOST cost-effective?","options":["Chạy workload trên EC2 Spot Instances","Mua Standard Reserved Instances kỳ hạn 1 năm","Chạy trên EC2 On-Demand Instances","Mua EC2 Instance Savings Plans kỳ hạn 3 năm"],"correctIndices":[0],"explanation":"Workload chịu được gián đoạn và có checkpoint là use case kinh điển cho Spot, vốn rẻ tới ~90% so với On-Demand.\n✓ Spot Instances tận dụng capacity dư thừa với giá thấp nhất, phù hợp workload fault-tolerant, có thể gián đoạn.\n✗ Reserved Instances cam kết 1 năm phù hợp workload chạy liên tục, không khai thác được tính fault-tolerant để giảm chi phí tối đa.\n✗ On-Demand đắt nhất, lãng phí khi workload chấp nhận được gián đoạn.\n✗ Savings Plans 3 năm cam kết chi tiêu cố định, không rẻ bằng Spot cho workload gián đoạn được.","domain":4,"mock":1},
  {"id":"saa-m2-057","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một ứng dụng web Java chạy ổn định 24/7 trên các EC2 instance họ m5 trong suốt 3 năm tới với mức sử dụng dự đoán được. Kiến trúc sư muốn giảm chi phí nhưng vẫn giữ linh hoạt thay đổi instance family, size và Region khi cần. Lựa chọn nào tối ưu nhất?","options":["Compute Savings Plans kỳ hạn 3 năm","Standard Reserved Instances kỳ hạn 3 năm gắn với họ m5","EC2 Spot Instances trong một Spot Fleet","On-Demand Capacity Reservations"],"correctIndices":[0],"explanation":"Compute Savings Plans cho giảm giá tương đương RI nhưng linh hoạt qua mọi instance family, size, OS, Region và cả Fargate/Lambda.\n✓ Compute Savings Plans áp dụng tự động khi bạn đổi family/size/Region, lý tưởng cho nhu cầu giữ linh hoạt với cam kết dài hạn.\n✗ Standard Reserved Instances 3 năm cho discount cao nhưng gắn cứng vào họ m5, khó đổi family/Region.\n✗ Spot không phù hợp cho web app cần chạy liên tục 24/7 ổn định.\n✗ On-Demand Capacity Reservations chỉ đảm bảo capacity, không tự giảm giá đáng kể.","domain":4,"mock":2},
  {"id":"saa-m3-056","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một đội phát triển chạy ứng dụng microservices trên Amazon ECS với EC2 launch type. Họ phát hiện cluster thường xuyên có nhiều CPU và memory nhàn rỗi vào ban đêm, gây lãng phí chi phí. Họ muốn giảm chi phí với LEAST operational overhead. Giải pháp nào tốt nhất?","options":["Chuyển sang ECS với AWS Fargate và bật Fargate Spot cho các task không quan trọng","Cấu hình EC2 Auto Scaling với target tracking và ECS Cluster Auto Scaling (capacity provider) để tự động scale in các node nhàn rỗi vào ban đêm","Mua Reserved Instances cho toàn bộ EC2 trong cluster để giảm đơn giá","Tăng kích thước instance để gộp nhiều task hơn trên ít node hơn"],"correctIndices":[1],"explanation":"Idle về đêm là vấn đề về capacity dư thừa; ECS Cluster Auto Scaling (capacity provider) phối hợp EC2 Auto Scaling tự động scale in để loại bỏ node nhàn rỗi.\n✓ ECS Cluster Auto Scaling qua capacity provider kết hợp EC2 Auto Scaling tự co giãn theo số task thực, loại bỏ node idle ban đêm với ít vận hành.\n✗ Chuyển toàn bộ sang Fargate là thay đổi launch type lớn, nhiều overhead di trú và không phải lúc nào cũng rẻ hơn cho tải ổn định.\n✗ Reserved Instances giảm đơn giá nhưng vẫn trả tiền cho node idle ban đêm, không giải quyết lãng phí.\n✗ Tăng instance size không tự giảm capacity khi tải thấp về đêm.","domain":4,"mock":3},
  {"id":"saa-m1-058","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một công ty chạy API REST có lưu lượng rất biến động: cao điểm ngắn vào giờ làm việc và gần như không có request vào ban đêm và cuối tuần. Hiện tại API chạy trên một EC2 Auto Scaling Group với mức tối thiểu luôn bật, dẫn đến trả tiền cho thời gian rỗi. Kiến trúc sư muốn giải pháp MOST cost-effective và serverless. Phương án nào tốt nhất?","options":["Tái kiến trúc API dùng Amazon API Gateway và AWS Lambda","Giảm minimum size của Auto Scaling Group xuống 1 và dùng Spot Instances","Chuyển sang AWS Fargate với một service luôn chạy 2 task","Mua Compute Savings Plans cho Auto Scaling Group hiện tại"],"correctIndices":[0],"explanation":"Với lưu lượng spiky và nhiều thời gian rỗi, mô hình pay-per-request của Lambda + API Gateway loại bỏ hoàn toàn chi phí idle và là kiến trúc serverless.\n✓ API Gateway + Lambda chỉ tính tiền theo số request/thời gian chạy, không trả tiền khi không có traffic, tối ưu nhất cho tải biến động và đúng yêu cầu serverless.\n✗ Giảm min size + Spot vẫn phải trả tiền cho instance đang chạy và không phải serverless.\n✗ Fargate với 2 task luôn chạy vẫn tốn tiền suốt thời gian rỗi ban đêm và cuối tuần.\n✗ Savings Plans cam kết chi tiêu cố định, lãng phí khi tải gần như bằng 0 phần lớn thời gian.","domain":4,"mock":1},
  {"id":"saa-m2-058","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng backend chạy trên EC2 m5.4xlarge. Sau khi xem AWS Compute Optimizer và CloudWatch, đội nhận thấy CPU trung bình chỉ ~8% và memory ~15% trong nhiều tuần. Ứng dụng là Java thuần, đã được kiểm thử chạy tốt trên ARM. Hành động nào MOST cost-effective?","options":["Right-size xuống một instance dựa trên Graviton (ví dụ m7g) có kích thước nhỏ hơn theo khuyến nghị của Compute Optimizer","Mua Standard Reserved Instances 3 năm cho m5.4xlarge để khóa giá thấp","Bật EC2 Auto Scaling để thêm m5.4xlarge khi tải tăng","Chuyển ứng dụng sang Spot Instances cùng kích thước m5.4xlarge"],"correctIndices":[0],"explanation":"Instance over-provisioned nghiêm trọng; right-sizing xuống bậc nhỏ hơn và sang Graviton vừa giảm lãng phí vừa cải thiện price/performance.\n✓ Right-size theo khuyến nghị Compute Optimizer + chuyển sang Graviton (m7g) cắt giảm cả số core thừa lẫn đơn giá, tối ưu chi phí nhất cho app ARM-ready.\n✗ Reserved Instances 3 năm cho m5.4xlarge khóa giá cho một instance vốn đã quá lớn, cố định lãng phí.\n✗ Auto Scaling thêm instance không giải quyết được việc instance hiện tại đã quá khổ.\n✗ Spot cùng size m5.4xlarge giảm đơn giá nhưng vẫn trả cho tài nguyên thừa và thêm rủi ro gián đoạn.","domain":4,"mock":2},
  {"id":"saa-m3-057","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một hệ thống xử lý hàng đợi đọc message từ Amazon SQS và xử lý ảnh. Tải nền (baseline) ổn định khoảng 10 instance suốt ngày đêm, nhưng có spike xử lý theo lô có thể chịu gián đoạn. Công ty muốn cấu trúc chi phí tối ưu cho cả baseline lẫn spike. Cách tiếp cận nào tốt nhất?","options":["Dùng Savings Plans (hoặc Reserved Instances) cho ~10 instance baseline và dùng Spot Instances cho phần spike fault-tolerant","Dùng On-Demand cho toàn bộ baseline và spike để đơn giản hóa vận hành","Mua Reserved Instances cho cả baseline lẫn công suất spike đỉnh","Dùng Spot Instances cho toàn bộ cả baseline lẫn spike để giá rẻ nhất"],"correctIndices":[0],"explanation":"Phối hợp commitment cho phần luôn chạy và Spot cho phần co giãn fault-tolerant là pattern tối ưu chi phí kinh điển.\n✓ Savings Plans/RI phủ baseline ổn định với discount, còn Spot phủ spike chịu gián đoạn được giá rẻ nhất — tối ưu cả hai phần.\n✗ On-Demand toàn bộ đơn giản nhưng đắt nhất, bỏ qua cơ hội discount cho baseline.\n✗ RI cho cả công suất spike đỉnh lãng phí vì spike không thường trực, cam kết phần ít dùng.\n✗ Spot cho toàn bộ rủi ro cho baseline cần độ ổn định, có thể bị reclaim đồng loạt.","domain":4,"mock":3},
  {"id":"saa-m1-059","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty fintech chạy fleet EC2 stateless lớn cho fanout xử lý sự kiện. Họ muốn dùng Spot để tiết kiệm nhưng cần giảm thiểu khả năng cả fleet bị gián đoạn cùng lúc khi capacity một pool cạn. Cấu hình nào giúp đạt MOST resilient mà vẫn tối ưu chi phí Spot?","options":["Dùng EC2 Auto Scaling với allocation strategy price-capacity-optimized và đa dạng hóa qua nhiều instance type và Availability Zone","Dùng một Spot Fleet chỉ chọn instance type duy nhất rẻ nhất trong một AZ","Dùng allocation strategy lowest-price chỉ giới hạn ở hai instance type rẻ nhất","Đặt một maximum Spot price thật cao để không bao giờ bị gián đoạn"],"correctIndices":[0],"explanation":"Đa dạng hóa qua nhiều instance type/AZ với price-capacity-optimized giúp Auto Scaling rút từ các pool dồi dào nhất, giảm rủi ro gián đoạn đồng loạt.\n✓ price-capacity-optimized + đa dạng hóa instance type và AZ phân tán rủi ro qua nhiều Spot pool, vừa rẻ vừa bền vững nhất.\n✗ Một instance type trong một AZ tập trung rủi ro vào một pool duy nhất, dễ bị reclaim toàn bộ.\n✗ lowest-price giới hạn 2 type ưu tiên giá nhưng vẫn ít pool, dễ cạn capacity cùng lúc.\n✗ Đặt max price cao không ngăn được gián đoạn do thiếu capacity — Spot bị reclaim khi hết capacity, không chỉ vì giá.","domain":4,"mock":1},
  {"id":"saa-m2-059","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một tổ chức đang chạy nhiều workload và muốn lập chương trình tối ưu chi phí compute toàn diện. Họ cần khuyến nghị dựa trên dữ liệu sử dụng thực tế và các pattern cam kết phù hợp. Những hành động nào sau đây giúp giảm chi phí compute một cách hợp lý? (Chọn 2)","options":["Dùng AWS Compute Optimizer để xác định instance over-provisioned và right-size theo khuyến nghị dựa trên metrics","Áp dụng Savings Plans cho phần tải compute baseline ổn định, dài hạn","Bật detailed monitoring trên mọi instance để tự động giảm đơn giá compute","Chuyển toàn bộ workload sang Dedicated Hosts để giảm chi phí","Mua nhiều Elastic IP để dự phòng nhằm tiết kiệm chi phí EC2"],"correctIndices":[0,1],"explanation":"Tối ưu chi phí dựa vào loại bỏ tài nguyên thừa (right-sizing) và cam kết cho phần tải ổn định (Savings Plans).\n✓ Compute Optimizer phân tích metrics để chỉ ra instance quá khổ và đề xuất right-sizing, cắt lãng phí trực tiếp.\n✓ Savings Plans cho baseline ổn định mang lại discount đáng kể so với On-Demand cho phần luôn chạy.\n✗ Detailed monitoring tăng độ chi tiết metrics nhưng còn tính thêm phí, không tự giảm đơn giá compute.\n✗ Dedicated Hosts thường đắt hơn, dùng cho licensing/compliance chứ không phải để tiết kiệm chung.\n✗ Elastic IP không liên quan giảm chi phí compute; EIP còn bị tính phí và nay tính phí cho cả IPv4 public đang dùng.","domain":4,"mock":2},
  {"id":"saa-m3-058","courseId":"SAA-C03","lesson":"ch4-01-compute-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một nền tảng SaaS chạy hàng nghìn Lambda function được gọi liên tục với khối lượng rất lớn và ổn định mỗi tháng. Hóa đơn Lambda đang tăng nhanh. Các function là Node.js thuần, không phụ thuộc kiến trúc. Kiến trúc sư muốn giảm chi phí Lambda mà giữ kiến trúc serverless và LEAST operational overhead. Kết hợp nào hợp lý nhất?","options":["Mua Compute Savings Plans để phủ phần Lambda usage ổn định và chuyển function sang kiến trúc Graviton (arm64)","Tăng memory cấp cho mỗi function lên mức tối đa để chạy nhanh hơn","Di trú toàn bộ function sang EC2 On-Demand để có giá compute rẻ hơn","Bật Provisioned Concurrency tối đa cho mọi function để giảm chi phí mỗi lần gọi"],"correctIndices":[0],"explanation":"Compute Savings Plans cũng áp dụng cho Lambda, và Graviton/arm64 cho Lambda rẻ hơn cùng price/performance tốt hơn — cả hai giữ nguyên serverless.\n✓ Compute Savings Plans phủ Lambda usage ổn định để giảm giá, cộng arm64 (Graviton) hạ thêm chi phí mỗi GB-giây với ít thay đổi vận hành.\n✗ Tăng memory tối đa làm tăng chi phí GB-giây trừ khi rút ngắn đáng kể thời gian chạy; bừa bãi sẽ đắt hơn.\n✗ Di trú sang EC2 đánh mất serverless và tăng mạnh operational overhead quản lý hạ tầng.\n✗ Provisioned Concurrency tối đa cho mọi function làm TĂNG chi phí vì trả tiền giữ sẵn capacity, không phải để giảm giá.","domain":4,"mock":3},
  {"id":"saa-m1-060","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty chạy ứng dụng nội bộ trên Amazon RDS for PostgreSQL với tải ổn định, dự đoán được và sẽ vận hành liên tục ít nhất 3 năm tới. Họ muốn giảm chi phí database đáng kể nhất mà không thay đổi kiến trúc. Giải pháp cost-effective nhất là gì?","options":["Mua RDS Reserved Instances kỳ hạn 3 năm cho instance đang dùng","Chuyển sang Aurora Serverless v2 để tự scale theo tải","Bật Multi-AZ để tận dụng instance dự phòng","Chuyển sang DynamoDB on-demand capacity mode"],"correctIndices":[0],"explanation":"Tải ổn định, dự đoán được và cam kết dùng dài hạn là điều kiện lý tưởng cho RDS Reserved Instances, tiết kiệm tới ~60% so với On-Demand.\n✓ RDS Reserved Instances 3 năm — đúng, tải ổn định + cam kết dài hạn cho chiết khấu cao nhất mà không đổi kiến trúc.\n✗ Aurora Serverless v2 — tối ưu cho tải biến động, không tiết kiệm bằng RI khi tải đã ổn định và chạy liên tục.\n✗ Multi-AZ — tăng độ sẵn sàng nhưng tăng gấp đôi chi phí compute, không phải tối ưu chi phí.\n✗ DynamoDB on-demand — đổi hẳn mô hình dữ liệu (NoSQL) và On-Demand đắt hơn cho tải ổn định.","domain":4,"mock":1},
  {"id":"saa-m2-060","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng SaaS dùng Amazon Aurora MySQL. Lưu lượng database có pattern khó dự đoán: gần như không có tải vào ban đêm và cuối tuần, nhưng tăng vọt bất ngờ trong giờ làm việc. Hiện tại provisioned instance được cấu hình theo peak nên lãng phí lớn lúc thấp tải. Giải pháp cost-effective nhất với least operational overhead là gì?","options":["Chuyển sang Aurora Serverless v2, đặt Aurora Capacity Units min/max phù hợp","Giữ provisioned instance nhưng viết Lambda + scheduler để stop/start theo giờ","Tạo thêm Aurora read replicas và phân tải bằng Route 53 weighted routing","Right-size provisioned instance xuống bằng tải trung bình hàng ngày"],"correctIndices":[0],"explanation":"Aurora Serverless v2 tự scale ACU lên/xuống gần như tức thời theo tải biến động khó dự đoán, chỉ trả tiền cho capacity thực dùng, không cần tự quản lý lịch.\n✓ Aurora Serverless v2 — đúng, tự co giãn theo spike, scale gần 0 lúc nhàn rỗi, least operational overhead.\n✗ Lambda stop/start theo giờ — không xử lý được spike bất ngờ và thêm gánh nặng vận hành lịch.\n✗ Read replicas + Route 53 — tăng chi phí và chỉ giúp đọc, không giải quyết lãng phí lúc thấp tải.\n✗ Right-size xuống mức trung bình — sẽ không chịu nổi spike giờ làm việc, gây nghẽn hiệu năng.","domain":4,"mock":2},
  {"id":"saa-m3-059","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một bảng Amazon DynamoDB phục vụ cho tính năng mới vừa ra mắt. Lưu lượng hiện rất thấp và cực kỳ khó dự đoán vì chưa rõ tốc độ tăng trưởng người dùng. Team không muốn phải tự dự báo capacity và muốn tránh throttling khi traffic tăng đột ngột. Capacity mode nào phù hợp nhất?","options":["On-demand capacity mode","Provisioned capacity mode với auto scaling","Provisioned capacity mode với reserved capacity","On-demand capacity mode kèm DynamoDB Accelerator (DAX)"],"correctIndices":[0],"explanation":"Với traffic mới, thấp và không dự đoán được, On-demand loại bỏ hoàn toàn việc dự báo capacity và tự xử lý spike, chỉ trả theo request thực tế.\n✓ On-demand — đúng, không cần dự báo, hấp thụ spike tức thì, lý tưởng cho workload mới/khó đoán.\n✗ Provisioned + auto scaling — auto scaling phản ứng chậm hơn với spike đột ngột và vẫn cần đặt min/max ban đầu.\n✗ Provisioned + reserved capacity — cam kết trước cho tải không dự đoán được là rủi ro lãng phí lớn.\n✗ On-demand + DAX — DAX là cache giải quyết latency đọc, không liên quan bài toán capacity/dự báo.","domain":4,"mock":3},
  {"id":"saa-m1-061","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một bảng DynamoDB đã chạy ổn định 18 tháng với throughput rất đều đặn, dự đoán tốt và nằm ở mức cao liên tục. Hiện đang dùng On-demand capacity mode và hóa đơn khá lớn. Solutions Architect muốn giảm chi phí. Cách tối ưu chi phí nhất là gì?","options":["Chuyển sang Provisioned capacity với auto scaling và mua reserved capacity","Giữ On-demand nhưng bật DynamoDB Standard-IA table class","Bật DynamoDB auto scaling trên chính bảng On-demand hiện tại","Chuyển dữ liệu sang Aurora Serverless v2 để rẻ hơn"],"correctIndices":[0],"explanation":"Throughput cao, đều và dự đoán được là điều kiện lý tưởng để rời On-demand sang Provisioned; reserved capacity còn giảm thêm chi phí cho phần baseline cam kết.\n✓ Provisioned + auto scaling + reserved capacity — đúng, rẻ hơn nhiều On-demand với tải đều đặn, dự đoán được.\n✗ Standard-IA table class — chỉ giảm chi phí storage cho dữ liệu ít truy cập, không giảm chi phí throughput cao.\n✗ Auto scaling trên On-demand — On-demand không dùng auto scaling capacity; tùy chọn này không hợp lệ về bản chất.\n✗ Chuyển sang Aurora Serverless v2 — di chuyển sang mô hình quan hệ là thay đổi lớn, không cần thiết khi DynamoDB đang phù hợp.","domain":4,"mock":1},
  {"id":"saa-m2-061","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng báo cáo dùng Amazon RDS for MySQL (Single-AZ). Phần lớn tải là các truy vấn analytics đọc nặng chạy ban ngày, làm primary instance quá tải, trong khi ghi rất ít. Team muốn giảm tải cho primary và tối ưu chi phí thay vì nâng cấp primary lên instance class lớn hơn nhiều. Giải pháp nào tốt nhất?","options":["Thêm một RDS read replica và định tuyến truy vấn analytics đọc sang replica","Bật Multi-AZ rồi đọc từ standby instance để giảm tải primary","Scale up primary lên instance class lớn gấp đôi để chịu tải đọc","Chuyển toàn bộ workload sang Aurora Serverless v2"],"correctIndices":[0],"explanation":"Read replica cho phép offload truy vấn đọc nặng khỏi primary với chi phí một instance bổ sung, rẻ và linh hoạt hơn nhiều so với phóng to primary.\n✓ Read replica + định tuyến đọc — đúng, giảm tải đọc cho primary đúng chỗ, chi phí tăng có kiểm soát.\n✗ Multi-AZ standby — standby trong RDS MySQL không phục vụ đọc; chỉ để failover.\n✗ Scale up primary gấp đôi — đắt và vẫn dùng một instance gánh cả đọc lẫn ghi, kém tối ưu chi phí.\n✗ Chuyển sang Aurora Serverless v2 — thay đổi engine/kiến trúc lớn, quá mức cho nhu cầu offload đọc.","domain":4,"mock":2},
  {"id":"saa-m3-060","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty fintech chạy Aurora PostgreSQL cluster cho hệ thống giao dịch. Tải đọc tăng đều theo tăng trưởng và họ đã thêm 4 read replicas, nhưng nhiều replica chỉ đạt ~20% sử dụng vào ban đêm, gây lãng phí. Họ cần đảm bảo đủ capacity đọc lúc cao điểm nhưng cost-effective lúc thấp điểm, với least operational overhead. Giải pháp nào tốt nhất?","options":["Dùng Aurora Auto Scaling cho read replicas dựa trên CPU/connections để tự thêm bớt replica theo tải","Thay tất cả replica bằng một Aurora Serverless v2 reader duy nhất rồi tự scale ACU","Đặt CloudWatch alarm gửi SNS để DBA thủ công thêm/xóa replica theo ca","Chuyển toàn bộ replica sang Reserved Instances để giảm giá mỗi replica"],"correctIndices":[0],"explanation":"Aurora Auto Scaling tự động thêm/bớt read replicas theo metric tải, đảm bảo capacity lúc peak và cắt giảm replica thừa lúc nhàn rỗi mà không cần can thiệp thủ công.\n✓ Aurora Auto Scaling cho read replicas — đúng, tự co giãn số replica theo tải, tối ưu chi phí và least operational overhead.\n✗ Một Serverless v2 reader duy nhất — mất tính dự phòng nhiều replica và giới hạn khả năng phân tải đọc lớn lúc peak.\n✗ CloudWatch alarm + DBA thủ công — phụ thuộc thao tác tay, operational overhead cao, dễ phản ứng chậm.\n✗ Reserved Instances cho mọi replica — cam kết trước cho replica vốn lãng phí ban đêm chỉ khóa cứng sự lãng phí đó.","domain":4,"mock":3},
  {"id":"saa-m1-062","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty có hai môi trường: production database (Aurora MySQL) chạy tải ổn định 24/7 dự đoán tốt, và một fleet các test/dev database chạy không liên tục, hay được tạo/xóa theo dự án. Họ muốn tối ưu chi phí toàn bộ với least operational overhead. Chiến lược nào phù hợp nhất cho từng môi trường?","options":["Production dùng provisioned + Reserved Instances; test/dev dùng Aurora Serverless v2","Cả hai môi trường dùng Aurora Serverless v2 để đồng nhất quản lý","Cả hai môi trường mua Reserved Instances 1 năm để giảm giá tối đa","Production dùng Aurora Serverless v2; test/dev mua Reserved Instances"],"correctIndices":[0],"explanation":"Tải ổn định 24/7 hợp với provisioned + RI để chiết khấu tối đa, còn test/dev chạy không liên tục hợp với Serverless v2 vì tự scale về thấp khi nhàn rỗi.\n✓ Production RI + test/dev Serverless v2 — đúng, mỗi môi trường dùng mô hình giá tối ưu theo đặc tính tải.\n✗ Cả hai Serverless v2 — production tải ổn định sẽ bỏ lỡ chiết khấu RI, đắt hơn không cần thiết.\n✗ Cả hai mua RI — test/dev không liên tục mà cam kết RI sẽ trả tiền cả lúc không dùng, lãng phí.\n✗ Đảo ngược (prod Serverless, dev RI) — sai cả hai chiều: prod mất chiết khấu, dev bị khóa cam kết lãng phí.","domain":4,"mock":1},
  {"id":"saa-m2-062","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"multi","question":"Solutions Architect review một Amazon RDS for PostgreSQL Multi-AZ instance đang chạy db.r5.4xlarge. Báo cáo CloudWatch cho thấy CPU trung bình 12%, FreeableMemory cao liên tục, IOPS thấp suốt 60 ngày. Mục tiêu là cost-optimize database mà vẫn giữ tính sẵn sàng cho production. Những hành động nào hợp lý? (Chọn 2)","options":["Right-size xuống một instance class nhỏ hơn (ví dụ db.r5.large) dựa trên metric thực tế","Mua Reserved Instance cho instance class mới sau khi đã right-size và xác nhận ổn định","Tắt Multi-AZ để tiết kiệm một nửa chi phí compute","Bật thêm 3 read replicas để phân tán tải CPU 12%","Chuyển ngay sang db.r5.8xlarge để có headroom cho tương lai"],"correctIndices":[0,1],"explanation":"Metric thấp kéo dài cho thấy instance bị over-provisioned: nên right-size xuống rồi mới cam kết RI cho instance class mới để khóa chiết khấu lâu dài.\n✓ Right-size xuống instance nhỏ hơn — đúng, CPU/RAM/IOPS thấp suốt 60 ngày là dấu hiệu rõ ràng over-provisioned.\n✓ Mua RI sau khi right-size — đúng, cam kết RI cho size đúng để tối ưu chi phí dài hạn của production ổn định.\n✗ Tắt Multi-AZ — hy sinh tính sẵn sàng của production, vi phạm yêu cầu giữ HA.\n✗ Thêm read replicas — CPU mới 12%, không có tải để phân tán; chỉ làm tăng chi phí.\n✗ Lên db.r5.8xlarge — phóng to ngược lại khi đang dư thừa, càng lãng phí.","domain":4,"mock":2},
  {"id":"saa-m3-061","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một ứng dụng e-commerce dùng DynamoDB với traffic có baseline ổn định khá lớn suốt ngày, nhưng đột biến gấp 8-10 lần trong các đợt flash sale theo lịch định trước vài lần mỗi tháng. Họ muốn cost-effective cho phần baseline nhưng tránh throttling lúc flash sale. Cách tiếp cận nào tối ưu nhất?","options":["Provisioned capacity với auto scaling cho baseline, và lên lịch tăng target capacity trước mỗi flash sale","Để bảng ở On-demand capacity mode suốt thời gian để khỏi lo capacity","Provisioned capacity cố định ở mức peak flash sale để luôn đủ throughput","Provisioned capacity ở mức baseline và dựa hoàn toàn vào burst capacity lúc flash sale"],"correctIndices":[0],"explanation":"Baseline lớn và ổn định nên dùng Provisioned (rẻ hơn On-demand) với auto scaling; vì flash sale theo lịch, có thể scheduled scaling nâng capacity trước sự kiện để tránh throttling.\n✓ Provisioned + auto scaling + lên lịch tăng trước flash sale — đúng, rẻ cho baseline và chủ động đủ capacity cho spike đã biết lịch.\n✗ On-demand suốt thời gian — an toàn nhưng đắt hơn nhiều cho phần baseline lớn, ổn định.\n✗ Provisioned cố định ở peak — trả tiền cho 8-10x capacity cả lúc bình thường, cực lãng phí.\n✗ Dựa vào burst capacity — burst chỉ là dự trữ ngắn hạn nhỏ, không đủ cho spike 8-10x kéo dài.","domain":4,"mock":3},
  {"id":"saa-m1-063","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một công ty chạy nhiều EC2 instances trong private subnets. Các instances này thường xuyên đọc và ghi một lượng lớn dữ liệu vào một S3 bucket trong cùng Region. Hiện tại traffic đi qua NAT Gateway, và hóa đơn data processing của NAT Gateway đang rất cao. Giải pháp nào giảm chi phí HIỆU QUẢ NHẤT mà không ảnh hưởng đến khả năng truy cập S3?","options":["Tạo một Gateway VPC endpoint cho S3 và cập nhật route table của private subnets để định tuyến traffic tới S3 qua endpoint này","Tạo một Interface VPC endpoint (PrivateLink) cho S3 và tính phí theo giờ cộng phí xử lý dữ liệu","Di chuyển các EC2 instances sang public subnets và gán Elastic IP cho mỗi instance","Tăng số lượng NAT Gateway lên nhiều AZ để phân tải và giảm chi phí data processing"],"correctIndices":[0],"explanation":"Gateway VPC endpoint cho S3 hoàn toàn miễn phí và loại bỏ traffic S3 khỏi NAT Gateway, cắt giảm phí data processing.\n✓ Gateway VPC endpoint cho S3 không tính phí giờ hay phí xử lý dữ liệu, traffic tới S3 đi nội bộ trong AWS network thay vì qua NAT Gateway.\n✗ Interface endpoint cho S3 phát sinh phí giờ và phí xử lý dữ liệu — Gateway endpoint mới là lựa chọn miễn phí tối ưu.\n✗ Chuyển sang public subnets làm mất tính bảo mật và vẫn phát sinh chi phí internet egress.\n✗ Thêm nhiều NAT Gateway chỉ làm tăng chi phí, không loại bỏ phí data processing.","domain":4,"mock":1},
  {"id":"saa-m2-063","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng web phục vụ nội dung tĩnh (hình ảnh, video, JS, CSS) từ một S3 bucket cho người dùng toàn cầu. Chi phí data transfer out từ S3 ra internet đang tăng nhanh, và người dùng ở xa Region phàn nàn về độ trễ. Kiến trúc nào vừa giảm chi phí egress vừa cải thiện hiệu năng HIỆU QUẢ NHẤT?","options":["Đặt một CloudFront distribution trước S3 bucket, dùng Origin Access Control để chỉ cho CloudFront truy cập origin","Bật S3 Transfer Acceleration trên bucket để tăng tốc độ tải xuống cho người dùng toàn cầu","Sao chép (replicate) bucket sang nhiều Region bằng S3 Cross-Region Replication và dùng Route 53 latency routing","Chuyển S3 sang storage class S3 One Zone-IA để giảm chi phí lưu trữ và data transfer"],"correctIndices":[0],"explanation":"CloudFront cache nội dung tại edge locations, giảm số request tới origin và data transfer từ S3 ra CloudFront rẻ hơn (thậm chí miễn phí), đồng thời phục vụ người dùng từ edge gần nhất.\n✓ CloudFront giảm origin egress nhờ caching và data transfer S3→CloudFront không tính phí, cải thiện độ trễ qua edge locations.\n✗ S3 Transfer Acceleration tối ưu cho upload và còn tính phí phụ thu, không giảm egress cost cho phân phối nội dung.\n✗ Cross-Region Replication tăng chi phí lưu trữ và replication, phức tạp hơn nhiều so với CloudFront.\n✗ Storage class chỉ ảnh hưởng phí lưu trữ, không giảm data transfer out ra internet.","domain":4,"mock":2},
  {"id":"saa-m3-062","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một kiến trúc multi-tier có web tier và database tier (Amazon RDS) được triển khai trên cùng một Region. Để đạt high availability, đội kỹ thuật đặt EC2 web instances trải đều trên ba Availability Zones, còn RDS Multi-AZ với primary ở AZ-a. Họ nhận thấy chi phí data transfer giữa các tier khá cao. Cách nào giảm cross-AZ data transfer cost mà vẫn duy trì tính sẵn sàng cao?","options":["Sử dụng RDS Proxy và đặt read replica ở mỗi AZ để các web instances đọc từ replica cùng AZ, đồng thời giữ Multi-AZ cho failover","Gom tất cả EC2 web instances vào cùng một AZ với RDS primary để loại bỏ hoàn toàn cross-AZ traffic","Chuyển RDS sang Single-AZ deployment để tránh phí đồng bộ giữa primary và standby","Dùng một Cluster placement group cho các web instances để giảm độ trễ mạng giữa chúng"],"correctIndices":[0],"explanation":"Đặt read replica ở mỗi AZ cho phép phần lớn lưu lượng đọc diễn ra trong cùng AZ (không phát sinh cross-AZ cost) trong khi vẫn giữ Multi-AZ cho HA.\n✓ Read replica theo từng AZ giúp read traffic ở lại trong AZ, giảm cross-AZ cost mà vẫn giữ Multi-AZ failover cho tính sẵn sàng cao.\n✗ Gom hết web instances vào một AZ phá vỡ high availability — mất một AZ là sập toàn bộ web tier.\n✗ Single-AZ RDS loại bỏ HA của database, đi ngược yêu cầu.\n✗ Cluster placement group nằm trong một AZ và không phù hợp cho thiết kế HA đa AZ, không giải quyết cost giữa tier.","domain":4,"mock":3},
  {"id":"saa-m1-064","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một doanh nghiệp có on-premises data center cần truyền 50 TB dữ liệu mỗi tháng lên AWS một cách ổn định và cần băng thông nhất quán cho ứng dụng nhạy cảm độ trễ. Hiện họ dùng Site-to-Site VPN qua internet và chi phí data transfer cùng độ trễ không ổn định là vấn đề. Giải pháp nào tối ưu chi phí dài hạn và đảm bảo băng thông ổn định?","options":["Thiết lập AWS Direct Connect với một dedicated connection từ on-premises tới AWS","Mở rộng Site-to-Site VPN thành nhiều tunnel để tăng băng thông tổng","Dùng AWS DataSync qua internet để tối ưu hóa tốc độ truyền dữ liệu","Sử dụng AWS Snowball để vận chuyển dữ liệu định kỳ mỗi tháng"],"correctIndices":[0],"explanation":"Direct Connect cung cấp băng thông ổn định, độ trễ nhất quán và giá data transfer out thấp hơn đáng kể so với internet/VPN, tối ưu cho khối lượng lớn ổn định dài hạn.\n✓ Direct Connect đảm bảo dedicated bandwidth, độ trễ ổn định và data transfer rate rẻ hơn internet — lý tưởng cho 50 TB/tháng đều đặn.\n✗ Nhiều VPN tunnel vẫn đi qua internet công cộng, băng thông và độ trễ không đảm bảo.\n✗ DataSync qua internet không giải quyết được vấn đề độ trễ ổn định và vẫn chịu phí internet transfer.\n✗ Snowball phù hợp cho di chuyển một lần lượng cực lớn, không phù hợp cho luồng dữ liệu liên tục cần băng thông realtime.","domain":4,"mock":1},
  {"id":"saa-m2-064","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"multi","question":"Một fleet EC2 trong private subnets cần truy cập nhiều AWS service trong cùng Region: S3, DynamoDB, Amazon ECR (kéo container images), và CloudWatch Logs. Toàn bộ traffic hiện đi qua NAT Gateway, chi phí data processing rất cao. Đội kiến trúc muốn giảm tối đa traffic qua NAT Gateway. Những hành động nào nên thực hiện? (Chọn HAI)","options":["Tạo Gateway VPC endpoints cho S3 và DynamoDB","Tạo Interface VPC endpoints cho ECR (ecr.api, ecr.dkr) và CloudWatch Logs","Thay thế NAT Gateway bằng NAT instance trên một EC2 nhỏ để giảm chi phí","Tạo Interface VPC endpoint cho S3 và DynamoDB để dùng PrivateLink","Chuyển tất cả service calls qua một Internet Gateway gắn vào private subnet"],"correctIndices":[0,1],"explanation":"S3 và DynamoDB hỗ trợ Gateway endpoint (miễn phí), còn ECR và CloudWatch Logs cần Interface endpoint; kết hợp cả hai loại bỏ phần lớn traffic qua NAT Gateway.\n✓ Gateway endpoints cho S3 và DynamoDB miễn phí và đưa traffic ra khỏi NAT Gateway.\n✓ Interface endpoints cho ECR (ecr.api, ecr.dkr) và CloudWatch Logs cho phép truy cập riêng tư các service này không qua NAT.\n✗ NAT instance phải tự quản lý, không có HA mặc định và vẫn xử lý cùng lượng traffic — không phải tối ưu.\n✗ S3 và DynamoDB dùng Gateway endpoint (miễn phí), dùng Interface endpoint sẽ phát sinh phí giờ và phí xử lý không cần thiết.\n✗ Không thể gắn Internet Gateway để định tuyến cho một private subnet theo cách đó; điều đó biến nó thành public subnet và phá vỡ thiết kế.","domain":4,"mock":2},
  {"id":"saa-m3-063","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một công ty media streaming phục vụ video on-demand toàn cầu qua CloudFront với origin là S3. Họ muốn giảm thêm chi phí và có cam kết lưu lượng lớn ổn định hàng tháng, đồng thời một phần lớn người dùng nằm ở các khu vực mà giá data transfer của CloudFront cao. Cách nào giảm chi phí phân phối HIỆU QUẢ NHẤT mà vẫn giữ chất lượng phục vụ toàn cầu?","options":["Cấu hình CloudFront price class để giới hạn edge locations ở các Region chi phí thấp hơn, kết hợp với CloudFront Security Savings Bundle (cam kết mức sử dụng hàng tháng để được giảm giá tới 30%)","Tắt CloudFront và phục vụ trực tiếp từ S3 với S3 Transfer Acceleration để tiết kiệm phí CloudFront","Bật field-level encryption trên CloudFront để giảm kích thước payload truyền tải","Chuyển origin từ S3 sang một EC2 Auto Scaling group để kiểm soát chi phí egress tốt hơn"],"correctIndices":[0],"explanation":"Price class giới hạn việc dùng các edge locations đắt tiền (cân bằng chi phí/độ phủ), và CloudFront Security Savings Bundle giảm đơn giá cho lưu lượng cam kết hàng tháng — cả hai trực tiếp giảm chi phí phân phối.\n✓ Giới hạn price class loại bỏ chi phí từ edge locations đắt nhất, còn Security Savings Bundle cam kết mức sử dụng hàng tháng để giảm giá tới 30% trên CloudFront usage gồm data transfer out.\n✗ Phục vụ trực tiếp từ S3 mất caching ở edge, làm tăng origin egress và độ trễ, không tiết kiệm.\n✗ Field-level encryption bảo vệ dữ liệu nhạy cảm, không giảm chi phí data transfer.\n✗ EC2 origin tự quản lý phức tạp hơn và không vốn dĩ rẻ hơn S3 cho việc phục vụ nội dung tĩnh qua CloudFront.","domain":4,"mock":3},
  {"id":"saa-m1-065","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"medium","type":"single","question":"Một ứng dụng phân tán có các microservices chạy trên EC2 ở hai Region khác nhau (us-east-1 và eu-west-1) để phục vụ người dùng địa phương. Các service gọi chéo Region khá thường xuyên qua public endpoints, làm chi phí inter-Region data transfer và độ trễ tăng cao. Cách nào giảm chi phí và độ trễ của giao tiếp cross-Region HIỆU QUẢ NHẤT?","options":["Thiết kế lại để mỗi Region tự chứa dữ liệu cần thiết (data locality) và chỉ đồng bộ không thường xuyên qua kết nối riêng, giảm thiểu các cuộc gọi đồng bộ cross-Region","Định tuyến toàn bộ traffic cross-Region qua một NAT Gateway trung tâm để tổng hợp chi phí","Tăng kích thước instance ở cả hai Region để xử lý độ trễ cross-Region nhanh hơn","Dùng S3 Cross-Region Replication cho mọi request giữa các service"],"correctIndices":[0],"explanation":"Giảm số lượng cuộc gọi đồng bộ cross-Region qua data locality là cách trực tiếp nhất để cắt cả chi phí inter-Region transfer lẫn độ trễ.\n✓ Đưa dữ liệu về local và hạn chế đồng bộ cross-Region loại bỏ phần lớn inter-Region data transfer đắt đỏ và độ trễ.\n✗ NAT Gateway không xử lý inter-Region traffic theo cách này và chỉ thêm chi phí data processing.\n✗ Tăng kích thước instance không thay đổi lượng dữ liệu truyền giữa các Region hay đơn giá transfer.\n✗ Cross-Region Replication cho mọi request không phù hợp với giao tiếp service synchronous và tăng chi phí replication.","domain":4,"mock":1},
  {"id":"saa-m2-065","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"hard","type":"single","question":"Một hệ thống xử lý dữ liệu lớn dùng EMR đọc dữ liệu từ S3. Các EMR cluster nằm trong private subnets và truy cập S3 cùng Region qua NAT Gateway. Khối lượng đọc rất lớn (hàng trăm TB mỗi ngày), khiến phí data processing của NAT Gateway chiếm phần lớn hóa đơn mạng. Ngoài chi phí, một vấn đề khác là NAT Gateway giới hạn throughput. Giải pháp nào giải quyết CẢ chi phí lẫn throughput tốt nhất?","options":["Triển khai Gateway VPC endpoint cho S3 để traffic đi trực tiếp tới S3 không qua NAT Gateway","Nâng cấp lên nhiều NAT Gateway và bật enhanced networking trên các EMR nodes","Bật S3 Transfer Acceleration để tăng throughput đọc dữ liệu từ S3","Đặt một Interface VPC endpoint cho S3 để giảm phụ thuộc vào NAT Gateway"],"correctIndices":[0],"explanation":"Gateway endpoint cho S3 miễn phí và không bị giới hạn throughput như NAT Gateway, giải quyết đồng thời cost và bottleneck.\n✓ Gateway endpoint loại bỏ phí data processing của NAT Gateway và không áp đặt giới hạn throughput của NAT, lý tưởng cho khối lượng đọc S3 cực lớn.\n✗ Nhiều NAT Gateway vẫn phát sinh phí data processing cao và tốn kém hơn nhiều so với Gateway endpoint miễn phí.\n✗ Transfer Acceleration phụ thu phí và hướng tới truyền qua internet, không giải quyết NAT cost nội Region.\n✗ Interface endpoint cho S3 phát sinh phí giờ và phí xử lý — Gateway endpoint mới là lựa chọn miễn phí và phù hợp throughput cao.","domain":4,"mock":2},
  {"id":"saa-m3-064","courseId":"SAA-C03","lesson":"ch4-03-db-network-cost","certifications":["SAA-C03"],"difficulty":"easy","type":"single","question":"Một startup vận hành một ứng dụng đơn giản trong một VPC. Các instances trong private subnet chỉ thỉnh thoảng cần tải bản cập nhật phần mềm nhỏ từ internet qua NAT Gateway. Đội tài chính nhận thấy NAT Gateway tính phí theo giờ ngay cả khi gần như không có traffic. Đối với một workload lưu lượng rất thấp và không liên tục, lựa chọn nào tối ưu chi phí nhất trong khi vẫn cho phép outbound internet khi cần?","options":["Cân nhắc dùng NAT instance trên một instance nhỏ (ví dụ t-family) thay cho NAT Gateway để giảm chi phí cố định theo giờ cho workload lưu lượng thấp","Giữ NAT Gateway nhưng triển khai thêm ở mỗi AZ để giảm chi phí trung bình","Gán Elastic IP trực tiếp cho các private instances để chúng tự ra internet","Dùng một Interface VPC endpoint cho mọi truy cập internet outbound"],"correctIndices":[0],"explanation":"Với lưu lượng rất thấp và không liên tục, NAT instance trên một instance nhỏ có chi phí cố định thấp hơn NAT Gateway, dù phải tự quản lý.\n✓ NAT instance nhỏ tránh được phí theo giờ và phí data processing cao của NAT Gateway, phù hợp workload lưu lượng thấp với chi phí thấp hơn.\n✗ Thêm NAT Gateway ở mỗi AZ làm tăng tổng chi phí, không giảm.\n✗ Gán Elastic IP cho private instance không cho ra internet nếu không có Internet Gateway và route phù hợp — và làm vậy biến chúng thành public, kém an toàn.\n✗ VPC endpoint chỉ truy cập AWS services cụ thể, không cung cấp truy cập internet công cộng chung chung.","domain":4,"mock":3}
];
