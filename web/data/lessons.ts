...
const dstLessons: Lesson[] = [
  ...
  { slug: "dst-19-migrations-online-schema", courseId: "DATASTORES", title: "Migrations & Online Schema Change ở quy mô lớn", shortTitle: "Migrations at Scale", chapter: "dst-ch7", order: 20, available: true,
    description: "Vì sao migration/DDL làm căng IO (buffer cache eviction, WAL/redo doubling, IOPS budget) & khoá bảng; lock levels, CREATE INDEX CONCURRENTLY, online DDL (MySQL INSTANT/INPLACE); gh-ost & pt-online-schema-change hoạt động thế nào; expand-migrate-contract cho bảng 500M dòng, backfill throttle; ảnh hưởng replication lag. Playbook zero-downtime.", file: "datastores/dst-19-migrations-online-schema.md" },
  { slug: "dst-20-index-ops-scale", courseId: "DATASTORES", title: "Index ở production: build IO, bloat & maintenance", shortTitle: "Index Ops at Scale", chapter: "dst-ch7", order: 21, available: true,
    description: "Chi phí IO khi build index (scan + sort + write + WAL), index bloat & REINDEX CONCURRENTLY, phát hiện index thừa/trùng/không dùng, khi index làm chậm write, monitoring index (pg_stat_user_indexes), invisible index để test an toàn. CODE.", file: "datastores/dst-20-index-ops-scale.md" },
  { slug: "dst-21-storage-reclamation", courseId: "DATASTORES", title: "Storage reclamation & compaction (VACUUM, LSM)", shortTitle: "Vacuum & Compaction", chapter: "dst-ch7", order: 22, available: true,
    description: "Postgres MVCC sinh dead tuple → bloat; VACUUM/autovacuum, txid wraparound (nguy hiểm), tuning autovacuum, pg_repack; LSM-tree compaction (Cassandra/RocksDB): write amplification, compaction strategy (STCS/LCS), read/space amplification; vì sao 'xoá không giải phóng đĩa ngay'. Sơ đồ + CODE.", file: "datastores/dst-21-storage-reclamation.md" },
  { slug: "dst-22-replication-lag-ops", courseId: "DATASTORES", title: "Replication lag thực chiến & read-replica pitfalls", shortTitle: "Replication Lag Ops", chapter: "dst-ch7", order: 23, available: true,
    description: "Physical (WAL replay đơn luồng, standby query cancellation) vs logical (chạy lại statement) replication; nguyên nhân lag (DDL/backfill nặng, long transaction, IO); read-replica pitfall: read-your-writes bị stale, cách xử lý (sticky, đọc master, wait-for-LSN); giám sát lag & failover/split-brain. Sơ đồ + CODE.", file: "datastores/dst-22-replication-lag-ops.md" },
  { slug: "dst-23-connection-pool-scale", courseId: "DATASTORES", title: "Connection pool exhaustion & pooler (PgBouncer)", shortTitle: "Connection Pooling", chapter: "dst-ch7", order: 24, available: true,
    description: "Vì sao mỗi connection tốn RAM/process ở DB, pool exhaustion & thundering connections khi app scale; pool sizing đúng (công thức, không phải càng to càng tốt), PgBouncer (session/transaction/statement pooling), timeout cascade & fail-fast; liên hệ circuit breaker. CODE.", file: "datastores/dst-23-connection-pool-scale.md" },
  { slug: "dst-24-hotspot-noisy-neighbor", courseId: "DATASTORES", title: "Hot key, hot partition & noisy neighbor ở scale", shortTitle: "Hotspot & Skew", chapter: "dst-ch7", order: 25, available: true,
    description: "Data skew: hot partition (một shard quá tải), hot key (một key bị đọc/ghi dồn), celebrity problem; cách gỡ (key salting, split hot key, cache trước DB, request coalescing, đổi partition key); noisy neighbor trong multi-tenant & isolation. Sơ đồ + ví dụ.", file: "datastores/dst-24-hotspot-noisy-neighbor.md" },
  { slug: "dst-cap-data-incident", courseId: "DATASTORES", title: "Capstone: Chẩn đoán một sự cố data-layer thực chiến", shortTitle: "Capstone: Data Incident", chapter: "dst-ch7", order: 26, available: true,
    description: "Dự án tổng kết: một migration thêm index trên bảng lớn làm replica lag tăng vọt → read replica trả dữ liệu cũ + query bị cancel + app timeout dây chuyền. Điều tra end-to-end (đọc metric IO/lag, EXPLAIN, pg_stat_activity), tìm root cause, khắc phục (throttle/gh-ost/failover), và viết postmortem + phòng ngừa. Ghép cả chương.", file: "datastores/dst-cap-data-incident.md" },
];
...
const dstChapters: Chapter[] = [
  ...
  { id: "dst-ch7", courseId: "DATASTORES", title: "Chương 7 — Vận hành Data ở quy mô lớn (Thực chiến)", lessonSlugs: ["dst-19-migrations-online-schema", "dst-20-index-ops-scale", "dst-21-storage-reclamation", "dst-22-replication-lag-ops", "dst-23-connection-pool-scale", "dst-24-hotspot-noisy-neighbor", "dst-cap-data-incident"], category: "security" },
];
...
// =====================================================================
// GAMESERVER — Game Server Engineering (42 bài / 10 chương)
// =====================================================================
const gsLessons: Lesson[] = [
  { slug: "gs-01-why-not-be-app", courseId: "GAMESERVER", title: "Vì sao game server không thể là một BE App", shortTitle: "Không phải BE App", chapter: "gs-ch1", order: 1, available: true,
    description: "Thiết kế Redis N² vỡ ở người thứ 6 trong khi CPU 8%; vì sao RAM-first là bắt buộc chứ không phải tối ưu; stateful & một trận một chủ sở hữu; chậm bằng hỏng.", file: "gameserver/gs-01-why-not-be-app.md" },
  { slug: "gs-02-system-map", courseId: "GAMESERVER", title: "Bản đồ một hệ thống game online", shortTitle: "Bản đồ hệ thống", chapter: "gs-ch1", order: 2, available: true,
    description: "Nửa stateless (login, shop, leaderboard) vs nửa stateful (gateway → game node → simulation); ranh giới giữa hai nửa và cách nửa dưới sập mà nửa trên vẫn sống.", file: "gameserver/gs-02-system-map.md" },
  { slug: "gs-03-genre-decides-netcode", courseId: "GAMESERVER", title: "Thể loại game quyết định netcode", shortTitle: "Thể loại & netcode", chapter: "gs-ch1", order: 3, available: true,
    description: "Cờ vua → RTS lockstep → MOBA → FPS → MMO → .io; bảng yêu cầu độ trễ/tick/băng thông theo thể loại; vì sao không tồn tại 'netcode đúng'.", file: "gameserver/gs-03-genre-decides-netcode.md" },
  { slug: "gs-04-netcode-history", courseId: "GAMESERVER", title: "Lịch sử netcode: 1993 tới nay", shortTitle: "Lịch sử netcode", chapter: "gs-ch1", order: 4, available: true,
    description: "Doom lockstep → Quake client-server → QuakeWorld prediction (Carmack) → Valve lag compensation (Bernier, GDC 2001) → GGPO rollback; hai nhánh còn sống và vì sao.", file: "gameserver/gs-04-netcode-history.md" },
  { slug: "gs-05-fixed-timestep", courseId: "GAMESERVER", title: "Fixed timestep: vì sao dt phải là hằng số", shortTitle: "Fixed timestep", chapter: "gs-ch2", order: 5, available: false,
    description: "Sai số tích phân rời rạc: cùng công thức ở 30/60/144 Hz ra ba kết quả khác nhau; sai số giảm một nửa mỗi lần gấp đôi Hz; vì sao mục tiêu là 'giống nhau' chứ không phải 'chính xác'.", file: "gameserver/gs-05-fixed-timestep.md" },
  { slug: "gs-06-accumulator-spiral", courseId: "GAMESERVER", title: "Accumulator, catch-up & spiral of death", shortTitle: "Accumulator", chapter: "gs-ch2", order: 6, available: false,
    description: "Nối thời gian thật với thời gian mô phỏng; số tick ≠ số sim step; MaxCatchUp và công thức dropped = floor(stall/dt) − MaxCatchUp; chấp nhận mất để server sống.", file: "gameserver/gs-06-accumulator-spiral.md" },
  { slug: "gs-07-clock-scheduling", courseId: "GAMESERVER", title: "Đồng hồ & scheduling: drift, jitter, lateness", shortTitle: "Đồng hồ & drift", chapter: "gs-ch2", order: 7, available: false,
    description: "sleep(dt) vs deadline tuyệt đối; drift cộng dồn và tăng theo tải; monotonic vs wall clock; busy-wait đổi CPU lấy p99; vì sao metric luôn xanh là metric không so với gì cả.", file: "gameserver/gs-07-clock-scheduling.md" },
  { slug: "gs-08-three-rates-budget", courseId: "GAMESERVER", title: "Ba loại nhịp & ngân sách độ trễ", shortTitle: "Ba nhịp & ngân sách", chapter: "gs-ch2", order: 8, available: false,
    description: "sim tick / snapshot rate / render FPS là ba thứ khác nhau tiêu ba nguồn lực khác nhau; bóc tách ngân sách từ lúc bấm tới lúc người khác nhìn thấy; tối ưu chỗ ăn nhiều nhất.", file: "gameserver/gs-08-three-rates-budget.md" },
  { slug: "gs-09-why-determinism", courseId: "GAMESERVER", title: "Determinism dùng để làm gì", shortTitle: "Vì sao determinism", chapter: "gs-ch3", order: 9, available: false,
    description: "Bốn thứ dựng thẳng lên nó: reconciliation, replay, rollback, anti-cheat; vì sao đây là tính chất không thể bổ sung sau.", file: "gameserver/gs-09-why-determinism.md" },
  { slug: "gs-10-determinism-killers", courseId: "GAMESERVER", title: "Những kẻ giết determinism", shortTitle: "Kẻ giết determinism", chapter: "gs-ch3", order: 10, available: false,
    description: "IEEE-754 và thứ tự phép tính/FMA; ARM vs x86; hàm siêu việt; map order của Go; rand toàn cục; thứ tự duyệt entity; multi-thread; cách khoá lại bằng test.", file: "gameserver/gs-10-determinism-killers.md" },
  { slug: "gs-11-fixed-point", courseId: "GAMESERVER", title: "Fixed-point & simulation tất định tuyệt đối", shortTitle: "Fixed-point", chapter: "gs-ch3", order: 11, available: false,
    description: "Vì sao rollback thật phải bỏ float; số nguyên Q16.16; nhân/chia và tràn số; sqrt/sin bằng bảng tra; test đối chiếu hai kiến trúc CPU.", file: "gameserver/gs-11-fixed-point.md" },
  { slug: "gs-12-tcp-vs-udp", courseId: "GAMESERVER", title: "TCP hay UDP — tính bằng số", shortTitle: "TCP vs UDP", chapter: "gs-ch4", order: 12, available: false,
    description: "Head-of-line blocking: một packet mất làm nghẽn cả stream sau nó, tính ra mili giây; vì sao game action thà mất gói còn hơn chờ; khi nào TCP vẫn là lựa chọn đúng.", file: "gameserver/gs-12-tcp-vs-udp.md" },
  { slug: "gs-13-websocket-quic", courseId: "GAMESERVER", title: "WebSocket, QUIC & WebTransport", shortTitle: "WebSocket & QUIC", chapter: "gs-ch4", order: 13, available: false,
    description: "Game chạy trong trình duyệt bị ràng buộc gì; QUIC stream vs datagram; WebTransport thay WebSocket ở đâu; chọn gì cho game .io.", file: "gameserver/gs-13-websocket-quic.md" },
  { slug: "gs-14-reliability-layer", courseId: "GAMESERVER", title: "Tự viết reliability layer trên UDP", shortTitle: "Reliability layer", chapter: "gs-ch4", order: 14, available: false,
    description: "Sequence number, ack bitfield kiểu Fiedler, ước lượng RTT/RTO; phân loại message reliable-ordered / unreliable-sequenced và vì sao game cần cả hai.", file: "gameserver/gs-14-reliability-layer.md" },
  { slug: "gs-15-mtu-loss", courseId: "GAMESERVER", title: "Giới hạn vật lý của gói tin: MTU, loss, jitter", shortTitle: "MTU & packet loss", chapter: "gs-ch4", order: 15, available: false,
    description: "Mốc ~1200 byte an toàn; fragmentation nhân xác suất mất gói; phân biệt loss vs jitter vs bandwidth; vì sao snapshot phải tự giới hạn kích thước.", file: "gameserver/gs-15-mtu-loss.md" },
  { slug: "gs-16-nat-connection", courseId: "GAMESERVER", title: "NAT, vòng đời kết nối & phục hồi", shortTitle: "NAT & reconnect", chapter: "gs-ch4", order: 16, available: false,
    description: "Vì sao dedicated server không cần hole punching còn P2P thì cần; STUN/TURN; connection lifecycle; reconnect và session resume không mất trận.", file: "gameserver/gs-16-nat-connection.md" },
  { slug: "gs-17-input-tick-sync", courseId: "GAMESERVER", title: "Input pipeline & đồng bộ tick client–server", shortTitle: "Input & tick sync", chapter: "gs-ch5", order: 17, available: false,
    description: "Input mang seq + tick; client cố ý chạy trước server một khoảng; input buffer ở server; sửa lệch bằng co giãn tick chứ không nhảy cóc.", file: "gameserver/gs-17-input-tick-sync.md" },
  { slug: "gs-18-client-prediction", courseId: "GAMESERVER", title: "Client-side prediction", shortTitle: "Prediction", chapter: "gs-ch5", order: 18, available: false,
    description: "Client apply input ngay và giữ buffer input chưa được ack; vì sao logic phải dùng chung client/server; cái giá: viết luật chơi hai lần và cách giảm đau.", file: "gameserver/gs-18-client-prediction.md" },
  { slug: "gs-19-reconciliation", courseId: "GAMESERVER", title: "Server reconciliation & rewind-replay", shortTitle: "Reconciliation", chapter: "gs-ch5", order: 19, available: false,
    description: "Nhận authoritative state kèm lastProcessedSeq → rewind → replay input chưa ack; ngưỡng bỏ qua sai lệch nhỏ; vì sao lệch một chút là rung liên tục.", file: "gameserver/gs-19-reconciliation.md" },
  { slug: "gs-20-interpolation", courseId: "GAMESERVER", title: "Entity interpolation & extrapolation", shortTitle: "Interpolation", chapter: "gs-ch5", order: 20, available: false,
    description: "Render trễ 2 snapshot để mượt dù snapshot chỉ 20Hz; đánh đổi 100ms độ trễ lấy sự mượt; khi nào extrapolate và vì sao nó tạo hiệu ứng cao su.", file: "gameserver/gs-20-interpolation.md" },
  { slug: "gs-21-lag-compensation", courseId: "GAMESERVER", title: "Lag compensation: tua ngược thế giới", shortTitle: "Lag compensation", chapter: "gs-ch5", order: 21, available: false,
    description: "Server giữ lịch sử vị trí ~1 giây, tua ngược world về thời điểm người bắn đã nhìn thấy rồi mới raycast; vì sao bạn chết sau khi đã nấp; giới hạn công bằng.", file: "gameserver/gs-21-lag-compensation.md" },
  { slug: "gs-22-rollback-lockstep", courseId: "GAMESERVER", title: "Rollback netcode (GGPO) & lockstep", shortTitle: "Rollback & lockstep", chapter: "gs-ch5", order: 22, available: false,
    description: "Tua ngược 7 frame, thay input, chạy lại tới hiện tại trong một frame; vì sao fighting game chọn nhánh này; lockstep cho RTS; điều kiện determinism tuyệt đối.", file: "gameserver/gs-22-rollback-lockstep.md" },
  { slug: "gs-23-replication-models", courseId: "GAMESERVER", title: "Ba mô hình replication: snapshot, delta, event", shortTitle: "Mô hình replication", chapter: "gs-ch6", order: 23, available: false,
    description: "Full snapshot vs delta vs event-based; cái nào chịu được mất gói cái nào không; vì sao event-based buộc phải reliable và cái giá của nó.", file: "gameserver/gs-23-replication-models.md" },
  { slug: "gs-24-serialization-quantization", courseId: "GAMESERVER", title: "Serialization & quantization", shortTitle: "Serialization", chapter: "gs-ch6", order: 24, available: false,
    description: "JSON → binary đo bằng số thật; float32 → int16 theo lưới; góc gói trong 1 byte; bit packing; versioning schema khi client cũ chưa update.", file: "gameserver/gs-24-serialization-quantization.md" },
  { slug: "gs-25-delta-compression", courseId: "GAMESERVER", title: "Delta compression & baseline", shortTitle: "Delta compression", chapter: "gs-ch6", order: 25, available: false,
    description: "Chỉ gửi field đổi so với snapshot client đã ack; vòng đời baseline; chuyện gì xảy ra khi ack mất; server phải giữ bao nhiêu snapshot trong bộ nhớ.", file: "gameserver/gs-25-delta-compression.md" },
  { slug: "gs-26-area-of-interest", courseId: "GAMESERVER", title: "Area of Interest & interest management", shortTitle: "Area of Interest", chapter: "gs-ch6", order: 26, available: false,
    description: "Grid/cell, bán kính, visibility; vì sao AOI giảm băng thông nhiều hơn mọi thủ thuật serialization cộng lại; xử lý entity vào/ra tầm nhìn.", file: "gameserver/gs-26-area-of-interest.md" },
  { slug: "gs-27-priority-bandwidth", courseId: "GAMESERVER", title: "Priority & ngân sách băng thông", shortTitle: "Priority & bandwidth", chapter: "gs-ch6", order: 27, available: false,
    description: "Không phải entity nào cũng đáng gửi mỗi tick; priority accumulator; đóng gói vừa MTU; đo bytes/player/s và quy thẳng ra tiền.", file: "gameserver/gs-27-priority-bandwidth.md" },
  { slug: "gs-28-room-concurrency", courseId: "GAMESERVER", title: "Room model & concurrency", shortTitle: "Room model", chapter: "gs-ch7", order: 28, available: false,
    description: "Mỗi room = 1 goroutine simulation + inbox channel; reader goroutine cho mỗi connection; vì sao simulation phải single-threaded; data race kinh điển của người mới.", file: "gameserver/gs-28-room-concurrency.md" },
  { slug: "gs-29-gateway-gamenode", courseId: "GAMESERVER", title: "Gateway vs game node", shortTitle: "Gateway & node", chapter: "gs-ch7", order: 29, available: false,
    description: "Gateway lo auth/WebSocket, game node lo simulation; routing theo match_id; vì sao load balancer round-robin sai hoàn toàn ở đây.", file: "gameserver/gs-29-gateway-gamenode.md" },
  { slug: "gs-30-matchmaking", courseId: "GAMESERVER", title: "Matchmaking: queue, MMR, region", shortTitle: "Matchmaking", chapter: "gs-ch7", order: 30, available: false,
    description: "Hàng đợi và ghép cặp; MMR/ELO; region & ping; đánh đổi thời gian chờ vs chất lượng trận; backfill; vì sao đây là bài toán tối ưu chứ không phải hàng đợi.", file: "gameserver/gs-30-matchmaking.md" },
  { slug: "gs-31-persistence-shutdown", courseId: "GAMESERVER", title: "Persistence, meta services & graceful shutdown", shortTitle: "Persistence", chapter: "gs-ch7", order: 31, available: false,
    description: "Ba tầng state simulation/session/persistent; DB ngoài critical path; snapshot theo chu kỳ; drain và flush state trước khi node chết.", file: "gameserver/gs-31-persistence-shutdown.md" },
  { slug: "gs-32-sharding-handoff", courseId: "GAMESERVER", title: "Sharding world & cross-node handoff", shortTitle: "Sharding & handoff", chapter: "gs-ch7", order: 32, available: false,
    description: "Chia zone/grid khi một trận quá đông; chuyển entity giữa node; vì sao cắt thế giới là quyết định thiết kế game chứ không phải quyết định hạ tầng.", file: "gameserver/gs-32-sharding-handoff.md" },
  { slug: "gs-33-authority-intent", courseId: "GAMESERVER", title: "Authority: client gửi ý định, không gửi kết quả", shortTitle: "Intent vs result", chapter: "gs-ch8", order: 33, available: false,
    description: "Phân loại gói tin theo ý định/kết quả; mọi tham số trỏ tới thế giới là một khẳng định cần kiểm chứng; cái giá thật của authoritative server.", file: "gameserver/gs-33-authority-intent.md" },
  { slug: "gs-34-cheat-taxonomy", courseId: "GAMESERVER", title: "Phân loại cheat & phòng thủ server-side", shortTitle: "Phân loại cheat", chapter: "gs-ch8", order: 34, available: false,
    description: "Speed hack, teleport, aimbot, wallhack, item duplication, replay attack; cái nào server chặn được tuyệt đối, cái nào chỉ giảm được, cái nào không chặn nổi.", file: "gameserver/gs-34-cheat-taxonomy.md" },
  { slug: "gs-35-anticheat-detection", courseId: "GAMESERVER", title: "Phát hiện gian lận & vận hành", shortTitle: "Phát hiện & vận hành", chapter: "gs-ch8", order: 35, available: false,
    description: "Thống kê server-side (aim pattern, input bất khả thi); replay verification; anti-cheat phía client và giới hạn của nó; xử lý false positive.", file: "gameserver/gs-35-anticheat-detection.md" },
  { slug: "gs-36-measuring", courseId: "GAMESERVER", title: "Đo cái gì và đo thế nào", shortTitle: "Đo lường", chapter: "gs-ch9", order: 36, available: false,
    description: "Ngân sách một tick đi đâu; luôn nhìn p99 không nhìn trung bình; tick time, số tick trễ, bytes/player/s, phân phối RTT; dùng pprof đúng chỗ.", file: "gameserver/gs-36-measuring.md" },
  { slug: "gs-37-zero-alloc", courseId: "GAMESERVER", title: "Zero-allocation hot loop & bố trí dữ liệu", shortTitle: "Zero-alloc & GC", chapter: "gs-ch9", order: 37, available: false,
    description: "GC là kẻ thù của tick loop; pre-allocate, sync.Pool, tránh interface boxing và closure; GOGC/GOMEMLIMIT; SoA/ECS-lite — bố trí dữ liệu quan trọng hơn số phép tính.", file: "gameserver/gs-37-zero-alloc.md" },
  { slug: "gs-38-concurrency", courseId: "GAMESERVER", title: "Concurrency đúng cách trong game server", shortTitle: "Concurrency", chapter: "gs-ch9", order: 38, available: false,
    description: "1 goroutine/connection cho I/O nhưng single-threaded cho simulation; channel vs mutex; backpressure khi client đọc chậm; tránh goroutine leak.", file: "gameserver/gs-38-concurrency.md" },
  { slug: "gs-39-load-testing", courseId: "GAMESERVER", title: "Load test bằng bot client headless", shortTitle: "Load test", chapter: "gs-ch9", order: 39, available: false,
  ...
  { slug: "gs-40-deploy-stateful", courseId: "GAMESERVER", title: "Deploy & vận hành hệ stateful", shortTitle: "Deploy stateful", chapter: "gs-ch9", order: 40, available: false,
...
const gsChapters: Chapter[] = [
  { id: "gs-ch1", courseId: "GAMESERVER", title: "Chương 1 — Mô hình tư duy", lessonSlugs: ["gs-01-why-not-be-app", "gs-02-system-map", "gs-03-genre-decides-netcode", "gs-04-netcode-history"], category: "foundation" },
  { id: "gs-ch2", courseId: "GAMESERVER", title: "Chương 2 — Thời gian & vòng lặp", lessonSlugs: ["gs-05-fixed-timestep", "gs-06-accumulator-spiral", "gs-07-clock-scheduling", "gs-08-three-rates-budget"], category: "foundation" },
  { id: "gs-ch3", courseId: "GAMESERVER", title: "Chương 3 — Determinism", lessonSlugs: ["gs-09-why-determinism", "gs-10-determinism-killers", "gs-11-fixed-point"], category: "foundation" },
  { id: "gs-ch4", courseId: "GAMESERVER", title: "Chương 4 — Mạng nền tảng", lessonSlugs: ["gs-12-tcp-vs-udp", "gs-13-websocket-quic", "gs-14-reliability-layer", "gs-15-mtu-loss", "gs-16-nat-connection"], category: "network" },
  { id: "gs-ch5", courseId: "GAMESERVER", title: "Chương 5 — Netcode lõi", lessonSlugs: ["gs-17-input-tick-sync", "gs-18-client-prediction", "gs-19-reconciliation", "gs-20-interpolation", "gs-21-lag-compensation", "gs-22-rollback-lockstep"], category: "compute" },
  { id: "gs-ch6", courseId: "GAMESERVER", title: "Chương 6 — Đồng bộ state & băng thông", lessonSlugs: ["gs-23-replication-models", "gs-24-serialization-quantization", "gs-25-delta-compression", "gs-26-area-of-interest", "gs-27-priority-bandwidth"], category: "network" },
  { id: "gs-ch7", courseId: "GAMESERVER", title: "Chương 7 — Kiến trúc & scale", lessonSlugs: ["gs-28-room-concurrency", "gs-29-gateway-gamenode", "gs-30-matchmaking", "gs-31-persistence-shutdown", "gs-32-sharding-handoff"], category: "compute" },
  { id: "gs-ch8", courseId: "GAMESERVER", title: "Chương 8 — Chống gian lận", lessonSlugs: ["gs-33-authority-intent", "gs-34-cheat-taxonomy", "gs-35-anticheat-detection"], category: "security" },
  { id: "gs-ch9", courseId: "GAMESERVER", title: "Chương 9 — Hiệu năng & vận hành", lessonSlugs: ["gs-36-measuring", "gs-37-zero-alloc", "gs-38-concurrency", "gs-39-load-testing", "gs-40-deploy-stateful"], category: "compute" },
  { id: "gs-ch10", courseId: "GAMESERVER", title: "Chương 10 — Capstone agar-lite", lessonSlugs: ["gs-41-capstone-1", "gs-42-capstone-2"], category: "compute" },
];

// =====================================================================
// Aggregate
// =====================================================================
export const lessons: Lesson[] = [...techLessons, ...progLessons, ...webLessons, ...sqlLessons, ...gitLessons, ...sysdLessons, ...foundLessons, ...engLessons, ...beLessons, ...csLessons, ...dsaLessons, ...secLessons, ...devopsLessons, ...sreLessons, ...aimlLessons, ...feLessons, ...capLessons, ...clfLessons, ...saaLessons, ...dvaLessons, ...bcLessons, ...dsLessons, ...dstLessons, ...msgLessons, ...cnLessons, ...gsLessons];
export const chapters: Chapter[] = [...techChapters, ...progChapters, ...webChapters, ...sqlChapters, ...gitChapters, ...sysdChapters, ...foundChapters, ...engChapters, ...beChapters, ...csChapters, ...dsaChapters, ...secChapters, ...devopsChapters, ...sreChapters, ...aimlChapters, ...feChapters, ...capChapters, ...clfChapters, ...saaChapters, ...dvaChapters, ...soaChapters, ...sapChapters, ...sysdChapters, ...bcChapters, ...dsChapters, ...dstChapters, ...msgChapters, ...cnChapters, ...gsChapters];
